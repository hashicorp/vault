...
var (
	metricLockUnlock  = []string{"oci_objectstorage", "lock", "unlock"}
	metricLockLock    = []string{"oci_objectstorage", "lock", "lock"}
	metricLockValue   = []string{"oci_objectstorage", "lock", "Value"}
	metricLeaderValue = []string{"oci_objectstorage", "leader", "Value"}
)
...
func (l *Lock) Lock(stopCh <-chan struct{}) (<-chan struct{}, error) {
	l.backend.logger.Debug("physical/oci: Lock() called")
...
func (l *Lock) attemptLock(stopCh <-chan struct{}) (bool, error) {
	l.backend.logger.Debug("physical/oci: AttemptLock() called")
...
func (l *Lock) renewLock() {
	l.backend.logger.Debug("physical/oci: RenewLock() called")
...
func (l *Lock) watchLock() {
	l.backend.logger.Debug("physical/oci: WatchLock() called")
	retries := 0
...
OUTER:
	for {
		...
		select {
		case <-l.stopCh:
			l.backend.logger.Debug("physical/oci: WatchLock():Stop lock signaled/closed.")
			break OUTER
		default:
		}
		...
		if retries >= l.watchRetryMax-1 {
			l.backend.logger.Debug("physical/oci: WatchLock: Failed to get lock data from object storage. Giving up the lease after max retries")
			break OUTER
		}
		...
		if (lockRecordCache == nil) ||
			...
			(time.Now().UTC().Sub(lockRecordCache.time) > l.ttl) {
			l.backend.logger.Debug("physical/oci: WatchLock: Lock record cache is nil, stale or does not belong to self.")
			break OUTER
		}
		...
		if err != nil {
			retries++
			l.backend.logger.Debug("physical/oci: WatchLock: Failed to get lock data from object storage. Retrying..")
		...
		if (lockRecord == nil) || (lockRecord.Identity != l.identity) {
			l.backend.logger.Debug("physical/oci: WatchLock: Lock record cache is nil or does not belong to self.")
			break OUTER
		}

		// reset retries counter on success
		retries = 0
		l.backend.logger.Debug("physical/oci: WatchLock() successful")
	...
	if !l.stopped {
		l.stopped = true
		l.backend.logger.Debug("physical/oci: Closing the stop channel to give up leadership.")
		close(l.stopCh)
	}
}

func (l *Lock) Unlock() error {
	l.backend.logger.Debug("physical/oci: Unlock() called")
	...
	if currentLockRecord != nil && currentLockRecord.Identity == l.identity {
		...
		if err != nil {
			l.backend.logger.Debug("physical/oci: Unlock: error generating UUID")
			opcClientRequestId = ""
		}
		l.backend.logger.Debug("physical/oci: Unlock", "opc-client-request-id", opcClientRequestId)
...
func (l *Lock) Value() (bool, string, error) {
	l.backend.logger.Debug("physical/oci: Value() called")
...
func (l *Lock) get(ctx context.Context) (*LockRecord, string, error) {
	...
	opcClientRequestId, err := uuid.GenerateUUID()
	if err != nil {
		l.backend.logger.Debug("physical/oci: getHa: error generating UUID")
		opcClientRequestId = ""
	}
	l.backend.logger.Debug("physical/oci: getHa", "opc-client-request-id", opcClientRequestId)
	...
	if err != nil {
		...
		l.backend.logger.Error("physical/oci: Error calling GET", "err", err)
	...
	if err != nil {
		metrics.SetGauge(metricGetFailed, 1)
		l.backend.logger.Error("physical/oci: Error reading content", "err", err)
	...
	if err != nil {
		metrics.SetGauge(metricGetFailed, 1)
		l.backend.logger.Error("physical/oci: Error un-marshalling content", "err", err)
		return nil, "", errwrap.Wrapf(fmt.Sprintf("failed to read Value for %q: {{err}}", l.key), err)
	}
...
func (l *Lock) writeLock() (bool, error) {
	l.backend.logger.Debug("physical/oci: WriteLock() called")
	...
	lockRecordCache := loadLockRecordCache(l)
	if (lockRecordCache == nil) || lockRecordCache.lockRecord == nil || lockRecordCache.lockRecord.Identity != l.identity || time.Now().UTC().Sub(lockRecordCache.time) > l.ttl {
		// case secondary
		...
		cachedLockRecordJson, err := json.Marshal(l.lockRecordCache)
		if err != nil {
			return false, errwrap.Wrapf("error marshalling lock record: {{err}}", err)
		}

		l.backend.logger.Debug("physical/oci: Cached lock record", string(cachedLockRecordJson))

		// Current lock record being null implies that there is no leader. In this case we want to try acquiring lock.
		if currentLockRecord != nil && time.Now().UTC().Sub(lockRecordCache.time) < LockCacheMinAcceptableAge {
			return false, nil
		}
	...
	opcClientRequestId, err := uuid.GenerateUUID()
	if err != nil {
		l.backend.logger.Debug("physical/oci: putHa: error generating UUID")
		opcClientRequestId = ""
	}
	l.backend.logger.Debug("physical/oci: putHa", "opc-client-request-id", opcClientRequestId)
	...
	for i := 1; i <= LockWriteRetriesOnFailures; i++ {
		...
		if putObjectResponse.RawResponse == nil {
			...
			l.backend.logger.Error("physical/oci PUT", "err", err)
			break
		}
		...
		if (putObjectResponse.RawResponse.StatusCode / 100) == 5 {
			metrics.SetGauge(metricPutFailed, 1)
			l.backend.logger.Warn("physical/oci PUT. Retrying..", "err", err)
	...
	l.backend.logger.Debug("physical/oci: Lock written", string(newLockRecordJson))
