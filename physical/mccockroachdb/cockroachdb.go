...
// Verify MultiClusterCockroachDB satisfies the correct interfaces
var (
	_ physical.Backend       = (*MultiClusterCockroachDB)(nil)
	_ physical.Transactional = (*MultiClusterCockroachDB)(nil)
)

var privatePaths = []string{"counters", "leader"}
...
// MultiClusterCockroachDB Backend is a physical backend that stores data
// within a CockroachDB database.
type MultiClusterCockroachDB struct {
	hostname        string
	table           string
	haTable         string
	client          *sql.DB
...
func (c *MultiClusterCockroachDB) bucketName(name string) string {
	if name == "" {
		name = "."
	}
...
func (c *MultiClusterCockroachDB) buckets(key string) []string {
...
func (c *MultiClusterCockroachDB) isPrivate(key string) bool {
	buckets := c.buckets(key)
	for i := range privatePaths {
		for j := range buckets {
			if strings.Contains(buckets[j], privatePaths[i]) {
				return true
			}
		}
	}
...
// NewMultiDCCockroachDBBackend constructs a CockroachDB backend using the given
// API client, server address, credentials, and database.
func NewMultiDCCockroachDBBackend(conf map[string]string, logger log.Logger) (physical.Backend, error) {
	...
	privateTable := dbTable + "_" + hostname
	createPrivateTableQuery := "CREATE TABLE IF NOT EXISTS " + privateTable + " (path STRING, value BYTES, PRIMARY KEY (path))"
	if _, err := db.Exec(createPrivateTableQuery); err != nil {
		return nil, fmt.Errorf("failed to create MultiDCCockoachDB private table: %w", err)
	}

	// Setup the backend
	c := &MultiClusterCockroachDB{
		table:    dbTable,
		hostname: hostname,
		haTable:  dbHATable,
		client:   db,
		rawStatements: map[string]string{
			...
			"putPrivate": "INSERT INTO " + privateTable + " VALUES($1, $2)" +
				" ON CONFLICT (path) DO " +
				" UPDATE SET (path, value) = ($1, $2)",
			"getPrivate":    "SELECT value FROM " + privateTable + " WHERE path = $1",
			"get":           "SELECT value FROM " + dbTable + " WHERE path = $1",
			"deletePrivate": "DELETE FROM " + privateTable + " WHERE path = $1",
			"delete":        "DELETE FROM " + dbTable + " WHERE path = $1",
			"list":          "SELECT path FROM " + dbTable + " WHERE path LIKE $1",
			"listPrivate":   "SELECT path FROM " + privateTable + " WHERE path LIKE $1",
		},
...
func (c *MultiClusterCockroachDB) prepare(statementMap map[string]*sql.Stmt, name, query string) error {
...
func (c *MultiClusterCockroachDB) Put(ctx context.Context, entry *physical.Entry) error {
	...
	stmt := "put"
	if c.isPrivate(entry.Key) {
		stmt = "putPrivate"
	}

	_, err := c.statements[stmt].Exec(entry.Key, entry.Value)
	if err != nil {
		return err
	}
	return nil
...
func (c *MultiClusterCockroachDB) Get(ctx context.Context, key string) (*physical.Entry, error) {
	...
	err := c.statements[stmt].QueryRow(key).Scan(&result)
...
func (c *MultiClusterCockroachDB) Delete(ctx context.Context, key string) error {
	...
	_, err := c.statements[stmt].Exec(key)
	if err != nil {
		return err
	}
	return nil
...
func (c *MultiClusterCockroachDB) List(ctx context.Context, prefix string) ([]string, error) {
	...
	stmt := "list"
	if c.isPrivate(prefix) {
		stmt = "listPrivate"
	}

	rows, err := c.statements[stmt].Query(likePrefix)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (c *MultiClusterCockroachDB) Transaction(ctx context.Context, txns []*physical.TxnEntry) error {
...
func (c *MultiClusterCockroachDB) transaction(tx *sql.Tx, txns []*physical.TxnEntry) error {

	deleteStmt, err := tx.Prepare(c.rawStatements["delete"])
	if err != nil {
		return err
	}
	deleteStmtPrivate, err := tx.Prepare(c.rawStatements["deletePrivate"])
	if err != nil {
		return err
	}
	...
	putStmtPrivate, err := tx.Prepare(c.rawStatements["putPrivate"])
	if err != nil {
		return err
	}
	...
	for _, op := range txns {
		...
		case physical.DeleteOperation:
			if c.isPrivate(op.Entry.Key) {
				_, err = deleteStmtPrivate.Exec(op.Entry.Key)
			} else {
				_, err = deleteStmt.Exec(op.Entry.Key)
			}
		case physical.PutOperation:
			if c.isPrivate(op.Entry.Key) {
				_, err = putStmtPrivate.Exec(op.Entry.Key, op.Entry.Value)
			} else {
