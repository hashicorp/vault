...
func getAuthConfig(ctx context.Context, url string, conf map[string]string, log log.Logger) (pgmultiauth.Config, error) {
	authOptons := pgmultiauth.DefaultAuthConfigOptions{
		AuthMethod: pgmultiauth.StandardAuth,
	}

	if conf["use_aws_iam"] == "true" {
		authOptons.AuthMethod = pgmultiauth.AWSAuth
		authOptons.AWSDBRegion = conf["aws_db_region"]
	} else if conf["use_azure_msi"] == "true" {
		authOptons.AuthMethod = pgmultiauth.AzureAuth
		authOptons.AzureClientID = conf["azure_client_id"]
	} else if conf["use_gcp_default_credentials"] == "true" {
		authOptons.AuthMethod = pgmultiauth.GCPAuth
	}

	config, err := pgmultiauth.DefaultConfig(ctx, url, authOptons, pgmultiauth.WithLogger(log))
	if err != nil {
