...
func newJWTAuthMethodFromFile(conf *auth.AuthConfig) (auth.AuthMethod, error) {
	j := &jwtMethodFromFile{
		logger:          conf.Logger,
...
func (j *jwtMethodFromFile) Authenticate(_ context.Context, client *api.Client) (string, http.Header, map[string]interface{}, error) {
...
func (j *jwtMethodFromFile) NewCreds() chan struct{} {
	return j.credsFound
}

func (j *jwtMethodFromFile) CredSuccess() {
	j.once.Do(func() {
		close(j.credSuccessGate)
	})
}

func (j *jwtMethodFromFile) Shutdown() {
	j.ticker.Stop()
	close(j.stopCh)
	<-j.doneCh
}

func (j *jwtMethodFromFile) runWatcher() {
...
func (j *jwtMethodFromFile) ingressToken() {
