...
func NewKerberosAuthMethod(conf *auth.AuthConfig) (auth.AuthMethod, error) {
	...
	disableFASTNegotiation := false
	if disableFASTNegotiationRaw, ok := conf.Config["disable_fast_negotiation"]; ok {
		disableFASTNegotiation, err = parseutil.ParseBool(disableFASTNegotiationRaw)
		if err != nil {
			return nil, errwrap.Wrapf("error parsing 'disable_fast_negotiation' value: {{err}}", err)
		}
	}
