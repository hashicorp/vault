...
func TestJWTEndToEndFromEnvVar(t *testing.T) {
	testJWTEndToEndFromEnvVar(t, false)
	testJWTEndToEndFromEnvVar(t, true)
}

const ENV_VAR_NAME = "AUTH_JWT_TEST"

func testJWTEndToEndFromEnvVar(t *testing.T, ahWrapping bool) {
	...
	err = os.Setenv(ENV_VAR_NAME, jwtToken)
	if err != nil {
		t.Fatal(err)
	} else {
		logger.Trace("set test jwt", "var", ENV_VAR_NAME)
	}
	...
	// to the handler
	jwtToken, _ = GetTestJWT(t)
	if err := os.Setenv(ENV_VAR_NAME, jwtToken); err != nil {
		t.Fatal(err)
	}
