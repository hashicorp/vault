...
func TestTokenPreloadFile_UsingAutoAuth(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("error determining home directory: %s", err)
	}

	err = ioutil.WriteFile(fmt.Sprintf("%s/.vault-token", home), []byte(token), 0644)
...
func TestTokenPreloadEnv_UsingAutoAuth(t *testing.T) {
	logger := logging.NewVaultLogger(hclog.Trace)
	coreConfig := &vault.CoreConfig{
		Logger: logger,
		LogicalBackends: map[string]logical.Factory{
	...
	_ = os.Remove(fmt.Sprintf("%s/.vault-token", home))
