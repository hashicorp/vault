...
func TestServer(t *testing.T) {
	...
	keyPath := wd + "reload_foo.key"
	certPath := wd + "reload_foo.pem"
	...
	cases := []struct {
		...
		{
			"common_ha",
			testBaseHCL(t, fmt.Sprintf(enableTLS, certPath, keyPath)) + enableTLS + inmemHCL,
			"(HA available)",
			0,
			"-test-verify-only",
		},
		{
			"separate_ha",
			testBaseHCL(t, fmt.Sprintf(enableTLS, certPath, keyPath)) + inmemHCL + haInmemHCL,
			"HA Storage:",
			0,
			"-test-verify-only",
		},
		{
			"bad_separate_ha",
			testBaseHCL(t, fmt.Sprintf(enableTLS, certPath, keyPath)) + inmemHCL + badHAInmemHCL,
		...
		},
		{
			...
			testBaseHCL(t, goodListenerTimeouts+fmt.Sprintf(enableTLS, certPath, keyPath)) + inmemHCL,
			"",
			0,
			"-test-server-config",
		},
		{
			...
			testBaseHCL(t, badListenerReadHeaderTimeout+fmt.Sprintf(enableTLS, certPath, keyPath)) + inmemHCL,
		...
		},
		{
			...
			testBaseHCL(t, badListenerReadTimeout+fmt.Sprintf(enableTLS, certPath, keyPath)) + inmemHCL,
		...
		},
		{
			...
			testBaseHCL(t, badListenerWriteTimeout+fmt.Sprintf(enableTLS, certPath, keyPath)) + inmemHCL,
		...
		},
		{
			...
			testBaseHCL(t, badListenerIdleTimeout+fmt.Sprintf(enableTLS, certPath, keyPath)) + inmemHCL,
			"unknown unit \"gophers\" in duration \"78gophers\"",
			1,
			"-test-server-config",
		},
		{
			"bad_listener_tls_disabled",
			testBaseHCL(t, disableTLS) + inmemHCL,
			"invalid configuration: all HTTP listeners must have TLS enabled",
			1,
			"-test-server-config",
		},
	}
