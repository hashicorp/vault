...
func TestOperatorDiagnoseCommand_Run(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			[]string{
				"-config", "./server/test-fixtures/invalid_config_duplicate_key.hcl",
			},
			[]*diagnose.Result{
				{
					Name:    "Parse Configuration",
					Status:  diagnose.ErrorStatus,
					Message: "Could not parse configuration: error loading configuration from ./server/test-fixtures/invalid_config_duplicate_key.hcl: The argument \"address\" at 11:3 was already set. Each argument can only be defined once.",
				},
			},
		},
	}
