...
// TestIsLeaseRenewalRequest verifies that lease-renewal endpoints are detected
// across namespaced, lease-ID-suffixed, and legacy-alias forms, and that
// unrelated paths (including token renewal and revocation) are not matched.
func TestIsLeaseRenewalRequest(t *testing.T) {
	cases := []struct {
		path string
		want bool
	}{
		...
		{"/v1/auth/token/renew", false},
		{"/v1/auth/token/renew-self", false},
		{"/v1/secret/data/foo", false},
	}
	for _, tc := range cases {
		req := httptest.NewRequest(http.MethodPut, "http://example.com"+tc.path, nil)
		if got := isLeaseRenewalRequest(req); got != tc.want {
			t.Errorf("isLeaseRenewalRequest(%q) = %v, want %v", tc.path, got, tc.want)
		}
	}
}
