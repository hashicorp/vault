...
- `account_name` `(string: "" <required - if generate is true and url is empty>)` – Specifies the name of the account associated with the key.
...
- `qr_size` `(int: 200)` – Specifies the pixel size of the square QR code when generating a new key. Only used if generate is true.
...
To embed the generated barcode in a web page, include the base64 string in an `img` tag with the prefix `data:image/png;base64`
