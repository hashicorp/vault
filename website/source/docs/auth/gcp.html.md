---
layout: "docs"
page_title: "Auth Backend: GCP"
sidebar_current: "docs-auth-gcp"
description: |-
  The gcp backend allows automated authentication of AWS entities.
---
...
The `gcp` backend allows authentication against Vault using Google credentials.
It treats GCP as a Trusted Third Party and expects a
...
Currently the backend allows of authentication of:
...
* **Roles**: Roles are associated with a authentication type/entity and a set of
...
The Vault authentication workflow for IAM service accounts is as follows (see diagram):
...
an authorized entity must fit in order to login. This restrictions are specific
to the role type.
...
TTLs. For a full list of accepted arguments, see [role path usage](#roles).
...
Parameters:
  ...
  * **'credentials'**: The contents of a Google credentials JSON file to be
    used by Vault to make calls to GCP APIs. If you would rather specify the
    file, you can use `credentials="@path/to/creds.json`.

    If this value is not specified or if it is explicitly set to empty, Vault will attempt to use [Application Default Credentials](https://developers.google.com/identity/protocols/application-default-credentials) for that server's machine.

The GCP permissions Vault currently requires are:
...
```sh
vault write auth/gcp/role/myrole \
  type='iam' \
  ...

vault read auth/gcp/role/myrole
...
Parameters:
  ...
  * `type`: Required. Type of the role. Currently supported: `iam`.
  * `project_id`: Required. GCP project ID that authorized entities must belong to.
  * `policies`: Optional, defaults to `[]"default"]`. A comma-separated   
  list of Vault policy names.
  * `max_jwt_exp`: Optional, defaults to 900 (15 min). Number of seconds past
    the time of authentication that the JWT given during login must expire
    within.
  * `ttl`: Defaults to 0. TTL of the resulting auth token.
  * `max_ttl`: Defaults to 0. Max TTL of the resulting auth token, including renewals.
  * `period`: Defaults to 0. If set to a non-zero value, the token generated
    using this role should never expire. The token should be renewed within the
 		duration specified by this value. At each renewal, the token's TTL will
 		be set to the value of this parameter.

`iam` Roles Only:
  * `service_accounts`: A comma-separated list of service account emails or IDs
  that restrict authorized service accounts to this list. If you set this to `*`,
  this will accept all service accounts (the role will still be restricted by
  project id).
...
Parameters:
  * `values`: Service accounts to add or remove.
...
The GCP auth backend has a full HTTP API. Please see the
...
Please file all feature requests, bugs, and pull requests specific to the GCP backend here.
