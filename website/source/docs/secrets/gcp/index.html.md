...
~> **WARNING (Deprecation of `access_token` Leases)**: In previous versions of this
    secrets engine, a lease was generated with access tokens. We have removed them after
    discovering that Google OAuth tokens specifically for IAM service accounts are non-revocable 
    and have a static 60 minute lifetime, and ignore revocation or different expiration times.
    To match these current limitations of the GCP APIs, the secrets engine will no longer
    allow for revocation or manage the token TTL - more specifically, **the access_token
     response will no longer include `lease_id` or other lease information**. See the
    [documentation for access tokens](#access-tokens) to see the new returned data format
    and  the [upgrade guide](#deprecation-of-access-token-leases) to learn more.
    This change does not reflect any change to the actual underlying OAuth tokens or GCP service accounts.
    **Note that this ONLY affects the `access_token` and not `service_account_key` secret type.**
...
created as type `access_token`:
    ...
    token                 ya29.c.ElodBmNPwHUNY5gcBpnXcE4ywG4w1kmyfaketoken...
    token_ttl             3599
    ```
...
```sh
$ curl -H "Authorization: Bearer ya29.c.ElodBmNPwHUNY5gcBpnXcE4ywG4w1kmyfaketoken..."
```    
...
~> **NOTE**: This only affects access tokens. 

In previous versions of this secrets engine, a lease was generated with access tokens. 
We removed them after it was discovered that these OAuth access tokens are irrevocable (specifically
only for IAM service accounts). 
