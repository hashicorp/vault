...
if [ ! -d "$PREVIEW_DIR" ]; then
    ...
    git clone --depth=1 --branch=brk.feat/developer-preview https://github.com/hashicorp/$REPO_TO_CLONE.git "$PREVIEW_DIR"
    should_pull=false
fi
...
if [ "$should_pull" = true ]; then
    git pull origin brk.feat/developer-preview
fi
