...
- `rotation_url` `(string: "")` – <EnterpriseAlert product="vault" inline /> The LDAP server to connect to for password rotation if it is different from `url` parameter. Examples: `ldap://ldap.myorg.com`, `ldaps://ldap.myorg.com:636`.
