...
- `ocsp_fail_open` `(bool: false)` - If true and an OCSP response cannot be fetched
  ...
  been revoked.
- `ocsp_this_update_max_ttl` `(integer:0 or string: "")` - If greater than 0, specifies
  the maximum TTL of an OCSP thisUpdate field. This avoids accepting old responses
