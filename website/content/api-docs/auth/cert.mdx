...
- `allowed_organizations` `(string: "" or array: [])` - Constrain the
  Organization (O) in the client certificate with a [globbed pattern](https://github.com/ryanuber/go-glob/blob/master/README.md#example). Value is
  a comma-separated list of O patterns. Authentication requires at least one
  O matching at least one pattern. If not set, defaults to allowing all Os.
