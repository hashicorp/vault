...
- `bound_audiences` `(array: <optional>)` - List of `aud` claims to match against.
  This parameter is **mandatory** for "jwt" roles if the JWT contains an audience (which is more often than not the case),
  and **must** match at least one of the JWT's associated `aud` claims. Optional for "oidc" roles.
