...
- `alias_name_source` `(string: "serviceaccount_uid")` - Configures how identity aliases are generated.
  ...
  When `serviceaccount_uid` is specified, the machine generated UID from the service account
  will be used as the identity alias name - this is the more secure option, and hence the
  default and the recommended option.

  When `serviceaccount_name` is specified, the service account's namespace and name will be
  used as the identity alias name e.g `vault/vault-auth`. If you can ensure strong controls
  around the creation/deletion/access of your Kubernetes service accounts, and want more
  control over how your Kubernetes service accounts map to Vault identities or the simplification
  of setting the entity alias ahead of time of the Kubernetes service account creation you may 
  use `serviceaccount_name` where the risks of this are mitigated or acceptable given your use case. 
