...
- `kubernetes_ca_cert` `(string: "")` - Optional PEM encoded CA cert for use by the TLS client used to talk with the Kubernetes API.
  ...
  If not set and `disable_local_ca_jwt` set to true, the system's trusted CA certificate pool will be used.
...
If you override the default cert behavior by setting `disable_local_ca_jwt` to `true`,
the plugin's TLS client will automatically default to use the system's trust store for TLS certificate verification.
