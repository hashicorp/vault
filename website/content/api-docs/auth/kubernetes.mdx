...
- `use_annotations_as_alias_metadata` `(bool: false)` - Indicate annotations with `vault.hashicorp.com/alias-metadata-KEY-NAME`
  keys of the request JWT service account will be added to the alias metadata. For example, if an annotation
  "vault.hashicorp.com/alias-metadata-test-key" is configured, "test-key" with its value will be added.
