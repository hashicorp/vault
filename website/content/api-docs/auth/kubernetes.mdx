...
- `kubernetes_ca_cert` `(string: "")` - Optional PEM encoded CA cert for use by the TLS client used to talk with the Kubernetes API.
  ...
  If `kubernetes_ca_cert` is unset, the local CA cert will be used if running in a Kubernetes pod.
  If `kubernetes_ca_cert` is unset and `disable_local_ca_jwt` set to true, the system's trusted CA certificate pool will be used.
