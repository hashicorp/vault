...
- `secret_id_num_uses` `(integer: 0)` - Number of times any particular SecretID
  ...
  Can be overriden by `num_uses` request field when generating a SecretID, but is still the highest possible num_uses.
- `secret_id_ttl` `(string: "")` - Duration in either an integer number of
  ...
  expires.
  Can be overriden by `ttl` request field when generating a SecretID, but is still the shortest possible ttl.
...
- `ttl` `(string: "")` - Duration in seconds (`3600`) or an integer time unit (`60m`)
  ...
  May not be shorter than role's secret_id_ttl.
