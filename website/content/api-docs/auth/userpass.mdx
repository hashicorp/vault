...
- `password` `(string: <required if password_hash is not given>)` - The password for the user. Only required when creating the user.
- `password_hash` `(string: <required if password is not given>)` - Pre-hashed version of this user's password in bcrypt format. Mutually exclusive with `password`.
