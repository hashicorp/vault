...
request is determined to be relative to the `X-Vault-Namespace` header. For instance, 
if a request is to the URI `secret/foo` with the `X-Vault-Namespace` header set 
to `ns1/ns2/`, then the resulting request path to Vault will be 
Note that it is semantically equivalent to using the fullly qualified path rather than specifying the

Note that it is semantically equivalent to using the full path rather than the 
`X-Vault-Namespace` header, Vault will match the corresponding namespace 
based on correlating user input. Similar results of a path may be achieved if 
`X-Vault-Namespace` is set to `ns1/` with the request path of `ns2/secret/foo` 
as well, or otherwise if `X-Vault-Namespace` omitted entirely and instead a 
...
All request data, body and response data to and from Vault is JSON except in a 
few exceptional cases that are documented. Vault sets the `Content-Type` header 
appropriately with its response and does not require it from the clients 
request.

Authentication and Secret engines, that are also referred to as backends or 
mounts, differ in their implementation of APIs according to functionality and 
how they're authored.

The demonstration below are using the `KVv1` secrets engine, which is a 
simple Key/Value store. Please read the API documentation of secret engine or 
authentication engine for detailed information on their use. 

There are documented difference in the path schemes as well as other 
functionality which may be applicable as for example with [`KVv2` compared to `KVv1`].
...
parameter `list=true`, or you use the `LIST` HTTP verb. For the `kv` secrets 
...
To retrieve the help for any API within Vault, including mounted engine, auth
methods, etc. then append `?help=1` to any URL. If you have valid permission to
access the path, then the help text will be return a markdown-formatted block 
...
[`KVv2` compared to `KVv1`]: /api-docs/secret/kv
