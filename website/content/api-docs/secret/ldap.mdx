...
unbound. **You cannot set a rotation window when using `rotation_period`**.
...
[rotation_schedule](/vault/api-docs/secret/ldap#rotation_schedule) field.
For example:
...
[rotation_window](/vault/api-docs/secret/ldap#rotation_window) in which
