...
- `rotation_period` `(string/int)` – <EnterpriseAlert product="vault" inline /> Specifies the amount of time Vault should
wait before rotating the root credential. The minimum is 5 seconds.  Uses [duration
format strings](/vault/docs/concepts/duration-format). Mutually exclusive
with `rotation_schedule`.
- `rotation_schedule` `(string)` – <EnterpriseAlert product="vault" inline /> A cron-style string that will define the
schedule on which root rotations should occur. This should be a "standard"
...
- `rotation_window` `(string/int)` – <EnterpriseAlert product="vault" inline /> Specifies the amount of time in which the
root rotation is allowed to occur starting from a given `rotation_schedule`. If
the root credential is not rotated during this window, due to a failure or
...
is set.
- `disable_automated_rotation` `(bool: false)` - <EnterpriseAlert product="vault" inline /> Cancels all upcoming rotations
of the root credential until unset.
