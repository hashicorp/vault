...
- `roles` `(list: <role or roles>)` - The list of roles to assign to the generated token. This
  is only available in Consul 1.5 and greater.

- `service_identities` `(list: <service identities or service identities>)` - The list of
  service identities to assign to the generated token. This is only available in Consul 1.5
  and greater. Semicolon-separated list if multiple service identities are supplied.

- `node_identities` `(list: <node identity or node identities>)` - The list of node identities
  to assign to the generated token. This is only available in Consul 1.8.1 and greater.
