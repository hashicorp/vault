...
To import key material into an existing key, see the FPE `import_version/` endpoint.
...
## Import Key Version for FPE Transformation

This endpoint imports new key material into an existing FPE transformation with an imported key.

| Method | Path                                                  |
| :----- | :---------------------------------------------------- |
| `POST` | `/transform/transformations/fpe/:name/import_version` |
...
$ curl \
    ...
    --request POST \
    --data @payload.json \
    https://127.0.0.1:8200/v1/transform/transformations/fpe/example-transformation/import_version
```
