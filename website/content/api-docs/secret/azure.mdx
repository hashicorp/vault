...
- `max_ttl` (`string: "0"`) – Specifies the maximum TTL for service principals generated using this role. Accepts time
 suffixed strings ("1h") or an integer number of seconds. Default value of zero defers to to the system/engine max TTL time.
- `explicit_max_ttl` (`string: "0"`) – Limits the lifetime of the service principal secret in Azure and sets an upper bound on the corresponding lease's TTL. This lifetime cannot be changed once a lease is created, and the lease cannot be renewed beyond the expiration of this lifetime. With a default value of zero, the service principal secret will be created with a default duration of 10 years and deleted once the corresponding lease in Vault expires.
