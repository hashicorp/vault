...
This endpoint configures shared information for the secrets engine. Credentials
can be configured either with Application Credentials for a privileged service account,
or using Plugin Workload Identity Federation (WIF).

### IAM
There are multiple ways to pass root credentials to the Vault server, specified
below with the highest precedence first. If credentials already exist, this will
overwrite them.

The official Google Cloud SDK is used for sourcing credentials from env vars and shared
files.
...
in the environment **on the server**
