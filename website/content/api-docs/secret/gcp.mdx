...
- `credentials` (`string: ""`) - Google Cloud service account key JSON
  credentials. See docs for [alternative ways](/docs/secrets/gcp#setup) to pass
  in to this parameter, as well as the
  [required permissions](/docs/secrets/gcp#required-permissions).

- `ttl` (`int: 0 || string: "0s"`) – **Deprecated** - use `vault secrets tune`
  instead. Specifies default config TTL for long-lived credentials (i.e. service
  account keys). Accepts integer number of seconds or Go duration format string.

- `max_ttl` (`int: 0 || string: "0s"`) – **Deprecated** - use `vault secrets
  tune` instead. Specifies the maximum config TTL for long-lived credentials
  (i.e. service account keys). Accepts integer number of seconds or Go duration
  format string.
