---
layout: api
page_title: HCP Terraform Secrets Engine - HTTP API
description: This is the API documentation for the HCP Terraform (previously known as Terraform Cloud) secrets engine.
---

# HCP Terraform secrets engine HTTP API

This is the API documentation for the Vault's HCP Terraform (previously known as
Terraform Cloud) secrets engine. For general information about the usage and
operation of the HCP Terraform secrets engine, please refer to the [HCP
Terraform secrets engine documentation](/vault/docs/secrets/terraform).

This documentation assumes the HCP Terraform secrets engine is mounted at the `/terraform` path
in Vault. Since it is possible to mount secret backends at any location, please
update your API calls accordingly.

## Configure access

This endpoint configures the access information for HCP Terraform. This access
information is used so that Vault can communicate with HCP Terraform and generate
HCP Terraform tokens.
...
- `address` `(string: "")` – Specifies the address of the HCP Terraform
  server, if using Terraform Enterprise, provided as `"protocol://host:port"`.
  The default is `https://app.terraform.io` for HCP Terraform.

- `token` `(string: "")` – Specifies the HCP Terraform authentication token to
...
This endpoint queries for information about the HCP Terraform connection.
...
This endpoint creates or updates the HCP Terraform role definition in Vault.
If the role does not exist, it will be created. If the role already exists, it
will receive updated attributes.

HCP Terraform offers three distinct types of API tokens with varying level of
...
To create a Vault role to manage an HCP Terraform User tokens
...
This endpoint queries for information about an HCP Terraform role with the given name.
...
This endpoint deletes an HCP Terraform role with the given name. Even if the role does
...
This endpoint rotates the credentials for an HCP Terraform role that manages an
...
This endpoint returns an HCP Terraform token based on the given role
...
- `name` `(string: <required>)` – Specifies the name of an existing role against
  which to create this HCP Terraform token. This is part of the request URL.
