...
- `username_format` `(string)` - A format string for mapping Identity names to MFA method names. Values to substitute should be placed in `{{}}`. For example, `"{{alias.name}}@example.com"`. If blank, the Alias's Name field is used as-is. Currently-supported mappings:
...
This endpoint deletes a PingID MFA method. Note that MFA methods can only be deleted if they're not currently in use
