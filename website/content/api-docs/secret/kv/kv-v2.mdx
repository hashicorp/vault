...
To list secrets for KV v2, a user must have a policy granting them the list capability on this `/metadata/` path - even all the rest of their interactions with the KV v2 are via the `/data/` APIs, access to at least list the `/metadata/` path should typically also be granted.
