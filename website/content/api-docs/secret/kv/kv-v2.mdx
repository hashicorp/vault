...
This endpoint retrieves the secret at the specified location. Metadata returned
here is version specific metadata. Not to be confused with the Metadata set on the
metadata endpoint.
...
- `cas` `(int: <optional>)` - A flag required if Check-And-Set is set to true on
  either the secret or the engine's config. A write will only then be allowed if 
  key's current version matches the specified in the cas flag.
