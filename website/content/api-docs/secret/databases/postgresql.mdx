...
- `password_encryption` `(string: "none")` - When set to "scram-sha-256", passwords will be encrypted by Vault and stored as-is by PostgreSQL.
  Using "scram-sha-256" requires a minimum version of PostgreSQL 10. Available options are "scram-sha-256" and "none". The default is "none".
  When set to "none", passwords will be sent to PostgresSQL in plaintext format and may appear in PostgreSQL logs as-is.
