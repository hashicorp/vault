...
- `creation_statements` `(list: <required>)` – Specifies the database
  ...
  array. The `{{name}}`, `{{password}}`, `{{hashed_password}}` and `{{expiration}}`
  values will be substituted. The generated password will be a random alphanumeric
  20 character string. Substitute `{{password}}` with `{{hashed_password}}` to
  avoid sending a plaintext password to the Postgres server. This will instead send
  a scram-sha-256 hash of the generated password. See
  [docs](https://www.postgresql.org/docs/current/auth-password.html) for more information.
