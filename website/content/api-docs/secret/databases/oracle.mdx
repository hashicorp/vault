...
- `split_statements` `(boolean: true)` - Indicates the statements specified should be split on semi-colons.
  When true, this allows using a single string to specify multiple SQL statements. If set to false this allows for more 
  complex queries such as stored or inline procedures.

- `disconnect_sessions` `(boolean: true)` -  Indicates the plugin should disconnect any open sessions prior to running the 
  revocation statements. If set to false, the revocation statements should include session disconnect logic.
