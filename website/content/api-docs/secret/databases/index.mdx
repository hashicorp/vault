...
- `self_managed_password` `(string)` – <EnterpriseAlert product="vault" inline />
  The password corresponding to this username in the database. Required when using
  the Rootless Password Rotation workflow for static roles. Only enabled for select
  DB engines. See the
  [database secrets engine capabilities](/vault/docs/secrets/databases#database-capabilities)
  for details.
