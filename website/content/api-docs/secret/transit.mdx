...
- `type` `(string: "aes256-gcm96")` – Specifies the type of key to create. The
  ...
  ~> **Note**: When key type is `ml-dsa` or `slh-dsa`, `parameter_set` must be specified.
...
- `parameter_set` `(string:"")` - The parameter set to use for ML-DSA and SLH-DSA. Required for
ML-DSA and SLH-DSA keys. Valid values for ML-DSA are `44`, `65`, and `87`. Valid values for SLH-DSA
are `slh-dsa-sha2-128s`, `slh-dsa-shake-128s`, `slh-dsa-sha2-128f`, `slh-dsa-shake-128f`, `slh-dsa-sha2-192s`,
