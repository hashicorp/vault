...
- `algorithm` `(string: "oaep")` – When using a RSA key, specifies the RSA
  decryption algorithm to use for decryption. Supported signature types are:
...
| Method | Path                     |
| :----- | :----------------------- |
| `POST` | `/transit/decrypt/:name(/:algorithm)` |
