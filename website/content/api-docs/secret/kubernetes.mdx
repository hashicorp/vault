...
- `kubernetes_role_ref_type` `(string: "")` - Specifies whether the Kubernetes role used as reference in the final RoleBinding
  is a Role or ClusterRole. (if not set, it will use the value from the `kubernetes_role_type`)
