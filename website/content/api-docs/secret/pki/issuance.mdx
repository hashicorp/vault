...
These are the SCEP protocol API paths currently supported from Vault's authentication
point of view:
...
This endpoint fetches the current SCEP configuration.
...
This endpoint will update SCEP related configuration, returning the
updated values as a response along with an updated `last_updated` field.
For a comprehensive overview of SCEP and how to configure it,
refer to the [SCEP documentation](/vault/docs/secrets/pki/scep).
...
- `default_path_policy` `(string: "")` - Specifies the behavior for non-role-qualified SCEP requests; can be
 `sign-verbatim` or a role given by `role:<role_name>`.

- `authenticators` `(map[string]map[string]string: "")` - Specifies the mount accessors SCEP should delegate authentication
 requests. Map keys can be either `scep` or `cert`, with associated maps containing the key `accessor` with a value
 containing the auth mount's accessor. For the `scep` type, an optional key `scep_role` parameter is supported which
 will be passed as the [name](/vault/api-docs/auth/scep#name) parameter during SCEP authentication attempts.
 For the `cert` type, an optional key `cert_role` parameter is supported which will be passed as the
 [name](/vault/api-docs/auth/cert#name-6) parameter during certificate authentication attempts.

- `allowed_encryption_algorithms` `([]string)` - the list of allowed encryption algorithms for SCEP requests, allowed
 values are: `des-cbc`, `3des-cbc`, `aes128-cbc`, `aes256-cbc`, `aes128-gcm`, `aes256-gcm`

- `allowed_digest_algorithms` `([]string)` - the list of allowed digest algorithms for SCEP requests.  Allowed values
  are: `sha-1`, `sha-256`, `sha-384`, `sha-512`

- `return_full_ca_chain` `(bool: true)` - whether to return the full CA chain within GetCACert responses, this
  should be set to `false` for compatibility with Microsoft Intune.

- `external_validation` `(map[string]map[string]string: "")` - a map that specifies 3rd party validation of SCEP
 requests. Only a single external validation can be configured, currently the supported type is `intune`.

  - For the `intune` external validation the following parameters can be configured. Note that all parameters can use
