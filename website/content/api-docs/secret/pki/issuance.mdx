...
Enabling EAB is strongly recommended for public-facing Vault
...
This endpoint returns a list of ACME account key identifiers.
...
This endpoint returns the account information for a given ACME account key along with
the associated orders and certificate details issued within the account.
...
- `key_id` `(string: <required>)` - The id of the ACME account to read.
...
This endpoint allows updating an ACME account's status to revoke or re-enable it by
a Vault operator. The returned response contains the ACME account information without,
any order or certificate details.
...
- `status` `(string: <required>)` - The status to update the account to, allowed
  values are `revoked` and `valid`.

<Note title="ACME account revocation does not revoke certificates">
Setting the status to 'revoked' will forbid any further operations
on the account, but existing certificates will not be revoked,
they will need to be manually revoked if required.
</Note>
