...
- `ext_key_usage` `([]string: OCSPSigning)` - This is a comma-separated list of
  extended key usages that will be encoded on the generated certificate.  This is
  a list of the names of each key usage, valid values can be found at
  https://golang.org/pkg/crypto/x509/#ExtKeyUsage just drop the `ExtKeyUsage` part
  of the names. Values are not case-sensitive. To specify no extended key usages,
  set this to an empty list.  If no extended key usage is set by this field nor by
  the ext_key_usage_oids field, then the extended key usage extension will be
  omitted entirely from the CSR.  This is overwritten by use_csr_values if an
  extended key usage extension exists within the csr.

- `ext_key_usage_oids` `([]string: [])` - This is a comma-separated string or list
  of extended key usage oids to be encoded on the generated certificate. Useful
  for adding extended key usages not supported by the Go standard library.  If no
  extended key usage is set by this field nor by the ext_key_usage field, then the
  extended key usage extension will be omitted entirely.  This is overwritten by
  use_csr_values if an extended key usage extension exists with in the CSR.
...
- `key_usage` `([]string: CRL,CertSign)` - This list of key usages will be added
  to the existing set of key usages, CRL,CertSign, on the generated certificate.
  Values can be found at https://golang.org/pkg/crypto/x509/#KeyUsage , just drop
  the "KeyUsage" part of the name.

- `ext_key_usage` `([]string: OCSPSigning)` - This is a comma-separated list of
  extended key usages that will be encoded on the generated certificate.  Values
  can be found at https://golang.org/pkg/crypto/x509/#ExtKeyUsage , just drop the
  `ExtKeyUsage` part of the value. Values are not case-sensitive. To specify no
  extended key usages, set this to an empty list.  If no extended key usage is set
  by this field nor by the ext_key_usage_oids field, then the extended key usage
  extension will be omitted entirely from the certificate.

- `ext_key_usage_oids` `([]string: [])` - This is a comma-separated string or list
   of extended key usage oids to be encoded on the generated certificate. Useful
   for adding extended key usages not supported by the Go standard library.
...
- `key_usage` `([]string: )` - Specifies key_usage to encode in the generated
  certificate signing request (CSR).  This is a list of the names of each key
  usage, valid values can be found at https://golang.org/pkg/crypto/x509/#KeyUsage
  just drop the `KeyUsage` part of the name.  If not set, key usage will not
  appear on the CSR.

- `ext_key_usage` `([]string: [])` - This is a comma-separated list of
  extended key usages that will be encoded on the generated certificate signing
  request (CSR).  This is a list of the names of each key usage, valid values can
  be found at https://golang.org/pkg/crypto/x509/#ExtKeyUsage just drop the
  `ExtKeyUsage` part of the names. Values are not case-sensitive. If no extended
  key usage is set by this field nor by the ext_key_usage_oids field, then the
  extended key usage extension will be omitted entirely from the CSR.
