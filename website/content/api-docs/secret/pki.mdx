...
This enterprise only endpoint fetches the current EST configuration.
...
This enterprise only endpoint will update EST related configuration, returning the
...
- `default_path_policy` `(string: "sign-verbatim")` - If `default_mount` is enabled, specifies the
...
- `authenticators` `(map[string]map[string]string: "")` - Specify the mount accessors EST should delegate authentication
 requests too. Map keys can be either `cert` or `userpass`, with associated maps containing the key `accessor` with a
 string value that is a mount accessor. For the `cert` type, an optional key `cert_role` parameter is supported which
 will be passed as the [name](/vault/api-docs/auth/cert#name-6) parameter during certificate auth attempts.
