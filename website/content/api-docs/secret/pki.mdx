...
- [Tidy](#tidy)
- [Managed Key](#managed-key) (Enterprise Only)
...
Note that this api supports [Managed Keys](#managed-key), for additional details
on using it are available below.
...
- `type` `(string: <required>)` – Specifies the type of the intermediate to
  ...
  later_. `kms` is also supported [see below](#managed-key). This is part of the request URL.
...
### Managed Key Parameters
See [Managed Key](#managed-key) for additional details on this feature, if
...
- `type` `(string: <required>)` – Specifies the type of the root to
  create. If `exported`, the private key will be returned in the response; if
  `internal` the private key will not be returned and _cannot be retrieved
  later_. `kms` is also supported [see below](#managed-key). This is part of the
  request URL.
...
~> Note: Managed key is an Enterprise only feature.

The [Generate Root](#generate-root) and [Generate Intermediate](#generate-intermediate)
api calls can leverage the managed key feature, delegating operations that
require private key material to an external system.

To leverage a managed key assuming it has already been configured, the type
parameter to either [Generate Root](#generate-root) or
[Generate Intermediate](#generate-intermediate) apis must be set to `kms`
and one of either `managed_key_name` or `managed_key_id` parameters set to identify
which key to use. As the `internal` type for those apis, if `kms` is leveraged
there is no way to read/fetch the private key.

The api call will fail if the specified managed key is not properly setup or
arguments detailing private key attributes are specified such as
`key_type` or `key_bits`.

Once this has been configured, all other PKI operations are the same requiring
no other special configuration or parameters.
