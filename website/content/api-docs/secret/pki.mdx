...
~> Note: Revocations performed using Proof of Possession (POP) or with Bring
   Your Own Certificate (BYOC) are not replicated cross-cluster. To enable
   cross-cluster BYOC based revocation, turn on `unified_crl`.
