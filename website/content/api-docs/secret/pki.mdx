...
- [Notes on Personas](#notes-on-personas)
...
However, two endpoints (for [listing all certificates](list-certificates)
issued by this authority and [listing all issuers](#list-issuers) in this
mount) are authenticated, though not generally considered privileged
information.
...
This endpoint retrieves the specified issuer's certificate. When the format
of the endpoint is notated below as JSON, this is a standard Vault data
structure that can be read by the Vault CLI. Otherwise, the DER and PEM
formats cannot be read via the `vault read` CLI; another client must be
used.

Note that the response differs between the legacy `/pki/cert/ca`
...
the default issuer. When the format of the endpoint is notated below as JSON,
this is a standard Vault data structure that can be read by the Vault CLI.
Otherwise, the PEM format cannot be read via the `vault read` CLI; another
client must be used.
...
This endpoint retrieves the specified issuer's CRL. When the format is
notated below as JSON, the response is a standard Vault data structure that
can be read by the Vault CLI. Otherwise, the DER and PEM formats cannot be
read via the `vault read` CLI; another client must be used.

Note that the response differs between the legacy `/pki/cert/crl` path and
...
| `GET`  | `/pki/crl`                        | `default` | DER    |
| `GET`  | `/pki/crl/pem`                    | `default` | PEM    |
| `GET`  | `/pki/issuer/:issuer_ref/crl`     | Selected  | JSON   |
| `GET`  | `/pki/issuer/:issuer_ref/crl/der` | Selected  | DER    |
| `GET`  | `/pki/issuer/:issuer_ref/crl/pem` | Selected  | PEM    |
...
including issued certificates. When the format is notated below as JSON,
the response is a standard Vault data structure that can be read by the
Vault CLI. Otherwise, the DER and PEM formats cannot be read via the
`vault read` CLI; another client must be used.
...
| `GET`  | `/pki/cert/:serial/raw`     | DER    |
| `GET`  | `/pki/cert/:serial/raw/pem` | PEM    |
...
| Method | Path                                       | Issuer    |
| :----- | :----------------------------------------- | :-------- |
| `POST` | `/pki/root/sign-self-issued`               | `default` |
| `POST` | `/pki/issuer/:issuer_ref/sign-self-issued` | Selected  |
...
~> **Note**: This operation is highly privileged as it allows revocation of
   ...
   validate that the requesting user issued the certificate or has possession
   of the private key.

~> **Note**: It is not possible to revoke issuers using this path.
...
on the final signed certificate are set at signing time and may or may not honor
...
In order to complete the intermediate generation, the CSR must be signed and
the resulting certificate imported.
...
- `leaf_not_after_behavior` `(string: "err")` - Behavior of a leaf's
  ...
  - `truncate` to silently truncate the requested `NotAfter` value to that
    of this issuer; or
  - `permit` to allow this issuance to suceed with a `NotAfter` value
...
- `key_ref` `(string: <required>)` - Reference to an existing key,
  ...
  to an key. This parameter is part of the request URL.
...
paramter that is configurable is the key's name.
...
Refer to the [earlier section](#read-roles) for more information about
reading roles.
...
The following is a condensed reference of various useful personas one might
enforce using [Vault Policies](/docs/concepts/policies). These are just a
suggestion; other personas and policy approaches may also be valid.

 - Operator; a privileged user who manages the health of the PKI
   subsystem; manages issuers and key material.
 - Agent; a semi-privileged that manages roles and handles
   revocation on behalf of an operator; may also handle delegated
   issuance.
 - Advanced; potentially a power-user that has access to additional
   issuance APIs.
 - Requester; a low-level user that simply requests certificates.
...
| `/crl` | Read | Yes | Yes | Yes | Yes | Yes |
| `/cert/:serial` | Read | Yes | Yes | Yes | Yes | Yes |
| `/cert/:serial/pem` | Read | Yes | Yes | Yes | Yes | Yes |
