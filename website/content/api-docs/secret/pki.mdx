...
storage, and so are forwarded to the cluster's primary for execution.  This table
outlines which operations can be executed on secondaries and thus scale 
horizontally.
...
\* Only if the corresponding role has no_store set to true.
