...
- `config` `(map<string|string>: nil)` – Specifies configuration options for
  ...
  - `listing_visibility` `(string: "")` - Specifies the information to show about
   this mount in the unauthenticated UI-specific listing endpoint. Valid values are:
      - `"hidden"`: hides mount from mount list, equivalent to the default `""` 
      - `"unauth"`: returns mount path, description and method type
      - `"dropdown"`: lists method in a customized dropdown in the login form
...
- `listing_visibility` `(string: "")` - Specifies whether to show this mount
  in the UI-specific listing endpoint. Valid values are `"unauth"`, `"dropdown"` or `"hidden"`,
