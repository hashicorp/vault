...
- `local` `(bool: false)` – Specifies if the auth method is local only. Local
  ...
  mount will be local to the cluster. If the goal of the marking an auth method 
  as `local` was to comply to GDPR guidelines, then care must be taken in not 
  setting data pertaining to local auth mount or local auth mount aliases in the 
