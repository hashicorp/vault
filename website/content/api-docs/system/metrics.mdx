...
```
The active node in a Vault cluster will respond with metrics, but the follower nodes will respond with no metrics (i.e. the curl command will exit 0 and there will be no output).
