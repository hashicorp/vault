...
- `queue_capacity` `(int: 1,000,000)` - Maximum number of pending sync operations allowed on the internal queue. When the
queue is at capacity, new operations are dropped to avoid starving Vault's memory. Dropped operations are retried automatically
on the next reconciliation loop tick.
