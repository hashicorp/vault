...
- `organization_name` `(string: <optional>)` - **(BETA)** Name of the GitHub organization. For example, if your organization is located at https://github.com/hashicorp the
name is hashicorp.  Requires `secrets_location` to be `organization`.

  - `organization_visibility` `(string: <optional>)` - **(BETA)** Controls which repositories within the organization will be able to see the synced secrets.
  Must be one of `all`, `private`, or `selected`. When set to `all`, all repositories have access to the secret. When set to `private`, only private and
  internal repositories have access. When set to `selected`, only the repositories listed in the `selected_repository_names` field have access. Requires
  `secrets_location` to be `organization`.

  - `selected_repository_names` `(list: <optional>)` - **(BETA)** The list of repository names within the organization that will be able to see the synced secrets.
