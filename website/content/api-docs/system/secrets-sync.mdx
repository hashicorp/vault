...
This endpoint associates a secret referenced via its mount and name to an existing destination. Setting an association
triggers a sync operation. If the secret is already associated with that destination, a duplicated association is not
created but a refresh operation will be executed. This is useful to refresh the secret value on the external system
or retry a failed sync operation if the underlying problem that caused the failure is now resolved.
...
This endpoint dissociates a secret referenced via its mount and name from an existing destination. Dissociating a secret
triggers an unsync operation and the secret will be deleted in the external system. If no corresponding association exists,
or if the secret no longer exists on the external system, the remove operation still succeeds.
