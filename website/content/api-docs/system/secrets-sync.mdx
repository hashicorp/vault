...
- `replication_locations` `(list: <optional>)` - The list of GCP location names for which the destination may replicate secret
  data into. Regardless of the selected regions for storing the secrets, they will remain globally readable when configured as such.
...
- `repository_owner` `(string: <optional>)` - GitHub username or organization name that owns the single repository. For example, if a repository is located at
https://github.com/hashicorp/vault.git the owner is hashicorp. Requires `secrets_location` to be `repository`.

  - `repository_name` `(string: <optional>)` - Name of the repository. For example, if a repository is located at https://github.com/hashicorp/vault.git
  the name is vault. Requires `secrets_location` to be `repository`.

  - `environment_name` `(string: "")` - Name of a GitHub environment within the repo specified by `repository_name`.
  By default, secrets are global to the targeted repository. Requires `secrets_location` to be `repository`.

- **(BETA)** `organization_name` `(string: <optional>)` - Name of the GitHub organization. For example, if your organization is located at https://github.com/hashicorp the
name is hashicorp.  Requires `secrets_location` to be `organization`.

  - `organization_visibility` `(string: <optional>)` - Controls which repositories within the organization will be able to see the synced secrets.
  Must be one of `all`, `private`, or `selected`. When set to all, all repositories within the organization have access to the secret. When set to private,
  only private and internal repositories have access to the secret. When set to selected, only the repositories named in the `selected_repository_names`
  fields will have access to the secret. Requires `secrets_location` to be `organization`.

  - `selected_repository_names` `(list: <optional>)` - The list of repository names within the organization that will be able to see the synced secrets.
  Requires `secrets_location` to be `organization`.
...
### Sample repository environment payload
```json
{
