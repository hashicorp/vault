...
- `replication_locations` `(list: <optional>)` - A list of GCP location names
  the destination can use to store replicated secrets. Setting a replication
  location as global overrides region-specific readability settings for the
  location.
...
- `repository_owner` `(string: <optional>)` - GitHub owner of the secrets sync
  ...
  - `repository_name` `(string: <optional>)` - GitHub repository name of the
...
- `organization_name` `(string: <optional>)` - **(BETA)** GitHub organization
  ...
  - `organization_visibility` `(string: <optional>)` - **(BETA)** Controls which
  ...
  - `selected_repository_names` `(list: <unset>)` - **(BETA)** Explicit list of
