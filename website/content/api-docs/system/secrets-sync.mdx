...
- **(BETA)** `organization_name` `(string: <optional>)` - Name of the GitHub organization. For example, if an org is located at https://github.com/hashicorp the
name is hashicorp.  Requires `secrets_location` to be `organization`.
  ...
  Must be one of `all`, `private`, or `selected`. When set to all, all repositories within the org have access to the secret. When set to private,
