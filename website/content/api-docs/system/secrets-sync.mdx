...
- `access_token` `(string: <optional>)` - Fine-grained or personal access token. This is an alternative to using a
  GitHub app for authentication.

- `app_name` `(string: <optional>)` - The name of the GitHub App to use for authentication. This is an alternative to using
  an `access_token` for authentication.

- `installation_id` `(string: <optional>)` - The installation ID of the GitHub App to use for authentication. This is required
  when using a GitHub App for authentication.
...
This endpoint configures a custom GitHub application to be used for syncing secrets with GitHub repositories, as an alternative to
...
- `name` `(string: <required>)` - Specifies a custom name for the GitHub application. This is the name used when configuring
the destination.

- `app_id` `(string: <required>)` - Specifies the GitHub application ID, provided by GitHub.

- `private_key` `(string: <required>)` - Specifies the private key (either file contents or '@path/to/file')
  used to authenticate with the GitHub application.
...
This endpoint returns the configured GitHub application.
...
This endpoint deletes the configured GitHub application. Deletes are only permitted when there are no destinations referencing the GitHub application.
This does not delete the GitHub application from GitHub.
