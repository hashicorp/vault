...
`retention_months` parameter cannot be configured below the default setting ([minimum_retention_months](#read-the-client-count-configuration)).
...
**Vault 1.14.0 or later:** The automated reporting feature exposes two
...
The `reporting_enabled` and `billing_start_timestamp` parameters are read-only
