...
<Note>
These endpoints are only available in Vault version 1.3+. Backwards compatibility is not guaranteed. These endpoints are subject to change or may disappear without notice.
</Note>
...
When [automated utilization
reporting](/vault/docs/enterprise/license/utilization-reporting) is enabled, the
`retention_months` parameter cannot be configured below the default setting ([minimum_retention_months](#minimum_retention_months))
and the `enabled` parameter cannot be set to `disable`.
...
Reading the configuration shows the current settings, as well as a flag as to
whether any data can be queried.

- `enabled` `(string)` - returns `default-enabled` or `default-disabled` if the
  configuration is `default`.
- `queries_available` `(bool)` - indicates whether any usage report is
  available. This will initially be false until the end of the first calendar
  month after the feature is enabled.

**Vault 1.14.0 or later:** The automated reporting feature exposes three
additional read-only system settings:

- `reporting_enabled` `(bool)` - indicates whether automated reporting is
  enabled.
- `billing_start_timestamp` `(time)` - returns the configured billing start
  timestamp to be used for automated reporting.
- `minimum_retention_months` `(int)` - returns the configured minimum value for `retention_months`.

<Note>

The `reporting_enabled`,  `billing_start_timestamp`, and `minimum_retention_months` parameters are read-only
fields and will have no effect if specified in the request payload. All other
fields will still be honored.
