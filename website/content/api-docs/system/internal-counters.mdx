...
- As of 1.12, the `end_time` can be the current month, so long as the `start_time`
is not also the current month. To get client count information solely for a billing 
period consisting of the current month, please use the 
`/sys/internal/counters/activity/monthly` endpoint. 
...
- Note that if the `end_date` specified is the current month (available as of 1.12), 
the last element in the `months` response stanza, signifying the current month, will
not have namespace attribution for both the `new_clients`
stanza and the top level `by_namespace` stanza. Furthermore, the `new_clients` counts returned
for the current month will be an approximation. That is to say, the response will appear as follows.

```json
{
   "months":[
      // accurate months for the non-current month,
      {
         "timestamp":"current_month_timestamp",
         "counts":{ // the following block contains accurate counts
         ...
         "namespaces": [], // this will be empty
         "new_clients":{
            "counts":{ // this block will contain estimated counts, not exact counts
               "distinct_entities":5,
               "entity_clients":5,
               "non_entity_tokens":5,
