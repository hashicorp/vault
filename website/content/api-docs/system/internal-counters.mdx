...
This endpoint returns the number of clients per namespace, as the sum of active entities and non-entity clients.
An "active entity" is a distinct entity that has created one or more tokens in the given time period.
A "non-entity client" is one that authenticates to vault using a "non-entity token", i.e. a token with
no attached entity ID. For more information on how non-entity clients are counted, please visit the 
