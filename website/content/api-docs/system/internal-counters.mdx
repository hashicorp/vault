...
<Note title="Behavior Differences between Vault Enterprise and Community Edition">

The behavior of the `start_time` and `end_time` parameters in the Counters API differs based on the Vault edition.
In **Vault Enterprise (ENT)**, these parameters are restricted to aligning with full billing periods only. In **Vault Community
Edition (CE)**, the API supports flexible historical time ranges, but excludes the current month and any future dates. 
...
In **Vault ENT**, `start_time` and `ent_time` are aligned to the start and end of a billing period, respectively:

- The Counters API only returns **data for an entire billing period**.
- By default, the API returns activity data for the **current billing period**.
- If `start_time` is provided, the query is aligned to the **start of the billing period** to which that timestamp belongs.
- If only `end_time` is provided, the query is aligned to the **start of the billing period** to which the `end_time` belongs.
- If `start_time` and `end_time` do **not** correspond to the start and end of the **same billing period**, Vault Enterprise:
   - **Aligns** the `start_time` to the start of the billing period (using either `start_time` or `end_time` as reference).
   - **Sets** the `end_time` to the **end of that billing period**.
   - **Returns a warning**:
...
### Sample request
Assume the start of the billing period is January 1, 2023 and current date is December 31, 2025.
...
If the start_time or end_time is within the billing period or has overlapping billing periods, the response looks like this:
...
```json
{
   ...
   "data":{
      :
      // data returned for that billing period
      :
...
In **Vault CE**, `end_time` is adjusted to the last month if the current month or a future date is provided:

- The Counters API supports **arbitrary historical time ranges**, **except** for the **current month or any future dates**.
- By default, the API returns results **until the end of the last complete month**.
- If `end_time` falls in the **current month or a future date**, Vault CE:
   - **Ignores** the unsupported value.
   - Returns data **only up to the end of the last month**.
   - **Returns a warning**:

      ```json
      {
         "warning": "end_time parameter can only be used to specify until the end of last month. The value provided for this parameter is not supported and is ignored. Showing counts until the end of last month."
      }
      ```

### Sample request
Assume the current date is September 25, 2025.
   ...
   ```json
{
   ...
   "wrap_info":null,
   "warnings":"end_time parameter can only be used to specify until the end of last month. The value provided for this parameter is not supported and is ignored. Showing counts until the end of last month.",
   "auth":null
}
```
...
- `end_time` `(string, optional)` - An RFC3339 timestamp or Unix epoch time. Specifies the end of the period
  for which client counts will be reported. If no end time is specified, the end of the last month will be used.
