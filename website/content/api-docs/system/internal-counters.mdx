...
- If  the provided`start_time` is outside of the current billing period, the query is aligned to entire default billing period to which `start_time` belongs.
- If only `end_time` is provided, the query is aligned to the entire default billing period to which the `end_time` belongs.
- If `start_time` and `end_time` do **not** correspond to the **same billing period**, Vault Enterprise:
   - Billing period will be aligned to the date specified in `start_time`
