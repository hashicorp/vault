...
- `503` if sealed
- `523` if an error occured during the post-unseal operations
...
- `failedtounsealedcode` `(int: 523)` – Specifies the status code that should be
  returned for a node that failed to perform the post-unseal operations. When this
  occurs Vault logs the error and will try to unseal itself automatically in case
  the error was transient but may not be able to.
