...
- `inheritable` `(bool: false)` - If set to `true` on a quota where `path` is set to a namespace,
  all child namespaces of the specified path namespace will inherit this quota. `inheritable` cannot be set to
  true if the `path` does not specify a namespace. All root namespace quotas are inheritable by default.
  All other namespace quotas are not inheritable by default.
