...
that can either be a namespace or mount, or a full path to restrict one specific endpoint.
...
- `path` `(string: "")` - Path of the mount or namespace to apply the quota.
  ...
  a specific secret on a K/V v2 mount in `namespace1`. Updating this field on an existing
