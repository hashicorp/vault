...
In the above example, please note the use of `passcode=` in the `mfa_payload`. Before Vault 1.13.0, that phrase should
have been used for the Duo MFA method only. However, from the new version going forward, it is optional to use that
phrase for all supported MFA methods.
