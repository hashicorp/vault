...
- `mfa_payload` `(map: <required>)` - A map of login MFA methodIDs to passcode credentials.
 In cases where an MFA method is configured not to use passcodes, the passcode remains an empty string.
...
If MFA validation succeeds, the response will be the same as a successful
