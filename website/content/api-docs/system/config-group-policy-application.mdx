...
<Warning>
  Starting with Vault versions 1.15.0, 1.14.4, and 1.13.8 the group policy application mode
  setting applies only to ACL policies. Vault Sentinel Role Governing Policies (RGPs) are not affected
  by group policy application mode.
</Warning>
