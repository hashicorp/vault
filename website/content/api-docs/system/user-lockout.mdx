---
layout: api
page_title: /sys/lockedusers - HTTP API
description: The `/sys/lockedusers` endpoint is used to manage locked users in Vault.
---

# `/sys/lockedusers`

The `/sys/lockedusers` endpoint is used to list and unlock locked users in Vault. 
...
$ curl \
    ...
    --request GET \
    http://127.0.0.1:8200/v1/sys/lockedusers
```
...
| Method | Path             |
| :----- | :--------------- |
| `POST` | `/sys/lockedusers/:mount_accessor/unlock/:alias-identifier` |
...
$ curl \
    --header "X-Vault-Token: ..." \
    --request POST \
    http://127.0.0.1:8200/v1/sys/lockedusers/auth_userpass_af8d1d32/unlock/bsmith
```