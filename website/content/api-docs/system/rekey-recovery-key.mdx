...
- `enable_unseal_recovery` `(bool: false)` - If true, the resulting new
  recovery keys can be used to unseal Vault using 
  [Unseal Recovery Mode](/vault/docs/concepts/seal#unseal-recovery-mode).  If so,
  submitting shares via update must also pass the `acknowledge_unseal_recovery`
  flag.  In other words, all parties must be aware and agree to enable this mode.
...
- `acknowledge_unseal_recovery` `(bool: false)` - Passed to agree to the enablement
  of unseal recovery mode at the start of rekeying.  If there is a disagreement
  rekeying is aborted.
