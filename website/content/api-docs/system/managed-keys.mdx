...
The `/sys/managed-keys` endpoint is used manage the managed keys in Vault. Managed keys are keys managed outside
of Vault's storage and can be leveraged to perform various operations within Vault.

## List Managed keys.

This endpoints lists all the managed keys within the namespace.
...
that the key is supposed to use such as pkcs11. The :name argument is unique name within all managed key types within
the namespace.
...
- `name` `(string: <required>)` - A unique lowercase name that serves as identifying the key. Must be
  unique throughout the namespace.
...
- `any_mount` `(string: "false")` - Allow usage from any mount point within the namespace if "true". If "false"
  specific mount points will need to be tuned setting `allowed_managed_keys` with the name value of the key
  granting usage.
...
- `slot` `(string: <slot or token label required>)`: The slot number to use,
  specified as a string (e.g. `"2305843009213693953"`).

- `token_label` `(string: <slot or token label required>)`: The slot token label to
  use.

- `curve` `(string: "")` - supplies the curve value when using the `CKM_ECDSA`
  mechanism. This key is required if  The supported values are
    - `P384`
    - `P521`
    - `P256`

- `key_bits` `(int: "")` - supplies the key bits when using `CKM_RSA_PKCS_PSS`,
  `CKM_RSA_PKCS_OAEP` or `CKM_RSA_PKCS` mechanisms, supported values are
    - 2048
    - 3072
    - 4096
...
## Read a Managed Key's properties

This endpoint returns the configuration of the managed key at the given path.
...
This endpoint deletes the managed key at the given auth path if it is not configured for usage within any mount point
