...
that the key is to use, such as `pkcs11`. The :name argument is unique name within all managed key types within
the namespace.
...
- `name` `(string: <required>)` - A unique lowercase name that serves as identifying the key. Must be
  unique throughout all types within the namespace.
...
This endpoint deletes the managed key at the given path if it is not configured for usage within any mount point.
