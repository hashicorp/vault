...
- `type` `(string: <required>)` – The backend type that will be leveraged for the managed key.
  Supported options are `pkcs11`, `awskms`, and 'gcpckms'.
...
- `region` `(string: <required>)`: The AWS region where the keys are stored (or will be stored). This
may also be provided in the `AWS_REGION` environment variable.
- `secret_key` `(string: <required>)`: The AWS secret key ID to use. This may also be provided in the
`AWS_SECRET_ACCESS_KEY` environment variable.
- `signature_algorithm` `(string: <required>)`: The algorithm to be used for signatures. The values
are defined by AWS KMS here: https://docs.aws.amazon.com/sdk-for-go/api/service/kms/#pkg-constants.
Supported values for RSA keys are:
    - `RSASSA_PSS_SHA_256`
    - `RSASSA_PSS_SHA_384`
    - `RSASSA_PSS_SHA_512`
...
Supported values for ECDSA keys are:
    - `ECDSA_SHA_256`
    - `ECDSA_SHA_384`
    - `ECDSA_SHA_512`
