...
`hashicorp/example-key-name`. If the value is `kms_key=alias/example-key-name`, then Vault will
 ...
 ~>**Note**: when creating a key through AWS KMS to be used with managed keys, note that the
 key usage must be `SIGN_VERIFY`. When using an elliptic curve key, the combination `key_type=ECDSA`
 and `curve=P256` corresponds to the AWS Key Spec `ECC_NIST_P256`.
