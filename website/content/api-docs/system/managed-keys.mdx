...
- `kms_key` `(string: "")`: An identifier for the key. If this parameter is not provided, then
  `allow_generate_key` must be set to `true`. The value can be either an AWS ARN, an AWS alias, or
  (if the key already exists) the key ID generated by AWS KMS. If the value is an alias, the `alias/`
  prefix is required. If Vault is generating a key, this parameter is used to create an alias for the
  generated key.
