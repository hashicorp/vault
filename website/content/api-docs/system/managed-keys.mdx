...
- `curve` `(string: "P256")`: The curve to use for an ECDSA key. This field is used when
  `key_type` is `ECDSA`. Supported options are `P256`, `P384`, or `P521`. If a value is not
  specified, Vault defaults to `P256`.
...
- `kms_key` `(string: "")`: An identifier for the key. If using an existing key, this value can
  be either an AWS alias for the key (including the `alias/` prefix) or the key ID generated by
  AWS. If this parameter is used when Vault is generating a key, the value will be used to create
  an alias for the generated key. To fully specify an alias for a generated key, provide the full
  alias, including the `alias/` prefix. If the provided value doesn't include this prefix, Vault
  will prepend it with `alias/hashicorp` when creating the alias.

  -> e.g., If the value provided is `kms_key=example-key-name`, then Vault will create
  the alias `hashicorp/example-key-name`. If the value is
  `kms_key=alias/example-key-name`, then Vault will use the alias `example-key-name`.
