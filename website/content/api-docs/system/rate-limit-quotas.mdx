...
all child namespaces of the specified path namespace will inherit this quota. The request
will fail if `inheritable` is set to true, while the `path` has a value that is not a namespace mount,
or if a value for `role` is specified. All global quotas are inheritable by default. All namespace quotas
are not inheritable by default.
