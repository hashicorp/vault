...
- `compressed` `(bool: false)` - Update using the compressed form of `value`. Defaults to false.

- `compression_type` `(string: "")` - Specifies the type of compression if `compressed` is true.
  Supported values are `gzip`, `lzw`, `lz4`, `snappy`, or `""`. If empty or omitted, uses the same compression type
  if the value already exists and is compressed.
