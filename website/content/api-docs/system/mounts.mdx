...
- `config` `(map<string|string>: nil)` – Specifies configuration options for
  ...
  - `identity_token_key` `(string: "")` - The key to use for signing identity tokens.
    If not provided, this will default to Vault's OIDC
    [default key](https://developer.hashicorp.com/vault/docs/concepts/oidc-provider#keys).
