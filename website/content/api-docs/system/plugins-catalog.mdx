...
- `download` `(bool: false)` - <EnterpriseAlert inline="true" /> **(BETA)** If set,
  indicates that Vault downloads the specified plugin from the
  [HashiCorp releases page](https://releases.hashicorp.com/).
  This is only supported for secret and auth plugin versions with artifacts
  containing `metadata.json` and `metadata.json.sig`.

~> **Note:** To use the `download` parameter, Vault must be configured to access
the HashiCorp releases page.
