...
- `command` `(string: "")` - Specifies the command to spawn the plugin. This defaults
  to the name of the plugin if both `oci_image` and `command` are unspecified.
...
### Sample payload not using OCI image
...
```json
{
  ...
  "oci_image": "mysql-database-plugin-oci-image",
