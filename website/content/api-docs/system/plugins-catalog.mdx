...
- `sha256` `(string: <required>)` – The SHA256 sum of a plugin binary or the OCI image. 
...
- `command` `(string: <required>)` - Specifies the command used to execute the
  ...
  This is required to register with a plugin binary but should not be specified when registering
  with a plugin artifact; if it is specified for an artifact, it will be ignored as the
  run command is known.
