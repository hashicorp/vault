...
- `sha256` `(string: "")` – The SHA256 of the plugin binary or the OCI image provided.
  This is required to register with the plugin binary but should not be specified when registering with an
  extracted artifact directory. When used, before a plugin is run, its SHA will be checked against this value.
  If the actual SHA of the plugin binary and the SHA provided in `sha256` do not match, Vault will not run the plugin.
