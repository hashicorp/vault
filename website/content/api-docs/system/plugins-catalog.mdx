...
- `name` `(string: <required>)` – Specifies the name for this plugin. The name
  ...
  URL. If the plugin is Enterprise, the value must match the plugin name on the
...
- `sha256` `(string: <required>)` – This is the SHA256 sum of the Community plugin's
  binary or the OCI image. Before a plugin is run, its SHA will be checked against this value.
  If they do not match the plugin can not be run. If the plugin is Enterprise, the value is not required.
