...
~> ** Note:** Vault does not allow the use of AWS IAM Roles for EKS Service Accounts 
   to authenticate to Amazon S3 buckets for the Automated Integrated Storage Snapshots. 
   The parameters: [`aws_access_key_id` & `aws_secret_access_key`](/vault/api-docs/system/storage/raftautosnapshots#aws_access_key_id) 
   must be set in the context of AWS EKS & S3 configuration.
