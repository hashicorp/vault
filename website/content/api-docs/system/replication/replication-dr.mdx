...
1. **Use primary cluster addresses with the `update_primary_addrs` parameter**.
    ...
    failover when updating a newly demoted DR secondary cluster. This was added
    in Vault 1.13.3.

The two update methods are mutually exclusive. You may use one or the
other, but not both. In most cases, update-primary is called on a DR
cluster after a failover, which will require the use of an activation
token. If you're confused as to which method to use, a good rule of
thumb is to use `token` on DR secondary clusters and `update_primary_addrs`
