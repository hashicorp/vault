...
but not both. We recommend updating primary cluster assignments with cluster
addresses whenever possible. Address updates are much faster than activation
token updates because Vault does less work behind the scenes, which means
potentially less disruption in replication.
