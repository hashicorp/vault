...
Plugin backends are the components in Vault that can be implemented separately
from Vault's builtin backends. These backends can be either authentication or
secrets engines.

The `api_addr` must be set in order for the plugin process to
...
[plugin catalog](/docs/internals/plugins/plugin-architecture#plugin-catalog).
