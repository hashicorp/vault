...
This integration leverages existing functionality in Vault and we have recently validated the integration between DB2 and Vault. By leveraging the LDAP security plugin for Db2 and Vault’s OpenLDAP secrets engine, Db2 can offload authentication and authorization to the LDAP security plugin and allows Vault to manage static credentials or even generate dynamic users. For more details, refer to the [IBM Db2 Credentials Management](https://learn.hashicorp.com/tutorials/vault/ibm-db2-openldap) tutorial.
