...
- Ability to view client counts per auth (UI and API), and changes to clients over months (API), providing more granular visibility into clients.
...
- Support of PKCE on Vault’s OIDC auth method with with Telemetry support for the Vault Agent.
