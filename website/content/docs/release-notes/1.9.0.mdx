...
### Azure Secrets Engine Enhancements
...
This feature provides a more streamlined method for managing [KV v2 secrets](https://www.vaultproject.io/api-docs/secret/kv/kv-v2), enabling customers to better maintain least privilege security in automated environments. This feature allows performing partial updates to KV v2 secrets without requiring to read the full KV secret's key/value pairs. In the future,  new KV secrets and new versions for existing KV secrets without having to provide the ability to read from the engine. This feature also provides the ability to read a key from a KV endpoint but not the value.
