...
When you use Vault 1.13.0 without a TLS certificate on the TCP listener through which you access the Vault WebUI you may receive an error like the one below when performing operations within the Vault WebUI. This is an unintended behaviour that is resolved in the forthcoming Vault 1.13.1 release.
