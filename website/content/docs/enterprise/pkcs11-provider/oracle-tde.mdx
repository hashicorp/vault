...
[Oracle Transparenet Data Encryption](https://docs.oracle.com/database/121/ASOAG/introduction-to-transparent-data-encryption.htm#ASOAG10270) (TDE)
is supported with the [Vault PKCS#11 provider](/docs/enterprise/pkcs11-provider).
In this setup, Vault's KMIP engine will generate and store the "TDE Master Encryption Key" that the Oracle Database will use to encrypt and decrypt the "TDE Table Keys".
...
On the Vault server, we need to [setup the KMIP secrets engine](https://www.vaultproject.io/docs/secrets/kmip), e.g.:
