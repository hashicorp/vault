...
1. Configure the [KMIP Secrets Engine](/docs/secrets/kmip) and a KMIP *scope*. The scope is used to hold keys and objects.
    ...
    `server_hostnames` and `server_ips` [configuration parameters](https://www.vaultproject.io/api-docs/secret/kmip#parameters),
...
- `scope` (required): the [KMIP scope](https://www.vaultproject.io/docs/secrets/kmip#scopes-and-roles) to authenticate against and where the TDE master keys and associated metadata will be stored.
