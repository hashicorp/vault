...
Vault provides a PKCS#11 library (or provider) so that Vault can be used as a SSM (Software Security Module).
This allows a user to treat Vault like any other PKCS#11 device, so you can manage keys, objects, and perform encryption and decryption in Vault using PKCS#11 calls.
It uses Vault's [KMIP Secrets Engine](/docs/secrets/kmip) to provide cryptographic operations and object storage.
...
1. To use the provider, you will need access to a Vault Enterprise instance with the KMIP secrets engine.
    ...
    For example, you can start one locally (if you have a license in the `VAULT_LICENSE` enviornment variable) with:
...
- `VAULT_KMIP_CERT_FILE`: location of the TLS certificate to be used for authentication to the KMIP engine.
- `VAULT_KMIP_KEY_FILE`: location of the TLS key to be used for authentication to the KMIP engine.
- `VAULT_KMIP_KEY_PASSWORD`: password for the TLS key file, if it is encrypted to the KMIP engine.
- `VAULT_KMIP_CA_FILE`: location of the TLS CA bundle to be used to authenticate the connection to the KMIP engine.
...
The TLS key returned by the KMIP engine is by default unencrypted.
...
Note that only a single password can be supplied via the `VAULT_KMIP_KEY_PASSWORD`, so if multiple slots in the HCL file use encrypted TLS keys, they will either need to be encrypted with the same password, or use the `C_Login` method to specify the password.
...
- The key and object IDs returned by `C_FindObjects`, etc., are randomized for each session, and cannot be shared between sessions and have no meaning after a session is closed.
  This is because KMIP objects, which are used to store the PKCS#11 objects, have long random strings as IDs, but the PKCS#11 object ID is limited to a 32-bit integer, in addition to the fact that the PKCS#11 provider does not have any local storage.
- The PKCS#11 provider's performance will be heavily dependent on the latency to the Vault server and its performance.
