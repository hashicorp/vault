...
description: |-
  The Vault PKCS#11 Provider can be used with AWS KMS External Key Store.
---

# Vault with AWS KMS External Key Store (XKS) via PKCS#11 and XKS Proxy

~> **Important**: AWS [`xks-proxy`](https://github.com/aws-samples/aws-kms-xks-proxy) is not considered production-ready.
...
1. You should not be able to use this Vault-backed key with the KMS API, e.g., `aws kms encrypt`.