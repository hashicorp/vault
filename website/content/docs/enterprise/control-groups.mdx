...
```
In this case, `list` will require one admin approval and `create` will require 
one superuser approval and one admin approval. `Read` will require no extra approvals
