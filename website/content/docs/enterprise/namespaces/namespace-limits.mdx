...
post-unseal initalization takes. As a result, **even with auto-unseal**, Vault
may be unresponsive during initialization for deployments with many namespaces.
...
about the paths available within the namespace. Given `N` namespacecs, your
...
By default, Vault uses 256 workers to perform rollback operations on active
mounts once every minute. Mounts with a large number of namespaces can become
bottlenecks that slow down the overall rollback process. The effects of the
slowdown vary based on the particular mounts. At minimum, your Vault deployment
will take longer to fully purge stale data and perodic rotations may happen less
frequentyl than intended.
...
0 – 60         | [`vault.rollback.waiting`](/vault/docs/internals/telemetry/metrics/core-system#rollback-metrics)
...
<Warning title="Avoid OIDC with many namespaces">
  ...
  many namespaces can severly delay the rotation and invalidation of OIDC keys.
