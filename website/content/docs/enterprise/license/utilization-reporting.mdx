   ...
   - `metrics` - A map of representations of snapshot metrics contained within
     this snapshot
      ...
      - `labels` - The labels associated with each collected metric
         - `entity` - The counter of tokens generated for a unique client identifier
         - `nonentity` - The counter of tokens without an entity attached
- `metadata` - Optional product-specific metadata
   - `billing_start` - The unique ID of the Replicated license
   - `cluster_id` - The Salesforce Opportunity ID associated with the license
