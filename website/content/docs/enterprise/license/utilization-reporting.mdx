...
```
[DEBUG] core: automated reporting agent instantiated
[DEBUG] core.reporting: beginning snapshot export
[DEBUG] core.reporting: creating payload
[DEBUG] core.reporting: marshalling payload to json
[DEBUG] core.reporting: generating authentication headers
...
When you have a cluster, each node must have the reporting stanza defined in its
configuration file to be consistent. In the event of leadership change, nodes
will use their server configuration to determine whether or not to opt-out the
