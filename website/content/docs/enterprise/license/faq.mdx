...
1. Put the license on disk, configure auto-loading (new `license_path` config option, OR new environment variable `VAULT_LICENSE_PATH` OR new environment variable `VAULT_LICENSE` containing the actual license itself).
1. Restart the Vault node.
1. A warning will explain that the license is in storage and using auto-loading.
1. Use the new command `vault delete /sys/license` to fully transition to auto-loading.
