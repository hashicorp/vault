...
2. Backup the old license file in a safe location.
3. Test the validity of the new license on a temporary test server instance with the same version of the Vault you are using in production to make sure that the license is accepted. If that works, you can proceed to the next step.
4. Replace the old license file on each Vault server with the new one.
5. Invoke the [reload command](/api-docs/system/config-reload#reload-license-file) on each individual Vault server, starting with the standbys and doing the leader last. Invoking in this manner reduces possible disruptions if something was perfomed incorrectly with the above steps.

~> **Note**: You can either use the reload command or send a SIGHUP.

# ADP Licensing

This FAQ section is for the ADP license changes introduced in Vault Enterprise 1.8.

### Q: What are the Vault ADP module licensing changes introduced in 1.8?

As of Vaut 1.8, the functionality formerly sold as the Vault ADP module is now separated between two new modules:
...
**ADP-Transform** includes: \* [Transform Secrets Engine (TSE)](/docs/secrets/transform)
