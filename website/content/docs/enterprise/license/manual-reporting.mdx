...
1. Run this CLI command to generate a data bundle:
   ...
   By default, the bundle will include all historical snapshots. You are
   required to report at the minimum [x days] of data, but no more than 400
   days. 
   ...
   **Available command flags:**
   ...
   <Note title="Vault API">

   The equivalent API endpoint to generate a snapshot bundle is:

   `GET sys/utilization?today-only=:today-only-boolean-value`
