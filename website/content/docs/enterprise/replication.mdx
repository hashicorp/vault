...
of relationships are either **disaster recovery** (DR) or
**performance replication** (PR) relationships.
...
* `local`: only downstream DR clusters receive it
...
Shared mounts (the default) usually replicate all their data to PR secondaries, but
...
the same result for that mount when you send the query to either the PR primary or
secondary, but if you list stored certs, you'll see different values.

Local mounts replicate their data only to DR secondaries.  A local mount created on a PR
primary isn't visible at all to its PR secondaries. Local mounts can also be created on
PR secondaries, in which case they aren't visible to the PR primary.

There exist other storage entries that aren't mount specific.  For example, the Integrated
Storage Autopilot configuration is an `ignored` storage entry, which allows for DR secondaries
to have a different configuration than their primary.  Tokens and leases are written to
