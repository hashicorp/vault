...
~> Note: Automated integrated storage snapshots are not replicated in Performance Replication. Each Vault cluster (forming the Primary and Secondary Vault Performance Replicas) should be configured to create separate automated snapshots.
