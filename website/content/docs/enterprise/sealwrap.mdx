...
- The [LDAP](/docs/auth/ldap), [RADIUS](/docs/auth/radius),
  [Okta](/docs/auth/okta), and [AWS](/docs/auth/aws) auth methods,
  for storing their config.
- [PKI](/docs/secrets/pki) for storing the issuers and their keys,
- [SSH](/docs/secrets/ssh) for storing the CA's keys,
- [KMIP](/docs/secrets/kmip) for storing managed objects (externally-provided
  keys) and its CA keys.
- [Transit](/docs/secrets/transit) for storing keys and their policy.
