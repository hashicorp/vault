...
Long-term support offers extended maintenance (minor releases) for select, major
Vault Enterprise versions.
...
**extended maintenance** for an additional year with patches for bugs that may
cause outages, critical vulnerabilities and exposures (CVEs), and high-risk
security patches.
