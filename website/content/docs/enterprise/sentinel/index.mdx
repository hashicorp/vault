...
<Warning>
  Starting with Vault versions 1.15.0, 1.14.4, and 1.13.8 RGPs derived
  from identity group memberships are applied to requests in child namespaces,
  instead of parent ones. Read the scenarios below to familiarize yourself with the changes.
</Warning>
