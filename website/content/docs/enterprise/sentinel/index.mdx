...
Plus](https://www.hashicorp.com/products/vault/pricing/) license, or an [HCP
Vault Plus](/hcp/docs/vault/tiers-and-features#plus-tier) cluster.
...
1. If the request is unauthenticated, skip to evaluating the EGPs. Otherwise,
   ...
   request.
2. Evaluate RGPs attached to the cliet token. All policies must pass according
...
While ACL policies and EGPs set rules on a specific path, an RGP may not have a
target path. RGPs are tied to tokens, identity entities, or identity groups that
you can write rules without specifying a path.
