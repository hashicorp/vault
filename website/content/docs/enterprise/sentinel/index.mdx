...
- `ACLs` - These are the [traditional Vault
  policies](/docs/concepts/policies) and remain unchanged.
...
Like with ACLs, [root tokens](/docs/concepts/tokens#root-tokens) tokens
...
1. RGPs attached to the token are evaluated. All policies must pass according
   to their enforcement level.
1. EGPs set on the requested path, and any prefix-matching EGPs set on
...
system](/docs/enterprise/mfa) in Vault Enterprise. Within a single
...
See the [Examples](/docs/enterprise/sentinel/examples) page for examples
