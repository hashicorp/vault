...
HTTP methods, such as `GET` and `LIST` are currently not subject to request
limiting. This allows operators to continue querying server state without
needing to retry. This decision was based on the observation that read-based
requests are not as likely to cause untenable load than their
resource-constrained counterparts.

## `503 - Service Unavailable`

When Vault has reached capacity, the new requests will be immediately rejected
with a new retryable `503` error:
...
Unavailable` for this feature emphasizes that that the server is temporarily
under excess load, which may not be related to a specific client.
