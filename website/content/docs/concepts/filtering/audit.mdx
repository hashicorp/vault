...
however the additional complexity could mean that a Vault Operator configures their
...
When exclusively using filtered audit devices, enabling the fallback audit device
results in any audit entry which would have been ignored/missed due to filter settings
being sent to the fallback device to be written to an audit log.
...
```shell-session
vault audit enable -path=my-fallback file file_path=/var/audit.fallback.log fallback=true
```
