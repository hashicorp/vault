...
<Warning title="Proceed with caution">
  ...
  **Always** test your audit configuration in a non-production environment
  ...
  with Vault auditing and filtering basics before enabling filters on your audit
  devices.
...
<Warning title="Always use a fallback to ensure proper security">
...
1. You can configured filtering when enabling one of the following audit device
types:
...
the predicate expression and not write to the new device. However, the test
message **will** still be written to your fallback audit device.

You can determine whether the test message should appear in the sink for the
fallback or the newly enabled audit device based on the following property
...
1. Enable a `file` audit device with a `mount_type` filter:
  ```shell-session
    vault audit enable                  \
      -path kv-only                     \
      file file_path=/logs/kv-audit.log \
