...
description: Item recovery allows you to recover a specific item from a snapshot, rather than restoring the entire snapshot.
---
...
1. Using a snapshot file:
   ...
   ```
2. If you are using Vault's [automated snapshot](/vault/docs/enterprise/automated-integrated-storage-snapshots) feature, you can load a snapshot
...
```shell
    $ vault operator raft snapshot load \
        auto_config_name=my-config \
        url="https://example.com/path/to/snapshot"
```
