...
1. A new single-use token is generated with the TTL supplied by the client
1. Internally, the original serialized response is stored in the single-use
   token's cubbyhole
1. A new response is generated, with the token ID, TTL, and path stored in the
   new response's wrap information object
1. The new response is returned to the caller
...
1. Perform a lookup on the response-wrapping token. This immediately tells you
...
1. With the token information in hand, validate that the creation path matches
...
1. After prefix validation, unwrap the token. If the unwrap fails, the response
