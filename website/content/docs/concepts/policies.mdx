...
<Note title="The ui/mounts endpoint is unauthenticated"> 

  Vault uses the endpoint for UI and CLI preflight checks and is intended for
  internal use only. Requests that include an `X-Vault-Token` will return
  all mounts the token has path capabilities on.
