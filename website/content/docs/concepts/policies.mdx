...
The `/sys/internal/ui/mounts` endpoint provides a list of currently visible
mounts. The visibility of a mount is determined by the
[`listing_visibility`](/vault/api-docs/system/mounts#listing_visibility)
parameter of the mount. Visibility can be set when the mount is enabled or tuned
at a later time with the [sys/auth](/vault/api-docs/system/auth#tune-auth-method)
endpoint.
