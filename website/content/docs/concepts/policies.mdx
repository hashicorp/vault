...
## UI policy restrictions

The Vault UI may require different ACL capabilities compared to the CLI. The
`default` UI policy includes the following paths, **which cannot be modified with
subsequent policies** once you
...
<Note title="Set UI policies before enabling the UI"> 

  You cannot make policy adjustments or overwrites to the `ui/mounts` and
  `ui/resultant-acl` endpoints once you enable the Vault UI. Vault ignores
  policy updates that target these paths with
  [explicit `deny`](/vault/docs/concepts/policies#deny) capabilities.

</Note>

The [/sys/internal/ui/mounts](/vault/api-docs/system/internal-ui-mounts) API provide a 
list of mounts as per what's typically visible when logging into Vault and observing the 
personal cubbyhole for example. The listing of mounts are determined by [the `listing_visibility` parameter](/vault/api-docs/system/mounts#listing_visibility) 
that may be configured during the initial enabling of the mount or otherwise 
[tuned afterwards](/vault/api-docs/system/auth#tune-auth-method).

The [/sys/internal/ui/resultant-acl](/vault/api-docs/system/internal-ui-resultant-acl) API 
is required for appropriate adaptation to responses used by the UI and if it were
inaccessible then you may observe related warning or error messages within the UI itself.

~> **Note:** [/sys/internal/ui/mounts](/vault/api-docs/system/internal-ui-mounts) is only used 
internally for UI or CLI preflight checks and it is an unauthenticated API. When a `X-Vault-Token` 
is presented with requests, the response will include additional mounts which the token has 
been granted path capabilities on.

!> **Note:** No policy adjustment or overwrite can be made to these UI related path. The 
default `deny` or other policies targeting these paths with an [explicit `deny`](/vault/docs/concepts/policies#deny)
capability will have no effect.
