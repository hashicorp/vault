...
The Vault UI has a have a few considerations that are different to the capabilities 
granted to all other CLI or API paths. When the [UI is `enabled`](/vault/docs/configuration/ui#activating-the-vault-ui) 
and using the `default` policy the following paths are automatically included:
...
These UI related API paths may not be changed or altered via any policy once the UI 
is enabled.
...
inaccessible then you may observer related warning or error messages within the UI itself.
