...
- **Consider memory usage when setting `disable_mlock` with integrated storage**.
  When `disable_mlock` is set to `false`, meaning `mlock` is enabled, no memory
  will be swapped to disk. This is more secure than swapping, but can risk "out
  of memory" errors if memory is under provisioned. If `disable_mlock` is set
  to `true`, memory may be swapped to disk to relieve memory pressure, but
  secrets may be read by inspecting swap. Given the recommendations in this
  document, enabling `mlock` and increasing memory is advised.
