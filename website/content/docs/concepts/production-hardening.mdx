...
-	**Consider memory usage when configuring disable_mlock with integrated storage**.
  ...
  Vault data remains in RAM, but it increases the risk of out-of-memory (OOM) errors
  ...
  secrets being exposed through the swap file. Encrypting swap is one possible
  way of preventing the swap from being inspected.
