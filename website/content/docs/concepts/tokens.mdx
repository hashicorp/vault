...
!> **Warning**: Since tokens are considered opaque values, their structure is undocumented and subject to change. For these reasons, we do not recommend relying on the structure of tokens within your scripts or automations since it can cause breakage.
