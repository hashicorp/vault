...
~> Vault tokens will be valid until they expire, unless the associated
canonical entity is disabled or deleted, or the token is explicitly revoked.
A token may persist and still be valid after the [auth method](/docs/concepts/auth#auth-methods)
that created it has been deleted. The [auth disable command](/docs/commands/auth/disable)
disables all tokens granted by the specified auth method.
