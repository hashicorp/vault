...
generates at startup.  It's clear how this can work when not using integrated
...
The simplest but least desirable solution is to do it by hand: issue raft join
commands specifying the explicit names or IPs of the nodes to join to.  In this
section we look at other TLS-compatible options that lend themselves more to automation.
...
Note that just because you're putting a name in the certificate's DNS SAN, doesn't
mean it actually has to be name registered in a DNS server.  Your nodes may not
have any names found in DNS, while still using certificate(s) that contain this
shared "servername".
...
#### Autojoin using a second TCP listener

Modify the Vault HCL to specify a second listener on another port that's used
solely for auto-join.  This second listener could use disable_tls in conjunction
with a network ACL or security group that limits access to the Vault nodes.

This technique was recommended at one point but is inferior to the others listed
here in that it provides a new attack vector.
...
#### DNS instead of autojoin

One proposal for making autojoin work in AWS is to have Vault reverse resolve
the IP addresses coming from go-discover and join to the resulting hostname
instead.  However, if the user is willing and able to configure DNS for each
Vault node (or is using Consul service registration), we can use DNS instead
of autojoin, as with the LB case.  In this case the retry_join target address
becomes the A record that maps to all Vault nodes.

Downsides: Not everyone uses DNS.  Not known to have been tested.  If not using
Consul for DNS, it feels like it could be slow, because we'd presumably be
round-robining across all nodes, and at the outset it may take time before we
actually reach a node that can be joined with.  Presumably in the LB and Consul
cases health checks would ensure that we don't get routed to a node that can't
be joined with, those we should validate that.
