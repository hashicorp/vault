...
Vault supports moving secrets and auth mounts, both within an across namespaces. The operation
is supported by API endpoints as well as a CLI helper around them.

## Why

There can be several reasons for wanting to migrate mounts. On OSS, the use cases could be for 
renaming mounts in order to align with org standards. 
On Enterprise, there are a few more potential use cases as namespaces come into the picture. 
...
will start the migration process, and the API will return a migration ID. That ID can in turn be used to monitor
the status of the migration using the `sys/remount/status` endpoint.
...
we have 3 namespaces ns1/, ns1/ns2/ and ns1/ns3. We want to move the mount `my-secret` from ns1/ns2/my-secret to ns1/ns3/my-secret. 
This can be done in one of two ways. The first way is to invoke `/sys/remount` from the root namespace with the `from` argument as ns1/ns2/my-secret 
...
The remount operation will result in existing leases on a mount being revoked. This is potentially the 
most time consuming part of the operation. Any dynamic secrets for a secrets mount, and any tokens associated
with an auth mount, will be revoked during the remount process.

## Configurations

The remount operation will retain all the configurations of the mount. For example, if you create a readonly role
on a database mount, or a user on an userpass mount, after remounting those will be available at the new location.

## Cleanup operations

There are 2 main cleanup operations that Vault operators will need to keep in mind after performing a remount.

### Entities and aliases

When an auth mount is moved, any aliases that refer to users on the auth mount could now point to an invalid mount.
This is not necessarily always true, a remount within a namespace will end in the aliases pointing to a valid mount,
but a remount across namespaces will always result in the aliases pointing to an invalid mount. In the latter case,
the Vault operator should find and remove those aliases from the source namespace, along with creating equivalent aliases
...
or in a parent of the target namespace, or updated to remove the original mount path if not. 
Any policies associated with roles on a moved auth mount, would need to be inspected
...
It is possible that the mount being moved has rate limit or lease count quotas associated with it. The remount process
will update these quotas to refer to the new path of the mount.

## Path Filters

The remount operation will respect any existing path filters. There are 3 use cases in particular that are being handled
here. 
