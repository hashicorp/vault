...
periodically. Tokens without entities are tracked separately and added to the "non-entity token" count.
...
As of Vault 1.9, the Vault Audit Log will contain a `client_id` field in the request. The `client_id` will
be empty for requests that are untracked by the Audit Log. Such requests are unauthenticated requests / 
login requests, requests made with the root token, and wrapped requests. The `client_id` field can also 
contain an Entity ID for requests that are made with tokens with entities, or a unique client ID for 
non-entitiy tokens. 

Consumers of the audit log will be able to distinguish between a non-entity `client_id` and a 
`client_id` that is also an entity IDs by either:
A. Comparing the value of the `client_id` in the request with the value of the `entity_id` in the `Auth`
section of the response and checking if they are the same.
B. Checking that the format of the `client_id` is `xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx`(this signifies 
that the `client_id` is an `entity_id`, versus a `client_id` which appears to be a base64 encoding 
signifies that the `client_id` is associated with a non-entity token). 
