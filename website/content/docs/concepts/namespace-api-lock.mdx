...
Vault will block all but a select few API endpoints from responding to clients
operating in a locked namespace (or a descendant of a locked namespace). The
endpoints that do respond, the exempt paths, are largely for checking the status
of various systems (e.g. `sys/health`), or for unlocking the API.
...
deployment strategies. One can restrict Vault access for just part of an
...
access for any entity under A, without disabling access for sub-organization B.
Once the cause has been identified and resolved, the API can be unlocked for A.
...
Vault will respond with an HTTP 503: Service Unavailable. It will also yield an
error of the form:

`API access to this namespace has been locked by an administrator - "nsA" must be unlocked to gain access.`

Similarly, the same error will be returned if a request is made in a descendant
of `nsA`.
