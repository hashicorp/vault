...
Non-entity tokens are tokens issued by the Token auth method without an identity. When you authenticate with Vault using auth methods other than the Token auth method, Vault generates a direct entity token upon successful authentication.

Client counts are not solely dependent on the identity entities to compute unique user authentications, as it would be easy to map to the client or users. In fact, computing client counts requires using a combination of unique identity entities and non-entity tokens. Below are some things to keep in mind:

* Tokens within Vault are the core method for authentication. You can use Tokens to authenticate directly, or use the [auth methods](/docs/concepts/auth) to generate tokens based on external identities dynamically.
* There are scenarios where tokens can be created outside of the identity system and without an associated entity. For this reason, unique identity entities alone cannot always add up to the total unique authentications made to Vault over a stipulated time period.
...
**Auditor tool**:
  ...
  - It was introduced in Vault 1.6
...
1. To compute new active unique clients (clients who are new to Vault) for the month of January, you need to use the UI to set the billing start date to January 1st and end date to January 31st:
...
If you selected an inaccurate billing period, then the clients reflected in the result will be incorrect. The API/UI considers the start date as a fresh start to re-count unique clients that may have been counted towards the billing period.
...
**Auditor tool**:
  - New updates to the client count logic (made in the API/UI starting Vault 1.8) are not reflected in the Vault auditor tool. We do not plan to update auditor tool to be consistent with the API/UI and will eventually deprecate it. This tool is meant to be a stop-gap until you upgrade to a newer version of Vault
...
* Some situations result in high cardinality aliases. The uniqueness of the identity alias is what defines the number of entities generated for an auth method. Some auth methods allow you to configure the identity alias (what defines the number of identities) as a value that is unique on every login. This will result in an unbounded client count. For instance, AWS IAM allows operators to configure the `iam_alias` to: Role ID (default), IAM unique ID, Full ARN.
AWS EC2 allows operators to configure the `ec2_alias` to one of: Role ID (default), EC2 instance ID, AMI ID. InstanceID is often unique per login. This results in an unbound client count.
For example:
...
The API tracks where a token was created, and an entity's token can only be created in the namespace they are a part of. Internally, the `TokenEntry` gets created when the token is created. When the token is used, we retrieve the namespace out of the `TokenEntry` and create the clientID. Therefore, even if a token is used in a child namespace, it is counted as a single client.
