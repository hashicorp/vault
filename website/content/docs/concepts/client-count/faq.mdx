...
~> **Note**: Starting in Vault 1.9, Vault changed the non-entity token computation logic to deduplicate non-entity tokens. Since the change was made, Vault 1.0 (via the UI, API, documentation, etc.) referred to these non-entity tokens as non-entity clients, and unique entities as entity clients. To summarize, starting in Vault 1.9, the terms used are: **total clients = entity clients + non entity clients**. Previously, the terms used were: total clients = unique entities + non-entity tokens.
...
- Following the above example, let's say after the move operation, the entity `abc` in `ns1/` has no other aliases associated with it, i.e., the alias for the source mount was its only alias. At this point, we can clean up and remove the `prat` entity in `ns1/`. This will decrease the client count by 1, as the old entity may have already been used during the billing period.
