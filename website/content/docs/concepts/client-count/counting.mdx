...
- Two non-entity tokens under the same namespace, with the same alias name and
  policy assignment will have the same entity assignment and are only counted
  **once**.
- Two authentication requests from a single ACME client for the same certificate
  identifiers from different mounts will have different entity assignments and
  are counted **twice**.
- An application authenticating with AppRol will have the same entity assignment
