...
As of Vault 1.12, the billing period for the activity log API can be specified to include the current month 
for the end date. For more information about how this can be done, please visit 
[the internal counters API docs](/api-docs/system/internal-counters.mdx). 

When the end date is the current month, the `new_clients` counts will be an approximation of the true 
number of new clients for the month, not an exact value. Note that the `new_clients` counts for the rest 
of the months will all be accurate. 

### Why An Approximation?

The `new_clients` counts for the current month is an approximation primarily to improve API 
performance and make the UI usable. In order to give an exact value for the current month's
...
The `new_clients` approximation is calculated is by using a [hyperloglog algorithm](https://en.wikipedia.org/wiki/HyperLogLog)
...
with 14 registers and the use of sparse representations when applicable. Some accuracy estimates can be found 
