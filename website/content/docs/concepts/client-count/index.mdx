...
## Entity assignment with secret syncs
...
sync entity** for client count calculations.
...
- Each synced secret is counted distinctly based on the path and namespace of
  ...
  which are both synced, then two distinct entities will be created.
...
- Secret syncs clients are only registered in Vault's client counting system
  when a sync happens. If you create `kv/secret` and associate it with a
  destination in January, update the secret in May, and then delete the secret
  in September, Vault will consider this client as only having been seen in
