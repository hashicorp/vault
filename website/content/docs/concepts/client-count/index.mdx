...
description:: |-
  Technical overview covering the concept of clients, entities, and TBD in Vault 
---
...
1. **Orchestrators** like Nomad, Terraform, Ansible, or Continuous
   Integration/Continuous Delivery (CI/CD) pipelines.
1. **Vault Agents and Vault Proxies** that act on behalf of an application or
   microservice.

## Identity and entity assignment

When clients authenticate, Vault assigns them a **client identity type**: 

Client identity type       | AuthN method
-------------------------- | ---------------------------------
Externally managed or SSO  | Active Directory, LDAP, OIDC, JWT, GitHub, username+password
Platform- or server-based  | Kubernetes, AWS, GCP, Azure, PKI, Cloud Foundry
Self                       | AppRole, tokens with no associated authN path or role
PKI mount: ACME            | PKI certificate

![Vault client types](https://www.datocms-assets.com/2885/1617325030-vault-clients.png)

Vault also assigns a unique identifier (**client entity**) in the
[Vault identity system](/vault/docs/secrets/identity) based on the
authentication method used or a previously assigned alias. **Entity aliases**
let clients authenticate with multiple methods but still be associated with a
single policy, share resources, and count as the same entity, regardless of the
authentication method used for a particular session.
...
## Entity assignment with PKI mounts
...
client entity to activities within any child namespaces because the client is
operating within the same isolated space. 

When a client authenticates across **different** namespaces, Vault treats the
single client as two distinct client entities because the client is operating
across different spaces potentially with different policy assignments and
resources.
...
Ultimately, non-entity tokens will trace back to a particular user in some way.
As a result, non-entity token count as a unique client entity based on the
combination of the assigned entity alias name, the associated policies, and the
