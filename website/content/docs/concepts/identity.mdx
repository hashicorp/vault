...
for both shared mounts and local auth mounts (using `local=true`) when Vault replication 
is in use.
If the goal of the marking an auth method as `local` was to comply to GDPR guidelines, 
then care must be taken in not setting data pertaining to local auth mount or local auth 
