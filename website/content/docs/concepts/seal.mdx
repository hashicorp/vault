...
1. The new active node will perform the migration. Monitor the server log in
   ...
   Note about seal-rewrap: Seal-rewrap gets invoked when changes in seal stanza is made, this is true even when you are trying to migrate from the same auto-unseal type like pkcs11 to pkcs11.
