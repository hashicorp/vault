...
to see that rewrap is running, and/or [`sys/seal-backend-status`](https://developer.hashicorp.com/vault/api-docs/system/seal-backend-status),
...
auto seals in a [simplified manner.](migration_from_auto_unseal_to_auto_unseal)
