...
state, Vault can access the physical
...
read the decryption key. The decryption key allows you to decrypt the data and access the Vault.

Prior to unsealing, the only possible Vault operations are to unseal the Vault and check the status
of the seal.

## Why?

Vault encrypts the data it stores.
It encrypts most data using an encryption key stored in the keyring.
You must have the encryption key to decrypt the data.

Vault encrypts the encryption key and the keyring itself using a root key.
You use the root key to decrypt the encryption key, but you decrypt the root key using another key known as the unseal key.

To summarize, Vault encrypts most data using the encryption key.
It encrypts the encryption key and the keyring using the root key, and it reassembles the root key using the unseal key.
...
1. [Step down](/vault/docs/commands/operator/step-down) the
   ...
   When you use integrated storage, ensure that quorum is reached and a leader is
   elected.
...
1. The new active node performs the migration. Monitor the server log in
   ...
   nodes in case of integrated storage. In Vault Enterprise, switching an auto seal
...
1. At this point, you can update the configuration files of all the nodes to have only the
...
Once the required threshold of recovery keys are entered, Vault migrates the recovery keys
...
is entered, the recovery keys will be kept and used as recovery keys in the new
seal.
...
still occur using the remaining seals, and Vault will rewrap values produced while a seal is down with all the seals when they become healthy again.
...
In addition to high availability, you can use seal HA to migrate between two
...
* You can only configure a maximum of three seals.
* As you must wrap seal wrapped values by all configured seals, it is possible
that large values may fail to persist because the size of the entry is multiplied by
