...
access to the Vault cluster until the mechanism is available again. **If seal the 
...
or similar.  
With Vault Enterprise one may use Disaster Recovery replicated clusters to act as a
backup in the case of a lost seal mechanism.
