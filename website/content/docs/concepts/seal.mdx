...
Note: When using the Shamir seal with multiple nodes, each node must be unsealed
with the required threshold of shards. Partial unsealing of each node is not
...
to be on a distinct client machine for better security.
