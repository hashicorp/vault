...
seal mechanism such that Vault can tolerate the loss of a seal service or device for
a time.  With Seal HA Vault can also start up and unseal if one of the configured
seals is still available (though Vault will remain in a degraded mode in this case).
While seals are unavailable, seal wrapping and entropy augmentation can still
occur using the remaining seals, and values produced while a seal is down will
...
the [`sys/sealwrap/rewrap`](/vault/api-docs/system/seal-rewrap) endpoint.
...
* Upon upgrading to Vault 1.15 or higher, Vault must start with Seal HA disabled
to perform migrations.  Then the beta environment variable can be set and an
extra seal added.
