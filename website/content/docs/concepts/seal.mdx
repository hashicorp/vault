...
There are certain operations in Vault that require a quorum of users to 
perform, e.g. generating a root token. When using a Shamir seal the unseal
keys must be provided to authorize these operations. When using Auto Unseal 
these operations require _recovery keys_ instead.
...
If an Auto Unseal method has failed, Vault may be explicitly placed in 
[Unseal Recovery Mode](#unseal-recovery-mode) via a configuration option to
...
- `disable-unseal-recovery`: If true, Vault will not be able to be unsealed
  using recovery keys.
...
When rekeying the recovery key, you can opt in or out of unseal recovery mode (see below).

## Unseal Recovery Mode

Auto seals that have failed or whose keys have been lost may in an emergency 
be unsealed using the recovery keys by placing the seal in Unseal Recovery Mode.
To do this, edit the seal stanza for the auto-seal, adding the option 
`recovery_keys_fallback = "true"`.  Then, using the recovery keys, unseal vault 
as if it was a Shamir seal.  This is possible if the cluster was initialized in 
version 1.13 or above, and `unseal-recovery-disabled` was not true at the time 
of initialization, or if the recovery keys are rekeyed with unseal recovery mode
requested.
...
Note that you may also migrate from an Auto Unseal in 
[unseal recovery mode](#unseal-recovery-mode).
