...
Vault encrypts the data it stores. You must have an encryption key
to decrypt the data. Vault stores the encryption key with the data
in the keyring, but it encrypts the encryption key using another key known as the root key.

You use the root key to decrypt the encryption key.
The root key resides with all other Vault data,
