...
When you [install the Vault binary](/vault/docs/install) to be run as a server,
...
1. Download a [precompiled binary or build from source](/vault/docs/install).
...
1. Create a directory to store Vault data. A good practice is to store Vault
   data, and Vault logs on a different volume than the operating system.
