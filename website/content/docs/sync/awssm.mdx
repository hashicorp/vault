...
Access to secrets can be allowed or restricted through the use of AWS Resource Tags attached to secrets. For example, the
...
Likewise, is it recommended to add a negative condition on other write-access policies not used by Vault to secrets to prevent
out-of-band overwrites:
