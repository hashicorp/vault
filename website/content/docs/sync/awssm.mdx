...
secrets when they either get dissocaited from the destination or deleted from Vault. This enables the
ability to keep control of all your secrets localized while leveraging the benefits Secrets Manager.
...
Once AWS credentials have been obtained, configured Vault with them to create an AWS destination. If
