...
with each desired destination. This means is associating a secret named `my-secret` with a
...
separately for each type of external system is being used. The supported destinations are listed:
