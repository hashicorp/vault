...
<Tip title="Use the project number, not the project ID">
  ...
  For Secret Manager secrets in GCP, a fully-qualified resource name must have the following format:
  ...
  the Project info panel on the web dashboard.
...
all write-access role bindings not used by Vault:
