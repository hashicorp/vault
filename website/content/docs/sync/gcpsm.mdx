...
In situations requiring stricter control over of where data is stored, such as for compliance, GCP allows for specifying
the regions where secrets are replicated to. Multiple GCP location names may be specified per-destination upon creation.
Regardless of the chosen regions to replication the secrets into, they will always be globally readable with correct permissions.
