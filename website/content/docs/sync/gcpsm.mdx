...
You can allow or restrict access to secrets based on IAM
[conditions](https://cloud.google.com/iam/docs/conditions-resource-attributes#resource-name)
against the fully qualified resource name.

<Tip title="Use the project number not the project ID">

  For Vault secrets, a fully qualified resource name must have the format:
