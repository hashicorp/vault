...
1. To get started with syncing Vault secrets to your GitHub, you will need a configured [GitHub application](#github-application) or an
  ...
  which has write permissions for Actions secrets within the location you want to manage. In the list of GitHub permissions, Actions
  secrets is simply called "Secrets". Choosing Secrets permissions will automatically include read-only Metadata access.

  If a secrets location is not specified on the destination, it will default to repository secrets.
...
<Note title="Beta limitation">

Organization secrets are currently in **beta**. The `organization_visibility` and `selected_repository_names` fields cannot
be updated after a destination is created. We are looking into allowing these fields to be changed at any time post-creation.

</Note>

Organization secrets allow for secrets to be shared by an organization across many of its repositories. Various subsets of repositories
within the organization can be configured to have access to the secrets synced by the destination. The choices for repos within the organization are all
repositories, all private repositories, or a list of selected repositories. See the [API docs](/vault/docs/sync/github#api) for more information regarding configuration.
Be aware that organization secrets are [not visible to private repositories for GitHub Free accounts](https://docs.github.com/en/actions/security-for-github-actions/security-guides/using-secrets-in-github-actions#creating-secrets-for-an-organization).
