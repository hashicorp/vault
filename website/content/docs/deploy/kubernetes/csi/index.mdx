...
<Note title="Note">
  ...
  Installation via Helm runs the Vault CSI provider alongside [Vault Agent](https://developer.hashicorp.com/vault/docs/deploy/kubernetes/csi) within the same pod. If the Vault Agent container is to stop or restart, the leases managed by Vault Agent will eventually be revoked since the token used to create the leases cannot be renewed..
