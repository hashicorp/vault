...
There are values for Deprecation Status: Supported, Deprecated, Pending Removal, and Removed.

### Supported

This status reflects a feature which is still supported. There is no unique behavior or functionality associated with this status.
...
1) Any pre-existing mount will cause Vault to log an `Error`-level message in the Vault log and automatically shutdown after starting up after upgrade.
