...
As of version 1.12, Vault implements a multi-phased approach to deprecation. The intent of this approach is to provide sufficient warning that a feature will be removed and safe handling of stored data when the associated associated feature has been removed.

The phases of deprecation are also known as "Deprecation Status". These statuses are currently reflected in builtin plugins and are exposed via the Vault `auth`, `secrets`, and `plugins` API endpoints and CLI. For more information, refer to the corresponding documentation.

The four phases of deprecation are: `Supported`, `Deprecated`, `Pending Removal`, and `Removed`.

### Supported

This status is the default, and reflects a feature which is still supported. There is no unique behavior or functionality associated with this status.
...
1) After an upgrade, any pre-existing mount will cause Vault to log an `Error`-level message in the Vault log and automatically shutdown.
...
This status reflects a feature which has been officially removed from Vault. This is the last phase of the deprecation process. During this phase, the code for this feature has been officially removed from Vault. In order to start Vault, any offending features must be removed by first downgrading Vault and carefully replacing `Removed` features as shown in the table below.
