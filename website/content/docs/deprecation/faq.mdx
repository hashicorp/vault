...
Note: Deprecation Status currently only applies to builtin `auth` and `secrets` plugins. All other plugins will report a status of `n/a`. This is expected behavior.
