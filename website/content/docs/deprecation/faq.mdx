...
This workaround will fail in a future version of Go, but the Go team has not said when.

Here are the use cases we that might use certificates with SHA-1 still:

- AWS Credential Plugin: AWS [still supports SHA-1 signatures](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/instance-identity-documents.html).
