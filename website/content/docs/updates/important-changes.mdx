...
Requests to auth methods mounted at paths using underscores (e.g. `oidc_test`)
...
1. Navigate directly to the URL for the desired method type: `${VAULT_ADDR}/ui/vault/auth?with=oidc`
1. Click "Sign in with other methods ->"
1. Select type from the dropdown
