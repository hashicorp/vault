...
As of version 1.20.0, Vault has added support for [authenticating to Snowflake
databases using key pairs](/vault/api-docs/secret/databases#configure-connection).
Snowflake is disabling password authentication for all users in November 2025,
and password authentication will eventually be removed in subsequent Vault releases.

Vault currently does not support rotate root for key pairs. Users can manually rotate the key pairs
by updating both the root configuration in Vault with the new private key and the public
key associated with the user within Snowflake. For more information on rotating key pairs, please refer
