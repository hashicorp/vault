...
or deleted and are used to decrypt older data. If old keys must be disabled or deleted, then [seal-rewrap](/vault/api-docs/system/sealwrap-rewrap#start-a-seal-rewrap-process) needs to be performed so that the vault data encryped by old key can be decrypted using the new key. 
