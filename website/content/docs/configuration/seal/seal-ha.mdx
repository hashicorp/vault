...
case) to any environment variable names.  For example, in the below example,
`ACCESS_KEY` could be specified as `ACCESS_KEY_aws_east`.  Keep in mind that
the seal name must be valid in an environment variable name to use it.
