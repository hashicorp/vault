...
Rate limit quotas protect Vault against external distributed denial of service
(DDoS) attacks and are fundamental to Vault's security model. Therefore, it is a
part of Vault's core feature set available in both Community and Enterprise
Editions.
...
The table lists available parameters to create a rate limit quota.
...
| `inheritable`    | boolean | **Vault Enterprise:** Determine whether to apply the quota rule to child namespaces.                                                                     |


<Tabs>
<Tab heading="CLI command" group="cli">

Use `vault write` and the `sys/quotas/rate-limit/{quota-name}` path to create a new rate limit quota.
...
workload to 1500 requests per second.
...
1. Create a file named, `main.tf` with the following content.
   ...
   to 1500 requests per second (line 5 - 9)
...
For Vault Enterprise clusters, additional parameters are available on rate limit
quotas are available.

You can use Vault identities to group incoming requests using `group_by` paramter. 

- **Group_by: IP** – same as the existing behavior and default behavior going forward
- **Group_by: entity** – creates one bucket for each entity ID 
   - **Group_by: entity_then_none** - groups requests on namespace, mount, or path by entity ID. For requests that have an entity ID, they are bucketed by entity ID and the rate applied to each bucket. Requests that have no associated entity ID are all handled by a collective secondary quota. 
   - **Group_by: entity_then_ip** - groups requests on namespace, mount, or path by entity ID. For requests that have an entity ID, they are bucketed by entity ID and the rate applied to each bucket. Requests that have no associated entity ID are instead bucketed by IP and the rate applied to each bucket. 
   Recommended if customers have reliably mappable IPs (specifically, customers with the three problems listed above will probably want to use entity_then_none instead) 
   - **Secondary_rate** – a configuration that allows customers to set a separate rate for entity ID than for the _then_[blank] secondary options. For example, with rate R = 1000 requests per second group_by: entity_then_none and secondary_rate = 2000 requests per second, items with an entity ID would be bucketed per entity ID at 1000 requests per second and items without an entity ID would be collectively limited at 2000 requests per second 
- **Group_by: none** – creates one bucket for the namespace, path, or mount and collectively limits all traffic to that target
