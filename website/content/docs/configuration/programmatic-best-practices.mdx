...
- **Use dynamic provider credentials for Vault when possible**. Dynamic provider
  credentials generate short-lived credentials as needed, which limits the need
  for static credentials and improves the security posture of your integration.
...
- **Use Vault-backed dynamic credentials for different cloud providers**.
  Vault-backed dynamic credentials limits the need for unique dynamic provider
  credentials across different cloud providers so you can centralize sensitive
  data management with Vault while generating short-lived credentials with
...
- **Use distinct token capabilities**. Use the minimal required Vault token
  ...
  KV datasource and drift detection functionality only require the `read` capability. The `create` or `update` capability is required to manage the KV resource and depends on whether the resource already exists. The `delete` capability is required if you remove the KV resource from the Terraform configuration.
