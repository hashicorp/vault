...
1. Create a new Terraform Vault Provider resource file called
   `vault_namespaces.tf` that defines each of the new or existing namespaces
   resources you want to manage. For example, to migrate the `admin` namespace
   and enable a new `dev` namespace:
...
1. Create a new Terraform Vault Provider resource file called `vault_secrets.tf`
   that defines each of the new or existing auth method resources you want to
   manage. For example, to  migrate the existing `transit` and `admin_keys`
   secret engines and enable a new `kv` engine under the new `dev` namespace
   called `dev_keys`:
...
1. Create a Terraform Vault Provider resource file called `vault_policies.tf`
   with the policy resources you need to migrate:
...
1. Update the `vault_policies.tf` file with any new policies you want to add.
   For example, to create a policy for the new `dev_keys` secret engine:
   ...
   policy = <<EOT
    path vault_mount.dev_keys.path {
...
1. Save your new resource files to your Terraform configuration directory.

1. Format your new configuration files:
  ```shell-session
  $ terraform fmt
...
1. Confirm the new configuration is valid:
  ```shell-session
  $ terraform validate
...
## Step 5: Import the preexisting root-level resources

Use the `terraform import` command to import the preexisting resources.

For example, import the `admin` namespace, `default` policy, and `transit`
plugin from the previous steps:

```shell-session
$ terraform import vault_namespace.admin admin

vault_namespace.admin: Importing from ID "admin"...
vault_namespace.admin: Import prepared!
  Prepared vault_namespace for import
vault_namespace.admin: Refreshing state... [id=admin]
...
```shell-session
$ terraform import vault_policy.vault_default default

vault_policy.vault_default: Importing from ID "default"...
vault_policy.vault_default: Import prepared!
  Prepared vault_policy for import
vault_policy.vault_default: Refreshing state... [id=default]
...
```shell-session
$ terraform import vault_mount.transit transit

vault_mount.transit: Importing from ID "transit"...
vault_mount.transit: Import prepared!
  Prepared vault_mount for import
vault_mount.transit: Refreshing state... [id=transit]
...
## Step 6: Import the preexisting nested resources
...
For example, to import the `admin_keys` secret engine:
...
1. Import the `vault_mount` resource `admin_keys`:
  ...
  ```shell-session
  $ terraform import vault_mount.admin_keys admin_keys

  vault_mount.admin_keys: Importing from ID "admin_keys"...
  vault_mount.admin_keys: Import prepared!
    Prepared vault_mount for import
  vault_mount.admin_keys: Refreshing state... [id=admin_keys]
...
1. Use the `terraform state show` command to check your Terraform state file and
  ...
  ```shell-session
  $ terraform state show vault_mount.admin_keys

  # vault_mount.admin_keys:
  resource "vault_mount" "admin_keys" {
...
## Step 7: Add the new Vault resources

1. Run `terraform plan` to confirm the list of new resources:
```shell-session
$ terraform plan
...
Terraform will perform the following actions:
  ...
  # vault_mount.dev_keys will be created
  + resource "vault_mount" "dev_keys" {
  ...
  # vault_namespace.dev will be created
  + resource "vault_namespace" "dev" {
...
1. Run `terraform apply` to create the new resources:
  ...
  vault_namespace.dev: Creating...
  vault_namespace.dev: Creation complete after 0s [id=dev/]
  vault_mount.dev_keys: Creating...
  vault_mount.dev_keys: Creation complete after 0s [id=dev_keys]
  vault_policy.dev_team: Creating...
  vault_policy.dev_team: Creation complete after 0s [id=dev-team]
...
1. Use the `terraform state show` command to check your Terraform state file and
  ...
  ```shell-session
  $ terraform state show vault_mount.dev_keys

  # vault_mount.dev_keys:
  resource "vault_mount" "dev_keys" {
