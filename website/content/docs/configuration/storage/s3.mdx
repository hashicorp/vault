...
- `ha_enabled` `(string: "false")` - Enable high availability mode. Use the
  string `true` to enable HA mode and the string `false` to disable HA support.
  You can also use the  `S3_STORAGE_HA_ENABLED` environment variable to 
  enable or disable high availability. The `S3_STORAGE_HA_ENABLED` environment
  variable takes precedence over the `ha_enabled` configuration parameter.
