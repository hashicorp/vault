...
- `performance_multiplier` `(integer: 0)` - An integer multiplier used by 
  ...
  Lower values represent higher frequencies with lower intervals while lower 
  values increase intervals with greater distances between internal events.

  By default, Vault will use a balanced timing value of 5 that's suitable for 
  most platforms & scenarios. Setting this to a value of 1 will configure Raft 
  to its highest-performance mode. The maximum allowed value is 10. Only 
  adjust this value if required and subject to the overall reliability your 
  platform (network, etc) as well as using available telemetry indicators that 
  may suggest a need for an increase or decrease.
