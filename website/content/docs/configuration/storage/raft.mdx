...
- `raft_wal` `(boolean: false)` - Setting this to `true` enables the
  [raft-wal](/vault/docs/internals/integrated-storage#configurable-raft-log-store) log store instead of the default of BoltDB.

- `raft_log_verifier_enabled` `(boolean: false)` - Setting this to `true` turns on the raft log
  verifier. The verifier periodically writes small raft logs and verifies checksums to 
  ensure that data has been written correctly. This works with both BoltDB and raft-wal log
  stores.
