...
- `trailing_logs` `(integer: 10000)` - This controls how many log entries are
  ...
  default value is 10000 which is suitable for all normal workloads. This is not the same as `max_trailing_logs`.
...
- `snapshot_interval` `(integer: 120 seconds)` - This controls how often Raft checks if a snapshot operation should be performed. Raft randomly staggers between this value and 2x this value to avoid the entire cluster from performing a snapshot at once. The default value is 120 seconds.
