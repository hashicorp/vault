...
- `hostname_header` `(bool: false)` - Enables the addition of an HTTP header
...
- `raft_node_id_header` `(bool: false)` - Enables the addition of an HTTP header
  in all of Vault's HTTP responses: `X-Vault-Raft-Node-ID`. If Vault is participating
  in a Raft cluster (e.g. using integrated storage), this header will contain the
  Raft node ID of the Vault node that serviced the HTTP request. If Vault is not
  participating in a Raft cluster, this header will be omitted, regardless of
  whether this configuration option is enabled.
