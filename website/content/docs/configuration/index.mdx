...
- `disable_mlock` `(bool: false)` – Required to be explicitly set
  if using integrated storage. Disables the server from executing the `mlock`
  syscall. `mlock` prevents memory from  being swapped to disk.
  Disabling `mlock` is not recommended unless using
  [integrated storage](/vault/docs/internals/integrated-storage).
  ...
  Versions of Vault prior to 1.20 defaulted to `false`.
