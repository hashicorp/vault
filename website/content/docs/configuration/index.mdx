...
of irrevocable leases. When the configured duration exceeds an irrevocable lease's expire time,
the lease will be deleted. The minimum value for this field is `2d`. When set to a value
below the minimum, it will default to `2d`. Deleting a lease may cause external resources to be orphaned.
