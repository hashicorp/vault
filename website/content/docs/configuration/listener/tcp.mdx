...
scans often examine such security related HTTP headers. In addition, an Application specific
custom headers can also be configured. For example, `"X-Custom-Header"` has been configured
in the example below. Note that if the same header is configured in both the configuration file and
