...
- `chroot_namespace` `(string: "")` – Specifies an alternate top-level namespace
  ...
  namespace path for the request. For example, if `chroot_namespace` is
  `admin` and a CLI call provides the `ns1` namespace, the full namespace
  path is `admin/ns1`. Calls to the listener will fail with a 4XX error if
