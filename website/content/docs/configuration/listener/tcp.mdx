...
- `chroot_namespace` `(string: "admin")` – Specifies the namespace that will be used
as the apparent root namespace by this listener. When not specified, this will default to
the usual root namespace. If the specified namespace does not exist, requests to this
listener will not succeed and a 400 error will be returned. If any namespace is simultaneously
provided through either the CLI flag or the `X-Vault-Namespace` header, it will be appended
to the `chroot_namespace` value specified in here to determine the absolute namespace path. 
