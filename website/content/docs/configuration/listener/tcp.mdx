...
- `max_request_size` `(int: 33554432)` – Specifies a hard maximum allowed
  request size, in bytes. Defaults to 32 MB. Specifying this value to be `0`
  results in the default value being used. Specifying a number less than`0` 
  turns off limiting altogether.
