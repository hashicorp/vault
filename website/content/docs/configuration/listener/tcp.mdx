...
- `chroot_namespace` `(string: "")` – Specifies an alternate top-level namespace
  for the listener. Vault appends the namespace provided in CLI the field or
  `X-Vault-Namespace` header to the top-level namespace to determine the full
  namespace path for the request. For example, if `chroot_namespace` is set to
  `admin` and the `X-Vault-Namespace` header has the value `ns1` namespace, the full
  namespace path is `admin/ns1`. Calls to the listener will fail with a 4XX error if
  the top-level namespace provided for `chroot_namespace` does not exist.
