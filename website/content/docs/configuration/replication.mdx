...
- `allow_forwarding_via_token` `(string: "")` - When set to `new_token`, causes requests sent to non-active nodes
  to be forwarded if the node does not yet have the token information in storage. 