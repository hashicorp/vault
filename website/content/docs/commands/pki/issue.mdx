...
description: |-
The "pki issue" command issues a new intermediate ("issuer") certificate, using
a parent certificate in vault.
---

# pki issue

This command creates a intermediate certificate authority certificate signed by
the <parent> in the <child_mount>, using the options to determine the fields on
that certificate.
...
- [options] are the superset of the options passed to generate/intermediate and
  sign-intermediate commands.  At least one option must be set.
