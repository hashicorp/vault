...
Frequently, one issued certificate is to be very similar to another.
...
~> **Note**: not all possible certificate fields are supported by Vault, and
  ...
  unknown fields are set there may not be a warning that those are missing from
  the new issuer.

- <child_mount> is the path of the mount in vault where the new issuer is saved.

- [options] are the superset of the options passed to generate/intermediate and
  sign-intermediate commands.  While every option is optional, at least one
  option must be set.
...
~> **Note**: It is only possible to generate a new certificate with an existing
   ...
   mount is provided. [TODO: I'm confused reading this]
