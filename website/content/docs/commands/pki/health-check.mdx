...
The usage of sign/sign-verbatim and issue APIs should be restricted against the root issuer. An intermediary issuer should be created, ideally in a different mount, signed by the root issuer and issue new leaf certificates using the intermediary issuer.
...
It is strongly suggested that roles either set `allow_glob_domains` or `allow_wildcard_certificates`, but not both to true. If both behaviors are required, we'd suggest splitting it into two roles.
...
Auto-tidy can be enabled with a good set of defaults with the following command:
...
enabling `auto-tidy` based on instructions above.
...
To help going forward, consider setting all role's `no_store` field to `true` and leverage [BYOC revocations](/vault/api-docs/secret/pki#revoke-certificate) to avoid storage.
