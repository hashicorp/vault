...
First perform a [CA rotation operation](/vault/docs/secrets/pki/rotation-primitives) for any with pending expiry.

Once successfully migrated to the new CA, the old CAs after they expire, can be removed using a [manual tidy](/vault/api-docs/secret/pki#tidy) run with `vault write <mount>/tidy tidy_expired_issuers=true` or the [delete issuer](/vault/api-docs/secret/pki#delete-issuer) API.
...
The following command can be used to enable auto-rebuilding of CRLs: `vault write <mount>/config/crl auto_rebuild=true`
...
The usage of `sign`, `sign-verbatim`, `issue`, and ACME APIs should be restricted against the root issuer. An intermediary issuer should be created, ideally in a different mount, signed by the root issuer and issue new leaf certificates using the intermediary issuer.
...
This health check checks each role to see whether the role allows localhost based issuance implicitly through (`allow_localhost=true`) with a non-empty allowed_domains value.

**Remediation Steps**:

Roles should have `allow_localhost` set to `false` and the `allowed_domains` field updated with an explicit list of allowed localhost-like domains. This is a best-practice to simplify understanding of roles.
...
This health check checks each role to see whether or not it allows wildcard issuance with glob domains as well - these two interact and can result in nested wildcards and other quirks.

**Remediation Steps**:

It is strongly suggested that roles either set `allow_glob_domains` or `allow_wildcard_certificates`, but not both to true. If both behaviors are required, we'd suggest splitting it into two roles unless required for all SANs on the certificate. Otherwise, add this role to `allowed_roles` to ignore future warnings about this role.
...
This health check checks each role to see whether it sets `no_store=false` or not. When used with lots of certificates and no temporal auto-rebuilding of CRLs, this can result in bad performance. Instead, [BYOC](/vault/api-docs/secret/pki#revoke-certificate) can be used for the occasional revocation and shorter certificate lifetimes used instead. Without auto-rebuild, this will be a warning, but when auto-rebuild is enabled, we can make it informational.

*NOTE* if the role is used for ACME issuance, `no_store=true` is required.

**Remediation Steps**:

Update the role setting the `no_store` field to `false` and switch to using [BYOC revocations](/vault/api-docs/secret/pki#revoke-certificate) and shorter certificate lifetimes if possible.
...
The following vault command can be used to set the desired audit parameters:
...
Add the missing headers to the `allowed_response_headers` and `passthrough_request_headers` with the command:
...
If ACME support is desired, the following command will enable the ACME specific headers as well
...
Auto-tidy can be enabled with a recommended set of defaults with the following command:
...
Read the tidy status endpoint, `vault read <mount>/tidy-status`, for possible error messages of why tidy has failed to run in the past, otherwise schedule a manual run of tidy or consider configuring auto-tidy

To manually run a tidy operation:
...
If a large amount of certificates have accumulated, verify that tidy has recently run using `vault read <mount>/tidy-status`. Consider
enabling `auto-tidy` based on instructions above and running a manual tidy.

Also ensure that certificates are not being renewed too soon based on lifetimes, roughly 2/3 of its lifetime, and
certificates are being issued with appropriate lifetimes based on their usage.

To help going forward, consider setting all roles' `no_store` fields to `true` and leverage [BYOC revocations](/vault/api-docs/secret/pki#revoke-certificate) to avoid storage.
...
Read about enabling ACME support in Vault within the [ACME Certificate Issuance](/vault/api-docs/secret/pki#acme-certificate-issuance) API documentation.
...
Add the missing headers to the `allowed_response_headers` with the command:
```
vault secrets tune \
  -allowed-response-headers="Last-Modified" \
  -allowed-response-headers="Replay-Nonce" \
