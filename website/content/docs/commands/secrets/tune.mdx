...
- `-delegated-auth-accessors` `(string: "")` - A list of permitted authentication
  accessors this backend can delegate authentication to. Note that multiple keys
  may be specified either by providing the key names as a comma separated string
  or by providing this option multiple times, each time with 1 key.
