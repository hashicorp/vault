...
- `-delegated-auth-accessors` `(string: "")` - A list of permitted authentication
  accessors this backend can delegate authentication to. Note that multiple accessors
  may be specified by providing this option multiple times, each time with 1 accessor.
