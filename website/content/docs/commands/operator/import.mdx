...
amount of data migrated for each source depends on the filters applied and the
...
destination, and mapping rules. To learn more about these types and secrets importing,
refer to [the Secrets Import documentation](/vault/docs/import).
...
- `apply` - Executes the secrets import operations to bring the specified secrets from the source into Vault. Apply first
  executes a plan, then asks the user to approve the results before performing the actual import.
...
- `-auto-approve` `(bool: <false>)` - Automatically response "yes" to all user-input prompts for the `apply` command.
