...
description: >-
  ...
  facilitate migrating Vault between configurations. It operates directly at the 
  storage level, with no decryption involved.
---

# operator migrate

Starts a storage backend migration process to copy all data from one backend to
another.

<CodeBlockConfig hideClipboard>

```shell-session
$ vault operator migrate [flags]

$ vault operator migrate [-help | -h]
```
...
`operator migrate` copies data between storage backends to facilitate migrating
Vault between configurations. This operates directly on encrypted data and does
not require a Vault server, nor any unsealing.

Keys in the destination storage backend will be overwritten, and the destination
should _not_ be initialized prior to the migrate operation. The source data is
not modified, with the exception of a small lock key added during migration.

This is intended to be an offline operation to ensure data consistency, and 
Vault will not allow starting the server if a migration is in progress.

### Configuration format

`operator migrate` uses a dedicated configuration file to specify the source
...
with the only difference being that two stanzas are required: 
`storage_source` and `storage_destination`.
...
### Migrate to integrated raft storage

The following configuration will migrate from Consul storage to integrated
...
Vault needs to be offline during the migration process. First, stop Vault.
...
@include 'cli/operator/flags/migrate-config.mdx'

<br /><hr /><br />

@include 'cli/operator/flags/log-level.mdx'

<br /><hr /><br />

@include 'cli/operator/flags/max-parallel.mdx'
...
@include 'cli/operator/flags/start.mdx'
