...
- `-max-parallel` `int: 1` - Allows the operator to specify the maximum number of lightweight threads (goroutines)
  ...
  the cost of more resources (e.g. CPU, memory). Permitted values range from `1` to the maximum value
  for an `integer`. When supplied, and greater than 1, then parallel migration will be attempted.
