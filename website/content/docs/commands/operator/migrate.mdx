...
- `-parallel` `int: 1` - Allows the operator to specify the maximum number of light weight threads (goroutines)
  which can be used to migrate data in parallel. This can potentially speed up migration on slower backends at 
