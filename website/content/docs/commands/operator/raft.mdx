...
Loads a snapshot into the cluster without restoring the cluster to the snapshot's state.
...
Unload a snapshot that was loaded with `vault operator raft snapshot load`.
