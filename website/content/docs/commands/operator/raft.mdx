...
State includes a list of all servers by name and IP address. Last Index can indicate how close the state on each node is to the leader.

A node can have a status of "leader", "voter", and "non-voter". Non-voters are described in <link>
...
- `server-stabilization-time` `(string)` - Minimum amount of time a server must
