...
During initialization, Vault generates an in-memory root key, which is stored alongside all other Vault data. The root key is considered encrypted and requires what's called an _unseal key_ to descrypt it.
