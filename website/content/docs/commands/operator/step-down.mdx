...
the lock and become active again. Due to this architecture, there may be a non-zero time
whilst the cluster has no active node, before another node grabs the lock and finishes
transitioning to active operation. This may result in failed forwarding of requests to the
leader node.
