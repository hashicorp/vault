...
- `-heap-profile` `(string: "")` - Directory where the generated profiles are
  created. Vault does not generate profiles when `heap-profile` is unset.
  Use `head-profile` temporarily during debugging sessions. Do not use
  `heap-profile` in regular production processes.
