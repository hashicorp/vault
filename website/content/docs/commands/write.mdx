...
arbitrary data. The specific behavior of this command is determined at the thing
mounted at the path.
...
To create an ACL policy, you invoke the `sys/policies/acl/` API endpoint.

```shell-session
$ vault write sys/policies/acl/admin policy=@admin-policy.hcl
```

Since policy management is a common task, the CLI provides a [`policy write`](/docs/commands/policy/write) command which simplify the policy creation.

```shell-session
$ vault policy write admin admin-policy.hcl
```

The two commands perform the same operation. The difference is the `policy` CLI
command simplifies the policy management by providing subcommands.
