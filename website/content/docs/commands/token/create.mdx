...
- `-explicit-max-ttl` `(duration: "")` - Explicit maximum lifetime for the
  ...
  exceeded. Uses [duration format strings](/concepts/duration-format).
...
- `-period` `(duration: "")` - If specified, every renewal will use the given
  ...
  requires sudo permissions. Uses [duration format strings](/concepts/duration-format).
...
- `-ttl` `(duration: "")` - Initial TTL to associate with the token. Token
  ...
  maximumTTLs. Uses [duration format strings](/concepts/duration-format).
