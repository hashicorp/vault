...
description: |-
  ...
  tokens from the old auth method are revoked, but all configuration
...
The command will trigger a remount operation, and use the returned migration ID to poll the 
...
```shell-session
$ vault secrets move ns1/approle/ ns2/new-approle/
```
