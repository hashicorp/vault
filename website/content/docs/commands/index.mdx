...
This behavior may be desirable if using Vault CLI on the server side.

~> Note: Disabling redirect following behavior could cause issues with commands such as 'vault operator raft snapshot' as this redirects to the primary node.
