...
There are two primary ways to get help in Vault: [CLI help](#cli-help) and [API
help](#api-help).

### CLI Help

Use `help` (or `-h` for shorthand) to see the help output which corresponds to
your Vault version.
...
**Example:** To get help on the `kv` subcommand.
...
Use `path-help` to see the help output for Vault API paths.
...
Use `path-help` to retrieve API help on any paths. Vault API endpoints provide
built-in help in markdown format. This includes system paths, secret engines,
and auth methods.

**Example:** To get API help on the [`auth/token/`](/api-docs/auth/token) path.

```shell-session
$ vault path-help auth/token
```

The help output displays supported child-paths and available parameters.

-> **CLI and API:** The [read](/docs/commands/read),
[write](/docs/commands/write), [delete](/docs/commands/delete), and
[list](/docs/commands/list) operations are available for most paths in Vault,
and those CLI commands are equivalent to HTTP verbs, `GET`, `POST` (or `PUT`),
`DELETE`, and `LIST`.

### Print cURL Command

CLI provides convenient wrapper for the Vault API. To see the equivalent API
call to perform the same operation, use the `-output-curl-string` flag after the
subcommand.

```shell-session
$ vault policy list -output-curl-string

curl -H "X-Vault-Request: true" -H "X-Vault-Token: $(vault print token)" http://127.0.0.1:8200/v1/sys/policies/acl?list=true
```
