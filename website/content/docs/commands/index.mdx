...
Prevents the Vault client from following HTTP redirects. By default, the Vault CLI client will only follow a single redirect and any additional redirection can result in an emptry or unexpected response. 
