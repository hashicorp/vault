...
Disables the default client behavior, which honors a single redirect response from a request.

Disables the default behavior of the Vault client, Prevent the Vault client from 
Do not verify Vault's presented certificate before communicating with it.
Setting this variable is not recommended and voids Vault's [security
model](/docs/internals/security).
