...
The `plugin runtime register` command registers a new plugin runtime in Vault's plugin runtime catalog.
Vault currently only supports plugin runtimes of type "container".
...
- `-cpu_nanos` `(int: 0)` - CPU limit to set per container in nanos. Defaults
  to no limit.

- `-memory_bytes` `(int: 0)` - Memory limit to set per container in bytes.
  Defaults to no limit.

- `-oci_runtime` `(string: "")` - OCI-compliant container runtime to use.
