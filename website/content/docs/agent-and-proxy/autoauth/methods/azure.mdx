...
- `authenticate_from_environment` `(boolean: false)` - If set to true, will attempt to make an Azure
  ...
  [Workload Identity][workload-identity] and others. If set to true, `object_id`, and
  `client_id` will be ignored in favour of this authentication approach.

- `scope` `(string: optional)` - The scope to use when obtaining the token. If not given, will be
  the value used for `resource` appended with `/.default` appended to it. For more information on
...
An example auto-auth configuration block for an example Azure [Workload Identity][workload-identity] configured client follows:
