...
description: |-
  The "agent generate-config" command generates a simple Vault Agent
...
refer to the [Auto-Auth docs](/vault/docs/agent-and-proxy/autoauth) for
...
Before generating a configuration file, insert a secret `foo`:
...
This will produce `agent.hcl` file in the current directory with the following
contents:
...
- `path` `(string: "")` - Path to a kv-v1 or kv-v2 secret
  (e.g. secret/data/foo, kv-v2/prefix/*); multiple secrets and tail '*'
