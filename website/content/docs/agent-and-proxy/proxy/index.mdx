...
- `-log-file` ((#\_log_file)) - writes all the Vault Proxy log messages to a file
at the path indicated by this flag. The filename defaults to `proxy.log`.
...
If the value ends in a path separator, `proxy-` will be appended to the value.
...
After rotation, the path and filename take the following form: `/var/log/proxy-{timestamp}.log`
