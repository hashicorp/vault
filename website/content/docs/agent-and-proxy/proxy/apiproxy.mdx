...
~> **Note**: When using the proxy's auto-auth token, one proxy per application is very
   strongly recommended, as Vault will otherwise be unable to disinguish requests coming from
   multiple applications through a single proxy. In situations where a single proxy is
   shared by multiple applications, setting `use_auto_auth_token` to `false` is recommended.
