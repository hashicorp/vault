...
An example policy snippet that would grant access static secret (KVv1 and KVv2) events is given below:
...
the cache, meaning Proxy only checks for a secret update upon receiving notification that the secret was updated.
