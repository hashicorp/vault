...
This functionality off by default, as it requires [auto-auth](/vault/docs/agent-and-proxy/autoauth) to be configured,
as well as the auto-auth token to have the permissions to subscribe to KV event updates via Vault's [event system](/vault/docs/concepts/events).
...
Refer to the [Vault Proxy Static Secret Caching][static-secret-caching] page for more information on
this functionality.
...
- `cache_static_secrets` `(bool: false)` - If set, Blah. Must TODO
. Refer to the [Vault Proxy Static Secret Caching][static-secret-caching] page for more information on
this functionality.
...
[static-secret-caching]: /vault/docs/agent-and-proxy/proxy#listener-stanza
