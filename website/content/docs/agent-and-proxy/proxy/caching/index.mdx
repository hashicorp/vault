...
Static secret caching is disabled by default. To enable secret caching you must
...
be `false`, as otherwise the cache does nothing. The top level `cache` block has the following configuration entries:
