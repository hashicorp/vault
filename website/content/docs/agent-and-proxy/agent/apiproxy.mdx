...
~> Note: Unlike Vault Agent, [Vault Proxy](/vault/docs/agent-and-proxy/proxy)'s API proxy supports
[the caching of static secrets](/vault/docs/agent-and-proxy/proxy/caching/static-secret-caching)
(KVv1 and KVv2), requiring fewer requests to be forwarded to Vault itself. We recommend
switching to Vault Proxy for API Proxy related workflows.
