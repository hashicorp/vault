...
via environment variables configured from `env_template` blocks
(see [Template Configurations](/vault/docs/agent-and-proxy/agent/template#template-configurations) for more info).
