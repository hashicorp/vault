...
!> Vault Agent's Process Supervisor Mode is in public beta. <br /> Please provide your feedback by opening a GitHub issue [here](https://github.com/hashicorp/vault/issues).
...
In many ways, Vault Agent will mirror the child process. Standard output streams (stdin/stdout/stderr) are all forwarded
to the child process. Also, Vault Agent will exit when the child process exits with the same exit code.
...
- `restart_stop_signal` `(string: "SIGTERM")` - Signal to send the child process when a secret has been updated and the
  process needs to be restarted. The process has 30 seconds after this signal is sent until `SIGKILL` is sent to force
