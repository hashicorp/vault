...
It is safe to run different versions, however you may not be able to take advantage of all the newest features in Vault if you do not upgrade to the most recent versions of Agent and Vault server. We recognize that this isn’t always possible, so we do support version mismatch as best as possible.

As of Vault Agent 1.13.0, Agent will write a note to its logs when it detects a mismatch between Agent and Vault server.  This is purely informative, intended to assist with debugging in case the mismatch is given rise to problems, e.g. because a newer Agent version is trying to make use of functionality that doesn't exist in the Vault server version it's talking to.  If Agent is behaving acceptably, the message may be ignored.
...
## Older version of Agent than Vault server

We do not anticipate any problems stemming from continuing to run an older Agent version after the Vault server nodes are upgraded to a later version.  Existing deployments using Agent should not be impacted, as we don't generally make backwards-incompatible changes to Vault server.
...
## Newer version of Agent than Vault server

It is possible that an Agent could depend on features that don’t exist in older Vault server versions.

Auto-auth:
- Agent may claim to support newer auth methods that have been introduced since Vault server was built, but they won't work due to Vault server not supporting them
- Agent may make use of new functionality for existing auth methods that isn't available in an older Vault server you're using
