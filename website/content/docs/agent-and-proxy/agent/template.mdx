...
or environment variables (via the [Process Supervisor Mode](process-supervisor)) using [Consul Template markup][consul-templating-language].
...
And the following demonstrates how the templates look when using `env_template` with [Process Supervisor Mode](process-supervisor)
