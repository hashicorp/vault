...
Each DR secondary is doing the heartbeats we spoke of earlier, containing that
...
1. Suppose that the active node on the to-be-promoted DR secondary has only just
...
be the address in the primary's failover address cache, and the only address from
this cluster that gets written to `core/primary-failover-addrs/dr`.  Now the current
