...
Perf standbys regularly issue heartbeat RPC requests to the active node, and
...
requests.  gRPC will discover that perf standbys can't service most RPCs, and
...
goes through its loop, and when gRPC asks for a forced ResolveNow, which seems
to happen when it's unable to perform RPCs on any of its target addresses.  In
...
RPCs, it can be serviced by perf standbys as well as the active node.  In
