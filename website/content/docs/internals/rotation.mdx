...
the guidelines of NIST publication 800-38D.  It is recommended that operators
monitor the sum of `vault.barrier.put` and `vault.token.creation` (with a label 
of `token_type` equaling `batch`) to estimate the encryption rate.  Often the simplest
strategy is to use those metrics to determine a frequency of rotation and make
that part of the operational process.  For example, if one determines that the 
estimated rate is 40 million operations per day, then rotating the key every
