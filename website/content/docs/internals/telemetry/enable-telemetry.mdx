...
<Tip heading="Use a prefix filter to reduce the volume of metrics you receive">
  ...
  Many metrics solutions charge by the metric. You can use the `prefix_filter`
  parameter to include and exclude specific values based on metric name to avoid
  paying for irrelevant information.
