...
plugin processes may be terminated by Vault. These cases include the following:
...
transparently Vault, and termination of these processes are typical in the above
scenarios. Vault will re-start these plugin processes if and when needed on the
next operation that requires an RPC call to the plugin, whether that's coming
from an explicit request operation, or as part of a background process from
Vault's expiration manager, for instance.
