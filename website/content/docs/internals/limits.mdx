...
enabled as this depends on the particular plugin resource utilization, the
extent to which that plugin is being called, and the available resources on the
system. In general, for plugins of the same type, each additional process
spawned results in a linear increase in resource utilization, if they operate
largely in the same fashion and frequency.


These processes
may continuously run in the background or be
