...
Alternatively, the [Vault telemetry metrics](/vault/docs/internals/telemetry) `vault.core.mount_table.num_entries` and `vault.core.mount_table.size` are available to monitor the number of mount points and size of each mount table, respectively. 
