...
The Raft protocol will not be covered in full here. A high level description is 
provided which may be useful for building a mental model. Refer to the 
complete specification that's described further on [this paper](https://raft.github.io/raft.pdf).
...
As soon as a node joins a Raft cluster, it attempts to catch up with the reset 
of the nodes via the data that it's replicating from the leader. While in 
...
For example - there may be a scenario where:
