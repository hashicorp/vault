...
I would probably move the warning about joining nodes to the Autopilot docs in a "If you disable server stabilization then keep this in mind ..." section. But we should probably make sure it can in fact be disabled first. The other changes in that PR are 
...
for when a node is treated as healthy before it's then an eligible voter in the 
Quorum list. Other features which may enabled include the ability to remove nodes 
considered as dead from the Quorum list after a certain period.

Autopilot is typically enabled in most current versions of Vault with default 
values that should be adapted to best suit the environmental requirements where 
Vault instances are deployed.

Vault 1.7.x & higher include a stabilization logic that's available as part of 
Autopilot and activated by default. Nodes that are new and a recent joiner are 
...
The Vault Reference Architecture recommends a 5 node cluster to ensure a minimum 
failure tolerance of at least 2.
...
never exceeding the available failure tolerance in any given one-time.
