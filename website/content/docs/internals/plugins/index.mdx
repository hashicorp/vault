...
desired plugin version. Next, navigate to the `cmd/:plugin-name` directory
contained alongside that plugin's code. For example, for AppRole navigate to
the `vault/builtin/credential/approle/cmd/approle` directory. Once in that
directory, run `go build` to obtain a new binary for the AppRole plugin.
Finally, [register](/docs/internals/plugins/plugin-architecture#plugin-registration)
the plugin and enable it.
