...
But, there is one additional mechanism to keep in mind and that is since the root key is stored alongside all other Vault data, it too is considered encrypted and requires what's called an _unseal key_ to decrypt it.
