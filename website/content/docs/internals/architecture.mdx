...
Components are separated and considered as either inside or outside of the security barrier. The storage backend and HTTP API are outside of the security barrier, while all other components reside inside the barrier.

When the Vault server starts, a storage backend is provided, making its data available across restarts. The storage backend, outside the barrier, stores encrypted data durably and is considered untrusted. The HTTP API must start similarly by the Vault server so that clients can interact with it.
...
providing the unseal keys. Once Vault is initialized, it generates an
...
All security-sensitive configurations for audit devices, auth methods, and secrets engines must be stored in Vault. Users with permissions can modify them and cannot be specified outside of the barrier. By storing them in Vault, changes are protected by the ACL system and tracked by audit logs.
...
client token is returned to the client and is used to make future requests.
