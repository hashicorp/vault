...
Vault is designed to support a highly available deployment to ensure that machine or process failure is minimally disruptive.

Primarily, Vault is used to manage secrets in production environments. Please note that downtime of the Vault service may affect downstream clients.

~> **Advanced Topic!** This page describes the technical details
of Vault. The details are documented here for knowledge and familiarity with Vault. A review of this section is not required to use Vault. However, due to the signficance of Vault in an environment, we recommend knowing the architecture for all Vault operators.
...
Storage backends, such as Consul, provide additional coordinative
