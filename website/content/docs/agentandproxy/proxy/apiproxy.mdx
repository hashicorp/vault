...
has been configured in the [`vault` stanza](/vault/docs/agentandproxy/proxy#vault-stanza) of Proxy Agent. This enables access to the Vault
...
request to Vault. See the [caching docs](/vault/docs/agentandproxy/proxy/caching) for more information on caching.

## Using Auto-Auth Token

Vault Proxy allows for easy authentication to Vault in a wide variety of
environments using [Auto-Auth](/vault/docs/agentandproxy/autoauth). By setting the
