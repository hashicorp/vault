...
Caching](/vault/docs/agentandproxy/agentandproxy/proxy/caching/persistent-caches) page for more information on
this functionality.
...
terminated and there is no way for ageproxynt to resume renewals after the fact.
...
[proxy-listener]: /vault/docs/agentandproxy/proxy#listener-stanza
...
the [API Proxy](/vault/docs/agentandproxy/proxy/apiproxy) and enables the API proxy to use the cache, if configured.
