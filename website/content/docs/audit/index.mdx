...
~> Note: Audit device configuration is replicated to all nodes within a cluster. Before enabling an audit device, ensure that all nodes within the cluster will be able to successfully log to the audit device to avoid Vault being blocked from serving requests. 
