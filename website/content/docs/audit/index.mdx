...
Vault Operators can audit specific HTTP headers when those headers are present in
the incoming client request. Configuration for audit request headers is done via
the [Vault API](/vault/api-docs/system/config-auditing).

When Vault Operators [create](/vault/api-docs/system/config-auditing#create-update-audit-request-header)
an exception allowing a request header to appear in the audit log, by default the
value of the header is **not** to be HMAC'ed, this is [configurable](/vault/api-docs/system/config-auditing#hmac)
per header.
...
<Warning title="Behavior changes">
  ...
  Prior to Vault 1.17 **only** headers configured by the Vault Operator could appear
  in an audit log.

  As of Vault 1.17 this behavior changes see below for further information.

</Warning>

As of Vault 1.17 the following headers are **always automatically enabled** in
order to facilitate correlation of requests across distributed systems which
include Vault.

- `correlation-id`
- `x-correlation-id`

By default, the values of these headers are **not** [HMAC'ed](/vault/api-docs/system/config-auditing#hmac),
however Vault Operators may override this setting by [manually configuring](/vault/api-docs/system/config-auditing#create-update-audit-request-header)
the headers described above, supplying an `HMAC` value of `true`.

#### Example override for `correlation-id`
...
#### Example override for `x-correlation-id`

```shell
curl \
    --header "X-Vault-Token: ..." \
    http://127.0.0.1:8200/v1/sys/config/auditing/request-headers/x-correlation-id \
    --data '{ "hmac": true }'
```
