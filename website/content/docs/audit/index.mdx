...
the incoming client request. Configuration for audit request headers is applied via
...
value of the header is **not** HMAC'd, this is [configurable](/vault/api-docs/system/config-auditing#hmac)
per header.
...
By default, the values of these headers are **not** [HMAC'd](/vault/api-docs/system/config-auditing#hmac),
