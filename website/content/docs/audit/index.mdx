...
- `fallback` `(bool: false)` - If enabled, marks this audit device as the [fallback
  device](/vault/docs/concepts/filtering/audit#fallback-device) to in conjunction
  with [filtering](/vault/docs/concepts/filtering/audit).
  **Vault supports enabling only a single fallback audit device**.

- `filter` (`string: ""`)` - Allows a predicate to be used by the audit device in
  order to filter which audit entries this device should attempt to log.
  See [filtering](/vault/docs/concepts/filtering/audit) for more information.
