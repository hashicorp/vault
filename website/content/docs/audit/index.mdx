...
~> Note: Audit device configuration is replicated to all nodes within a cluster by default. Before enabling an audit device, ensure that all nodes within the cluster will be able to successfully log to the audit device to avoid Vault being blocked from serving requests. An audit device can be enabled as a local device with the [`local`](api/system/audit#local) parameter. 
