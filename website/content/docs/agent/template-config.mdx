...
The parameter `error_on_missing_key` can be specified within the `template`
...
If the desired is to have Agent fail and exit on a missing key, both
...
- `exit_on_retry_failure` `(bool: false)` - This option tells Vault Agent to create
