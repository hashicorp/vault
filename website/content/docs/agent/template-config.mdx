...
Below is an example setting of `template_config` where the additional 
[`error_on_missing_key` parameter in the template stanza](/docs/agent/template#error_on_missing_key) in conjunction with 
`exit_on_retry_failure` result in the agent existing in case of no key / value 
instead of the default retry behaviour when no configuration is set.

```hcl
# ... top level configuration parameters truncated.
template_config {
