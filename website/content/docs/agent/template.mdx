...
In another setting `template_config` with [`error_on_missing_key` parameter in the template stanza](/docs/agent/template#error_on_missing_key)
as well as `exit_on_retry_failure` result in the agent exiting in case of no key 
/ value issues instead of the default retry behaviour.
...
- `error_on_missing_key` `(bool: false)` - Exit with an error when accessing
  ...
  to "true". Also see [`exit_on_retry_failure` in global Vault Agent Template Config](/docs/agent/template-config#interaction-between-exit_on_retry_failure-and-error_on_missing_key).
