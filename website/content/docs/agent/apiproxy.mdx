...
has been configured in the `vault` stanza of Vault Agent. This enables access to the Vault
...
go through and utilize the cache subsystem. See the [caching docs](/docs/agent/caching)
...
The following two `api_proxy` options are only useful when talking to a Vault
