...
- `wrap_ttl` `(string or integer: optional)` - If specified, the written token
  ...
  structure. Uses [duration format strings](/concepts/duration-format).
...
- `min_backoff` `(string or integer: "1s")` - The minimum backoff time Agent
  ...
  Uses [duration format strings](/concepts/duration-format).
...
- `max_backoff` `(string or integer: "5m")` - The maximum time Agent will delay
  ...
  performed before giving up. Uses [duration format strings](/concepts/duration-format).
