...
[described in the API Proxy documentation](/docs/agent/apiproxy#configuration-api_proxy) for
legacy reasons. This configuration cannot be specified alongside `api_proxy` equivalents,
should not be preferred over configuring these values in the `api_proxy` block,
and should be considered legacy.
...
the [API Proxy](/docs/agent/apiproxy) and enables the API proxy to use the cache.
