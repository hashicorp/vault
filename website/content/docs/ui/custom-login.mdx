...
- **You must have the appropriate permissions**:
  - You must have `update` permission for the [`sys/config/ui/login/default-auth`](/vault/api-docs/system/config-ui-login-default-auth) endpoint.
...
----------------------- | ------
Default only            | User only see the selected authentication method during login.
Backup(s) only          | User only sees from the selected authentication method(s) during login.
Default and backup(s)   | User see the default method first but can access the backup methods as alternate options.
...
so the Vault GUI knows which login URL to authenticate against.
...
As a result, the Vault GUI assumes each login method is mounted at its default mount [path](/vault/api-docs/system/auth#path), 
...
make it obvious that the mount is a default or backup login method(s) for the
Vault GUI.
