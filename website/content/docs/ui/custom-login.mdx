...
If at any point Vault GUI users need to authenticate with an authentication method that is not
defined as a default or backup method they can navigate directly to any method type using the 
`?with=` query parameter in the URL. 

`${VAULT_ADDR}/ui/vault/auth?with=token` 

It is recommended to leverage the query parameter for mount paths tuned with `listing_visibility="unauth"`.
This displays a default view for that method and logs in at the specified path. 

`${VAULT_ADDR}/ui/vault/auth?with=gui%2Fldap%2F`
