...
Use login customizations to preferred authentication methods and control which methods display in the Vault GUI login form.
...
Only the following methods are supported by the GUI and can be set as a default or preferred backup method:
...
the login settings using a direct URL for their method of choice. 

#### URL for `root` namespace
`${VAULT_ADDR}/ui/vault/auth?with=<method_type_or_mount_path>` 

#### URL for a child namespace
`${VAULT_ADDR}/ui/vault/auth?namespace=<child_namespace>&with=<method_type_or_mount_path>` 

The query parameter accepts either a method [type](/vault/api-docs/system/auth#type) or [path](/vault/api-docs/system/auth#path).
If specifying a path, the mount must be tuned with `listing_visibility="unauth"`  

URL specifying type: `${VAULT_ADDR}/ui/vault/auth?with=token` 
URL specifying a custom mount path: `${VAULT_ADDR}/ui/vault/auth?with=gui%2Fldap%2F`
