...
the custom messages created on a [namespaces](/vault/docs/enterprise/namespaces)
are inherited by its child namespaces. To create custom messages specific to a
namespace, be sure to log into your target namespace.
...
Each namespace is dedicated to an organization.

To create a custom message intended to the marketing organization, you must log
into the `marketing` namespace.
...
namespace and its child namespaces. To verify, log into the parent namespace
(e.g. `admin`) to make sure that the banner does not show.
