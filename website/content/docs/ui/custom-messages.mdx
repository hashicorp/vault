...
- **You must have a Vault Enterprise license for Custom Messages.**
- **You must have Vault 1.16.0 enterprise or higher installed.**
...
### Namespaces consideration

When you are running Vault Enterprise or [HCP Vault](/hcp/docs/vault) clusters,
the custom messages created on a [namespace](/vault/docs/enterprise/namespaces)
are inherited by its child namespaces.

For example, your cluster has the following namespace hierarchy.
...
To create a custom message intended to the marketing team but not for `finance`,
you must log into the `marketing` namespace to create a message.

1. On the Vault UI login page, enter `marketing` in the **Namespace** text
   field, and select an authentication method to log in.
...
1. Select the **+ Create message** button in the toolbar to open and fill out
   ...
   ![Custom message](/img/ui-custom-msg.png)

1. Click **Preview** to see how your message will appear to users.

1. Click **Create message** to save your new message.

The message banner will only appear when a user logged into the `marketing`
namespace and its child namespaces. To verify, log into the `admin` or `finance`
namespace to make sure that the banner does not show.
