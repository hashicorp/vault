...
When you are running Vault Enterprise or HCP Vault clusters, the custom messages
created on a [namespaces](/vault/docs/enterprise/namespaces) are inherited by
its child namespaces. To create custom messages specific to a namespace, be
sure to log into your target namespace.
...
To create a custom message intended to the marketing organization, you must be
logged into the `marketing` namespace. 

1. Enter `marketing` in the **Namespace** text field.

1. Select an authentication method and log in. 
