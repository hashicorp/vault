...
- **You must have the appropriate permissions**:
  ...
  - To **create or edit messages**, you must have `read` and `update` permissions
    for the `sys/config/ui/custom-messages/:id` endpoint
  - To **delete messages**, `delete` permission for the
    `sys/config/ui/custom-messages/:id` endpoint.
