...
- **You must have permission to call the `sys/config/ui/custom-message` endpoint**:
  - To **create or edit messages**, you must have the `list` and `create` permissions for the `sys/config/ui/custom-messages` endpoint and the `read` and `update` permissions for the `sys/config/ui/custom-messages/:id` endpoint
  - To **delete messages**, you must have the `list` permission for the `sys/config/ui/custom-messages` endpoint and the `delete` permission for the `sys/config/ui/custom-messages/:id` endpoint.
