...
description: >- 
   Learn about the plugin ecosystem including external vs embedded plugins and
   preregistered plugins.
---
...
An **embedded plugin** is any authentication, secrets, or database plugin built
from code maintained within
[the Vault repository on GitHub](https://github.com/hashicorp/vault).

An **external plugin** is any authentication, secrets, or database plugin built
from code maintained outside the Vault repository on GitHub.
...
Vault ships with a set of preregistered plugins (also called "built-in" plugins)
for commonly used integrations.  You can enable and use any preregistered
plugin without manually registering them, regardless of whether the plugin is
...
Preregistered plugin versions include tags to show whether the plugin was
preregistered as an embedded plugin (`+builtin.vault`) or an external plugin
(`+builtin`).

For example, in the following CLI output:

- `azure` is version 0.20.1 of a preregistered, external plugin.
- `aws` is version 1.18.5 of a preregistered, embedded plugin.
- `mykv` is version 1.0.0 of a manually registered, external plugin.
- `testkv` is an unversioned, manually registered, external plugin.
...
Vault reserves the `+builtin` metadata identifier for preregistered plugins.
You cannot use the `+builtin` string when manually registering a plugin.
