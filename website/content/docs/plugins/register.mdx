...
Vault Enterprise v1.20.0 introduces the `download` **(BETA)** parameter to the
plugin register API. Vault downloads the plugin binary from the
[HashiCorp release page](https://releases.hashicorp.com/) when you set `download`
to `true`. See [`download`](/vault/api-docs/system/plugins-catalog#download) for
more information.
