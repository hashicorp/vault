...
~> Note: Prior to Vault SDK v0.TODO.TODO, Vault will start plugin processes when
needed, typically by lazily loading the plugin when a request that requires
...
~> Note: Prior to Vault SDK v0.TODO.TODO, secrets engines and auth methods make
use of Vault [wrapping tokens](/docs/concepts/response-wrapping) to enable TLS
communication. Vault passes a single use, short lived wrapping token to the
plugin process' environment. Once unwrapped, it provides the plugin with a
uniquely generated TLS certificate and private key for it to use to talk to the
original Vault process.
...
plugins can be made to implement plugin multiplexing. This allows a single
...
plugin, i.e., the plugin calls the `Serve` function. More details on
implementing plugin multiplexing can be found in [Serving a Multiplexed
Plugin](/docs/secrets/databases/custom#serving-a-plugin-with-multiplexing).
