...
the [releases](https://releases.hashicorp.com/) and placed in the plugin
directory.
...
The system running Vault Enterprise is linux/amd64. To register Key Management
secrets engine v0.16.0+ent, vault-plugin-secrets-keymgmt_v0.16.0+ent_linux_amd64.zip
needs to be downloaded and placed in the plugin directory.

<CodeBlockConfig hideClipboard>

  ```shell-session
  $ vault plugin register -version=0.16.0+ent \
  secret \
  vault-plugin-secrets-keymgmt
  ...
  Name                                      Version
  ----                                      -------
  vault-plugin-secrets-keymgmt              v0.16.0+ent
  # ...
  ```
