---
layout: docs
page_title: Containerized Plugins (Beta)
description: Running external plugins for Vault in containers.
---

# Containerized Plugins (Beta)
...
for [`plugin_directory`](/docs/configuration#plugin_directory) is not required
...
1. Register a plugin, specifying your container image as `-oci_image`:
    ```bash
    vault plugin register \
        -sha256="$SHA256" \
...
of plugin containers. If they are killed out of band, Vault will restart it
before servicing any requests that need to be handled by it. Vault will also
[multiplex](/docs/plugins/plugin-architecture#plugin-multiplexing) multiple
...
[`vault plugin runtime`](/docs/commands/plugin/runtime) for more details.
