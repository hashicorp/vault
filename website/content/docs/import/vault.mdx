...
environment variable VAULT_ADDR for destination and IMPORT_SOURCE_VAULT_ADDR for source.
- `namespace` `(string: "")` - Optional namespace to read from. If absent, the CLI uses the
environment variable VAULT_NAMESPACE for destination and IMPORT_SOURCE_VAULT_NAMESPACE for source.
...
environment variable VAULT_TOKEN for destination and IMPORT_SOURCE_VAULT_TOKEN for source.
- `is_hcp_vault_cluster` `(boolean: false)` - Set to true if destination vault belongs to HCP Vault Dedicated. 
If true `namespace` field will be relative to `admin` namespace of HCP Vault Dedicated.
