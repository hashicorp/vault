...
The secrets import feature requires manual activation through a one-time trigger. If a import-related endpoint is called prior to
activation, an error response will be received indicating that the feature has not been activated yet.

1. Activation directly through the UI.

2. Activation through the CLI:

  ```shell-session
  $ vault write -f sys/activation-flags/secrets-import/activate
  ```

3. Activation through a POST or PUT request:
