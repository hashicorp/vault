...
Getting to know Vault is helpful before reviewing the use cases. Listed below are some common uses cases for Vault. Please notes that not all use cases may be listed.
...
Audit logs gather and list all secret accessed by users (employees). Vault's audit logs make it effortless to manage nd roll keys to keep secrets secure.
...
In addition storing secrets, Vault can be used to
...
primary data store. This provides a benefit to developers since the responsibility of encryption lies within the Vault's security team to manage, and thereby, allowing developers to solely focus on encrypting/depcrypting data as needed. 
