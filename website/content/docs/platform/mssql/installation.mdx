...
* Microsoft Windows Server editions for installing Vault EKM provider.
...
1. You can now create an asymmetric key using the transit key set up earlier:
    ...
    !> **Warning:** Encountering the SQL error response `Provider error code: 2050`
    at this stage is typically caused by an incorrect license that's lacking
    "Key Management Transparent Data Encryption". Using the Vault Audit Logs confirm
    that EKM is in network reach to Vault API path `/sys/license/status`. Failing 
    attempts by EKM to connect to Vault in order to check its license or the use 
    of the wrong license are likely causes for the `2050` error response. This 
    would need to be recitified for the above SQL to be successfully before 
    commencing any farther.
