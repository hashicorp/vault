...
* Select your desired auth method from Agent's [supported auth methods](/vault/docs/agentandproxy/autoauth/methods)
...
* Install Vault Agent as a service running `vault agent -config=/path/to/agent.hcl`
  * Documentation for Windows service installation [here](/vault/docs/agentandproxy/agent/winsvc)
