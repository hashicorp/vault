...
!> ServiceNow version UTAH onwards, please use the  "HashiCorp Vault Credential Resolver" App 
from the ServiceNow App store to install the Vault Credential Resolver. Post installation of the app,
ensure that the name of the credential resolver matches the name of the jar,
`vault-servicenow-credential-resolver`.If a custom name is desired, please rename the deployed jar. 
The steps below are for pre ServiceNow UTAH versions
