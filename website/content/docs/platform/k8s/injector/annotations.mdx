...
- `vault.hashicorp.com/service` - configures the Vault address for the injected
  ...
  same Kubernetes cluster as the injector, or an external Vault URL.  Also available as
  an environment variable (`AGENT_INJECT_VAULT_ADDR`) to set the default for all
  injected agents.
