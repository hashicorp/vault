...
- `vault.hashicorp.com/template-config-exit-on-retry-failure` - controls whether
  ...
  Config](/docs/agent/template#exit_on_retry_failure) for more details.
...
- `vault.hashicorp.com/template-static-secret-render-interval` - If specified,
  ...
  See [Vault Agent Template Config](/docs/agent/template#static_secret_render_interval) for more details.
