...
- `vault.hashicorp.com/agent-json-patch` - change the injected agent container using a [JSON patch](https://jsonpatch.com/) before it is created.
  This can be used to add, remove, or modify any attribute of the container.
  For example, setting this to `[{"op": "replace", "path": "/name", "value": "different-name"}]` will result in the agent's container to be named `different-name`
