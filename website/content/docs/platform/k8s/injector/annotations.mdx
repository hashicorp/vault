...
- `vault.hashicorp.com/agent-limits-ephemeral` - configures the ephemeral
  ...
  (`AGENT_INJECT_EPHEMERAL_LIMIT`) to set the default ephemeral storage limit
  for all injected Agent containers.
...
- `vault.hashicorp.com/agent-requests-ephemeral` - configures the ephemeral
  ...
  (`AGENT_INJECT_EPHEMERAL_REQUEST`) to set the default ephemeral storage
  requests for all injected Agent containers.
