...
The Vault Secrets Operator can instantly update Kubernetes Secrets when changes are made in Vault, instead of waiting for the next refresh interval. Requires Vault Enterprise 1.16.3+ due to the use of [Vault Event Notifications][vault-events].
