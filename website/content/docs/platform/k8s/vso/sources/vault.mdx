...
The Vault Secrets Operator (VSO) supports Vault as a secret source. That means that VSO is able to
seamlessly integrate with Vault running on any platform.
...
The following Vault specific features are supported by the Vault Secrets Operator:
