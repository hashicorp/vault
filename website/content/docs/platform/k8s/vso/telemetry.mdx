...
* Ability to deploy a Prometheus Service Monitor, configured via the Helm Chart [`telemetry`](https://developer.hashicorp.com/vault/docs/platform/k8s/vso/helm#h-telemetry) stanza.
* Ability to deploy a Metrics Service, configured via the Helm Chart [`metricsService`](https://developer.hashicorp.com/vault/docs/platform/k8s/vso/helm#h-metricsservice) stanza.
