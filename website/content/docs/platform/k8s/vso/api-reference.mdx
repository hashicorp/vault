...
<!--
  ...
  commit SHA=107235204b2dace4e46f1b7f4b7d02b37457b30e
-->
# API Reference
...
| `requestHTTPMethod` _string_ | RequestHTTPMethod to use when syncing Secrets from Vault. Setting a value here is not typically required. If left unset the Operator will make requests using the GET method. In the case where Params are specified the Operator will use the PUT method. Please consult [docs](/vault/docs/secrets) if you are uncertain about what method to use. Of note, the Vault client treats PUT and POST as being equivalent. The underlying Vault client implementation will always use the PUT method. |
| `path` _string_ | Path in Vault to get the credentials for, and is relative to Mount. Please consult [docs](/vault/docs/secrets) if you are uncertain about what 'path' should be set to. |
| `params` _object (keys:string, values:string)_ | Params that can be passed when requesting credentials/secrets. When Params is set the configured RequestHTTPMethod will be ignored. See RequestHTTPMethod for more details. Please consult [docs](vault/docs/secrets) if you are uncertain about what 'params' should/can be set to. |
...
| `version` _integer_ | Version of the secret to fetch. Only valid for type kv-v2. Corresponds to [version query parameter](/vault/api-docs/secret/kv/kv-v2#version) |
