...
| `namespace` _string_ | Namespace to get the secret from in Vault |
| `mount` _string_ | Mount for the secret in Vault |
| `path` _string_ | Path of the secret in Vault. Corresponds to `path` parameter: [kv-v1](/vault/api-docs/secret/kv/kv-v1#read-secret) and [kv-v2](/vault/api-docs/secret/kv/kv-v2#read-secret-version) |
