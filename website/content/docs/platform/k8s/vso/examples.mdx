...
```shell
demo $ kubectl get --raw "$(kubectl get --raw /.well-known/openid-configuration | jq -r '.jwks_uri' | sed -r 's/.*\.[^/]+(.*)/\1/')" | jq  '.keys | .[0]'
{
  "use": "sig",
  "kty": "RSA",
...
Should the JWT Signing Key used by Kubernetes be rotated, the process should be repeated with the new Key.
