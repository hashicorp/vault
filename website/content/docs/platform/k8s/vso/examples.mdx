...
Vault Secrets Operator supports using the [JWT auth method](/docs/platform/k8s/vso/api-reference#vaultauthconfigjwt).
In the default JWT configuration this involves an API call from the Vault server back to the Kubernetes API server to [verify](/docs/auth/jwt#jwt-verification) the JWT.
It is possible to configure Vault to use [`jwt_validation_pubkeys`](/api-docs/auth/jwt#jwt_validation_pubkeys) fields of the JWT auth configuration
...
PEM file in your `jwt_validation_pubkeys` field when [configuring](/api-docs/auth/jwt#configure) the JWT Auth Method in Vault.
