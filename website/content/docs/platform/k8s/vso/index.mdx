...
In this example, the PKI secret engine is mounted at `pki` path. A Vault role named `default` is configured to map to a procedure for generating a certificate.
...
In this example, the database secret engine is mounted at `db` path. A Vault role named `my-postgresql-role`
is configured to map a name in Vault to an executable database statement to create the database credential.
...
In this example, the AWS secret engine is mounted at `aws` path. A Vault role named `my-role` is configured to map to a set of permissions in AWS as well as an AWS credential type.
