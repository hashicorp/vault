...
The PKI secrets engine's mounted path is specified under `spec.mount` of `VaultPKISecret` custom resource. Please consult [PKI Secrets Engine - Setup and Usage](vault/docs/secrets/pki/setup) for configuring PKI secrets engine.
...
The database secret engine's `db` mounted path is specified under `spec.mount` of `VaultDynamicSecret` custom resource. A Vault role named `my-postgresql-role` is configured to map a name in Vault to an executable database statement to create the database credential. Please consult [Database Secrets Engine - Setup](vault/docs/secrets/databases#setup) for configuring the database secrets engine.
...
The AWS secrets engine's `aws` mounted path is specified under `spec.mount` of `VaultDynamicSecret` custom resource. A Vault role named `my-role` is configured to map to a set of permissions in AWS as well as an AWS credential type. Please consult [AWS Secrets Engine - Setup](vault/docs/secrets/aws#setup) for configuring AWS secrets engine.
