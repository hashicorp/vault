...
- `server` - Values that configure running a Vault server within Kubernetes.
  ...
  - `preStopSleepSeconds` (`int: 5`) - Used to set the sleep time during the preStop step, if custom preStop commands are not set.

  - `preStop` (`array: []`) - Used to define custom preStop exec commands to run before the pod is terminated. If not set, this will default to:
