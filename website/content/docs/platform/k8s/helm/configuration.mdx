...
- `server` - Values that configure running a Vault server within Kubernetes.
  ...
  - `enabled` (`string: "-"`) - When set to `true`, the Vault server will be created. When set to `"-"`, defaults to the value of `global.enabled`.
 ...
 Values that configure the Vault Docker image.
  ...
  - `ingress` - Values that configure Ingress services for Vault. If deployment is on OpenShift, the following block is ignored. In order to expose the service, use the route section below
    ...
    - `labels` - Labels for the ingress service.

    - `annotations` - This value defines additional annotations to add to the Ingress service. This can either be YAML or a YAML-formatted multi-line templated string.
 ...
 ~> If [`ha`](#ha) is enabled the Route will point to the active vault server via the `active` Service (requires vault 1.4+ and [service_registration](/vault/docs/configuration/service-registration/kubernetes) to be set in the vault config).
    ...
    - `labels` - Labels for the Route.

    - `annotations` - Annotations to add to the Route. This can either be YAML or a YAML-formatted multi-line templated string.
 ...
 When set to a value, configures how many probe failures will be tolerated by Kubernetes.
  ...
  - `extraInitContainers` (`array: ""`) - extraInitContainers is a list of init containers. Specified as a YAML list. This is useful if you need to run a script to provision TLS certificates or write out configuration files in a dynamic way.
  ...
  - `extraEnvironmentVars` - The extra environment variables to be applied to the Vault server.
  ...
  - `volumes` (`array: ""`) - A list of volumes made available to all containers. This takes standard Kubernetes volume definitions.
  ...
  - `volumeMounts` - A list of volumes mounts made available to all containers. This takes standard Kubernetes volume mount definitions.
  ...
  - `nodeSelector` - This value defines additional node selection criteria for more control over where the Vault servers are deployed. This should be formatted as a multi-line string or YAML map.
  ...
  - `extraLabels` - This value defines additional labels for server pods.
  ...
  - `annotations` - This value defines additional annotations for server pods. This can either be YAML or a YAML-formatted multi-line templated string.
  ...
  - `service` - Values that configure the Kubernetes service created for Vault. These options are also used for the `active` and `standby` services when [`ha`](#ha) is ## TODO(tomp): What to do with this link? Can we auto-generate it? enabled.
    ...
    - `externalTrafficPolicy` (`string: "Cluster"`) - Sets the type of service to create, such as `NodePort`. type: ClusterIP
 The [externalTrafficPolicy](https://kubernetes.io/docs/concepts/services-networking/service/#external-traffic-policy) can be set to either Cluster or Local and is only valid for LoadBalancer and NodePort service types.
    ...
    - `publishNotReadyAddresses` (`boolean: true`) - When type is set to `NodePort`, the bound node port can be configured using this value. A random port will be assigned if this is left blank. nodePort: 30000
 (When HA mode is enabled) If type is set to "NodePort", a specific nodePort value can be configured for the `active` service, and will be random if left blank. activeNodePort: 30001
 (When HA mode is enabled) If type is set to "NodePort", a specific nodePort value can be configured for the `standby` service, will be random if left blank. standbyNodePort: 30002
 If true, do not wait for server pods to be ready before adding them to the service pool.
    ...
    - `annotations` - This value defines additional annotations for the service. This can either be YAML or a YAML-formatted multi-line templated string.
  ...
  - `serviceAccount` - Values that configure the Kubernetes service account created for Vault.
    ...
    - `annotations` - This value defines additional annotations for the service account. This can either be YAML or a YAML-formatted multi-line templated string.
    ...
    - `extraLabels` - This value defines additional labels for the Vault Server service account.
  ...
  - `dataStorage` - This configures the volume used for storing Vault data when not using external storage such as Consul.
    ...
    - `annotations` - This value defines additional annotations to add to the data PVCs. This can either be YAML or a YAML-formatted multi-line templated string.
  ...
  - `auditStorage` - This configures the volume used for storing Vault's audit logs. See the [Vault documentation](/vault/docs/audit) for more information.
    ...
    - `annotations` - This value defines additional annotations to add to the audit PVCs. This can either be YAML or a YAML-formatted multi-line templated string.
 ...
 This can also be set using Helm's `--set` flag (vault-helm v0.1.0 and later), using the following syntax:
  ...
  - `statefulSet` - This configures settings for the Vault Statefulset.
    ...
    - `annotations` - This value defines additional annotations to add to the Vault statefulset. This can either be YAML or a YAML-formatted multi-line templated string.
    ...
    - `securityContext` - Set the Pod and container security contexts.
      ...
      - `pod` - Defines the securityContext for the server Pods, as YAML or a YAML-formatted multi-line templated string.
 ...
 Defaults to empty if not specified and `global.openshift=true`.
      ...
      - `container` - Defines the securityContext for the server containers, as YAML or a YAML-formatted multi-line templated string.
 Default if not specified and `global.openshift=false`:
