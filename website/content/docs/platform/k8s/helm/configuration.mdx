...
- `server` - Values that configure running a Vault server within Kubernetes.
  ...
  - `extraSecretEnvironmentVars` (`array: []`) - The extra environment variables populated from a secret to be applied to the Vault server. This should be a multi-line key/value string.
