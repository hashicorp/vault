...
- `injector` - Values that configure running a Vault Agent Injector Admission Webhook Controller within Kubernetes.
  ...
  - `replicas` (`int: 1`) - The number of pods to deploy to create a highly available cluster of Vault Agent Injectors. Requires Vault Helm 0.7.0 to have more than 1 replica.

  - `leaderElector` - Values that configure the Vault Agent Injector leader election for HA deployments.

    - `enabled` (`boolean: false`) - When set to `true`, enables leader election for Vault Agent Injector. This is required when using auto-tls and more than 1 replica.
    ...
    - `ttl` - The frequency in which leader elections are performed in the Vault Agent Injector cluster.
  ...
  - `image` - Values that configure the Vault Docker image.
    ...
    * `readinessProbe` - Values that configure the readiness probe for the Vault pods.
    ...
    - `networkPolicy` - Values that configure the Vault Network Policy.
      ...
      - `egress` (`string: null`) - This value configures the [egress](https://kubernetes.io/docs/concepts/services-networking/network-policies/) network policy rules.
