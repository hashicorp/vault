...
!> **IMPORTANT NOTE:** If `ha.raft` enabled with Vault 1.11+, Autopilot has the ability to do
...
the same upgrade procedure below will continue to work as expected.
As such, the Vault team highly recommends using `OnDelete` with Raft Autopilot automated upgrades.
If overriding the strategy type to `RollingUpdate`, then ensure that there are sufficient cluster
resources available for the rolling update or that the pod anti-affinity rules in
`server.affinity` are adjusted.
...
(If using Autopilot Automated Upgrades and the cluster size is at least 3,
then the primary will now be demoted and one of the standby pods will be elected
as the leader.)
