...
After a few moments the Vault cluster should elect a new active primary if it did
not already with the Raft Autopilot Automated Upgrade. The Vault cluster is now
upgraded!

#### Interaction with Raft Autopilot Automated Upgrades

If `server.ha.raft` is enabled with Vault 1.11+, Autopilot has the ability to do
[Automated Upgrades](/docs/concepts/integrated-storage/autopilot#automated-upgrades).
If using the default `server.updateStrategyType` of `OnDelete` for the Vault StatefulSet, then
the same upgrade procedure above will continue to work as expected,
except that the leader will automatically switch over once the majority of pods
are running the newer version of Vault.

!> **IMPORTANT NOTE**: Use `OnDelete` instead of `RollingUpdate` for these automated upgrades,
as `RollingUpdate` will kill the pods in an unpredictable order and could cause the cluster to
upgrade and then downgrade the leader's version, which must be avoided.

If you increase the number of replicas during the upgrade, be sure that **all** pods
are upgraded to the new version before reducing the number of replicas again.
Kubernetes tends to kill newer pods first, potentially leaving only non-voter Vault nodes
running the older version.
This will cause the cluster to be leaderless, and the cluster will be down.

After the upgrade, clean up extra `PersistentVolumeClaims` that resulted from increasing
