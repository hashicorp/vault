...
1. Retrieve the certificate.
   ...
   ```bash
   serverCert=$(kubectl get csr ${CSR_NAME} -o jsonpath='{.spec.request}')
   ```
...
3. Retrieve Kubernetes CA.
   ...
   ```bash
   kubectl config view --raw -o jsonpath='{.clusters[1].cluster.certificate-authority-data}' | base64 -d > ${TMPDIR}/vault.ca
   ```
