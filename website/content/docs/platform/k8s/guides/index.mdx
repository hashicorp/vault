---
layout: docs
page_title: Consul to Raft storage migration
description: >-
  Guide to migration Consul storage to Raft. 
---

# Consul to Raft storage migration
...
This guide uses basic examples and default Vault configurations.  This guide is for illustrative purposes, and adaption to specific configurations relevant to your environment is still required.
...
1. Uninstall Vault via Helm
  ...
  Vault servers using Consul storage are by default stateless. Unless explicitly configured otherwise, the Vault server `StatefulSet` does not create any Persistent Volume Claims (PVC) or other artifacts.  Vault's index holds it's state, which is entirely stored in the Consul server `StatefulSet`'s persistent volumes.
...
1. Install Vault via Helm deployment with Raft Migration storage configuration
  ...
  **Configuration notes**
    - `storage “raft”` configuration to specify the path for the Raft DB (`/vault/data` by default), and any `retry_join` parameters in your original configuration.
      - This example uses `auto_join` to automatically find Raft peers via the Kubernetes API.  See the [`retry_join`](https://developer.hashicorp.com/vault/docs/configuration/storage/raft#retry_join-stanza) for more information.
...
1. Review migration logs and unseal Vault
...
1. Unseal Vault and confirm the Raft peers have formed a quorum.
