...
description: >-
  The Vault CSI Provider allows pods consume Vault secrets using CSI Secret Store
  volumes.
---

# Vault CSI Provider

The Vault CSI Provider allows pods consume Vault secrets using 
[CSI Secret Store](https://github.com/kubernetes-sigs/secrets-store-csi-driver) volumes.
...
At a high level, the CSI Secret Store controller allows users to create `SecretStoreClass` objects. 
This object defines which secret provider to use and what secrets to retrieve. When the provider 
used is `vault`, the Vault CSI Provider will receive the request, use the pods service account to retrieve 
the specified secrets from Vault, and share them with the pod via a host path volume.

The secret is retrieved from Vault and populated to the host path volume during the `ContainerCreation` phase. 
This means that the secrets requested from Vault will be populated in the host path volume before containers 
start running.
...
* All Vault secret engines can be used.
...
The following features are on the roadmap for the Vault CSI Provider:

* Secret renewals and updates
* Templating secret strings
...
The following is an example of a Secret Class Object using the `vault` provider:
...
~> Secret Provier Class is a namespace object in Kuberenetes.

## Using Secret Provider Classes

An application pod uses the example above Secret Provider Class by mounting it as a CSI volume:
