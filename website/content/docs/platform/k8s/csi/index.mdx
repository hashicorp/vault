...
At a high level, the CSI Secret Store controller allows users to create `SecretProviderClass` objects. 
...
the specified secrets from Vault, and <span class="x x-first x-last">mount</span> them <span class="x x-first x-last">into</span> the pod<span class="x x-first x-last">'s CSI</span> volume.
