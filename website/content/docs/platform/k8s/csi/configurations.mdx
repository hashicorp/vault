...
- `vaultTLSClientCertPath` `(string: "")` - The path on disk where the client certificate used 
  by the Vault CSI Provider for mTLS communications with Vault.

- `vaultTLSClientKeyPath` `(string: "")` - The path on disk where the client key used 
  by the Vault CSI Provider for mTLS communications with Vault.
...
- `objects` `(array)` - An array of secrets to retrieve from Vault.
  ...
  - `objectName` `(string: "")` - The alias of the object which can be referenced within the secret provider class.
