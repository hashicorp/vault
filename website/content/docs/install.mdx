...
Since `make` builds in the `bin` subdirectory, Vault should be present in `./bin/vault` 
(assuming you ran `make` in the current directory). If you add that directory to your
`PATH`, or install the Vault binary into your existing `PATH`, the following should work:

```shell-session
$ vault -h
```

But you can also run the following without any `PATH` change:
