...
1. Replace the Vault binary with the new version; ensure that `mlock()` capability is added to the new binary with [setcap](/docs/configuration#disable_mlock)
1. Start the standby node
1. Unseal the standby node
1. Verify `vault status` shows correct Version and HA Mode is `standby`
1. Review the node's logs to ensure successful startup and unseal
...
1. Upgrade Cluster C, which now has an upgraded downstream cluster (Cluster D).
   Because Cluster C is a cluster, it should also use the HA upgrade process.
1. Finally, upgrade Cluster A. All clusters downstream of A will already be
