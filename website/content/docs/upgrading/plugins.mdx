...
2. [Mount][plugin_management] the plugin. Skip this step if your initial plugin
   is already mounted.
...
3. Register a second version of your plugin. You **must** use the same plugin
...
5. If you wish, you can check the updated configuration. Notice the "Version" is
...
6. Finally, trigger a [plugin reload](/docs/commands/plugin/reload) to reload all
...
Until step 6, the mount will still run the first version of `my-secret-plugin`. When
...
4. Update the database config with the new version. The database secrets
...
Until step 4, the mount will still run the first version of `my-db-plugin`. When
