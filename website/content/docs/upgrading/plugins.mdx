...
out if a plugin is built-in we can query it's info:
...
To revert the override, deregister the plugin:

```shell-session
$ vault plugin deregister secret azure
```

Verify the override has been reverted and we are now using the built-in plugin:
```shell-session
$ vault plugin info secret azure
Key        Value
---        -----
...
Note that we still have to reload even when going back to the built-in:
