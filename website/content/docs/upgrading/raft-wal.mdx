...
1. If any issues or errors are encountered, roll back upgraded servers by repeating this process but removing the
   config lines that were added earlier, instead of adding any new lines.
...
If, for some reason, it's desired to move nodes back to raft-boltdb after using raft-wal for some time, one can
