...
When reviewing system logs, consider the entity alias **and** the associated
mount path.
...
Despite having identical, case-insensitive names in the example logs, the four
aliases represent two distinct identities because they come from different authN
mounts:

- `alias-case` and `alias-cAsE` are duplicates from the mount `auth_userpass_34aca7ec`
- `alias-case` and `alias-cAsE` are duplicates from the mount `auth_userpass_a555989a

Always consider the authN mount and namespace of potential duplicates when
grouping potential duplicates for resolution.
