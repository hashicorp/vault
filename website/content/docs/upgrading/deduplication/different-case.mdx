...
But case-sensitive matching 
This mode is not well supported however  
as it deviates from Vault's typical assumptions. Remaining in this mode can lead to unexpected behavior. 
...
difference is a **mergeable duplicate** or **unmergeable duplicate**:
...
- For unmergeable duplicates, the case difference is **deliberate** and the
  ...
  that correspond to different humans with distinct permission sets.


## Case 1: Fixing mergeable duplicates

You can ignore mergeable duplicates since Vault will force-merge them once you
enable forced identity deduplication with the `force-identity-deduplication`
flag. To confirm the merge behavior **before** enabling the flag, review the
`force_deduplication` label in the log line. During deduplication, Vault merges
any policies attached to the duplicates so the final entry has the union of all
permissions granted to the duplicates.
...
## Case 2: Fixing unmergeable duplicates
