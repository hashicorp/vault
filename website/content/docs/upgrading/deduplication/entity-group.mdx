...
- Listing entities/groups by ID returns all duplicates instead of a specific,
  individual identity.

## Example server log

Entities or groups may exist with duplicate names. These might be exact matches,
or may differ in case such as `Admin` and `admin`. For example:
...
Despite having identical, case-insensitive names in the example logs, Vault only
targets 8 for renaming because the names belong to different namespaces:
