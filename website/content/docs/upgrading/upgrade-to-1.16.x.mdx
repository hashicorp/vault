...
The LDAP auth method requires the `userdn` value to be scoped to a path which
includes the OU in which users reside.

The consequence of not configuring this correctly means users will be unable to login.
