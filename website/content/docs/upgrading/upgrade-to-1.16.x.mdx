...
<Warning title="JWT auth login requires bound audiences on the role">

  The bound_audience claim of "jwt" roles **must** match at least one of the
  associated `aud` claims. The `aud` claim can be a single string or a list of
  JSON web tokens that are
