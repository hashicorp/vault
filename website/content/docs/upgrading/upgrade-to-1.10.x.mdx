...
resources will be skipped without an error. We recommend deleting any resources that have
naming collisions with the built-in resources when upgrading from Vault 1.9 to Vault 1.10.
Additionally, we recommend reading the corresponding details in the OIDC provider [concepts](/docs/concepts/oidc-provider)
document to understand the implications of not deleting such resources.
