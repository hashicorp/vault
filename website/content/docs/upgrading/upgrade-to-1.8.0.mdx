...
-> **Note:** Policies containing wildcard should be avoided when giving users read access
...
- The introduction of `/gcp/roleset/:roleset/token` and `/gcp/roleset/:roleset/key` could inadvertently give
  users the ability to generate tokens and key if wildcards are used in policies. To avoid issues like this,
  wildcards should not be used in policies. The following policy grants a user the ability to read all
  rolesets, but would also allow them to generate tokens and keys:
