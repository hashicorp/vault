...
a plugin identity token. The plugin identity token is a JWT that is internally signed by
Vault's identity token provider, and it can be exchanged with the external AWS system to
establish a trust relationship with Vault's identity token provider. This enables a
secretless configuration that obtains ephemeral STS credentials using
[AWS Web Identity Federation](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_providers_oidc.html).

Please see the [API documentation](/vault/api-docs/secret/aws#configure-root-credentials) for more details on
the fields associated with this feature.
...
1.  Alternatively, configure the audience claim value and the role ARN to assume for plugin workload identity federation:
    ...
    This token will be exchanged to establish a trust relationship with AWS and to obtain
    ephemeral STS credentials.

    ~> **Notice:** For this trust relationship to be established, AWS must have WIF
    configured with information about the fully qualified and network-reachable
    Issuer URL for Vault's OIDC Provider. This is to ensure that AWS can fetch
    the JWKS [public keys](/vault/api-docs/secret/identity/tokens#read-active-public-keys)
