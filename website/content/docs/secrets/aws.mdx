...
STS session tokens inherit whatever permissions are granted to the `aws/config/root`
credentials. Then the `root_access` role assigns an inline policy with the same
`ec2:*` permissions.
...
To generate a new set of STS federation token credentials, write to the `root_access`
