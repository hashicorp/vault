...
It's worth noting that the character limit includes the full path to the token; for example
`aws/sts/dev005_vault-test_testtest` exceeds the limit with 34 characters, but `aws/roles/dev005_vaulttest-test` at 31 does not.
