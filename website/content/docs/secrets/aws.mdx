...
1. The user inline policy configured in the Vault role
1. The managed policy ARNs configured in the Vault role
1. An implicit deny policy on IAM or STS operations.
...
1. An IAM role
1. IAM inline policies and/or managed policies attached to the IAM role
1. IAM trust policy attached to the IAM role to grant privileges for Vault to
   assume the role
...
1. Assumed roles can invoke IAM and STS operations, if granted by the role's
   IAM policies.
1. Assumed roles support cross-account authentication
1. Temporary credentials (such as those granted by running Vault on an EC2
