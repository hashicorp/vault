...
1.  Configure the credentials that Vault uses to communicate with AWS to generate
    ...
    In some cases, providing sensitive IAM security credentials to Vault as configuration 
    is not possible. For example, your organization may require that all security 
    credentials are short-lived or tied to a machine identity. We recommend using Vault 
    [plugin workload identity federation](#plugin-workload-identity-federation-wif) (WIF) 
    in this case.

    See the IAM [permissions policy]((#iam-permissions-policy-for-vault)) for Vault section 
    for the permissions the credentials need to have for Vault to perform its actions.
...
Given a trust relationship is configured between Vault and AWS via [Web Identity Federation](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_providers_oidc.html),
the secrets engine can exchange its identity token for short-lived STS credentials needed to
perform its actions. This enables the secrets engine to operate without needing sensitive
IAM security credentials as a part of its configuration.

To configure the secrets engine to use plugin WIF, you must first:
...
1. Ensure that Vault [openid-configuration](/vault/api-docs/secret/identity/tokens#read-plugin-identity-token-issuer-s-openid-configuration)
   ...
   APIs are network-reachable by AWS
   
   An API proxy or gateway can be used to limit Vault API exposure depending on your environment.  

1. Create an [IAM OIDC identity provider](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_providers_create_oidc.html) in AWS
   
   The provider URL must point at Vault's [plugin identity token issuer](/vault/api-docs/secret/identity/tokens#read-plugin-workload-identity-issuer-s-openid-configuration). 
   Remove the `/.well-known/openid-configuration` suffix of the URL and provide it like 
   `https://host:port/v1/identity/oidc/plugins`.
   
   The audience should uniquely identify the recipient of the plugin identity token. In AWS, 
   the recipient is the identity provider. We recommend setting this to the `host:port/v1/identity/oidc/plugins`
   portion of the provider URL since it's enforced as unique per configured identity provider.

1. Create a [web identity role](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_create_for-idp_oidc.html#idp_oidc_Create) in AWS
   
   Select the identity provider and audience created in step 2 when configuring the role.

Next, configure the AWS secrets engine with the audience value and role ARN created from steps 2 and 3:
...
The secrets engine is now configured to use plugin WIF for its configuration credentials. 
These [credentials](https://docs.aws.amazon.com/STS/latest/APIReference/API_AssumeRoleWithWebIdentity.html) 
are short-lived (default 1hr) and will be automatically refreshed through expiration without 
