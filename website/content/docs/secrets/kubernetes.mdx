...
optionally service accounts, role bindings, and roles. The created service account tokens are
time-based and are automatically revoked when the Vault lease expires.

Vault will create a service account token for each lease that provides appropriate access to the
...
The Kubernetes Secrets Engine, like most secrets engines, must be configured in advance before it
can perform its functions. These steps are usually completed by an operator or configuration
management tool.

1. Internally, Vault will connect to Kubernetes using its own service account.
   ...
   service account tokens, and optionally manage service accounts, roles, and role bindings. This
   is usually done with a Kuberentes role or cluster role attached to the Vault service account
   with a role binding or cluster role binding, respectively.
   ...
   You create this role in Kubernetes (e.g., with `kubectl apply -f`).
   Then, you will need to create a role binding to bind the role to Vault's service account to
   grant Vault permission to manage tokens.
...
2. If Vault will not be automatically managing roles or service accounts (see
   [Automatically Managing Roles and Service Accounts](#automatically-managing-roles-and-service-accounts)),
   then we will need to set up a service account that Vault will issue tokens for.
   ...
   We will run everything under the namespace `test`, which we can create if it does not already exist.
   ...
   We can create these objects with `kubectl apply -f`.

3. Enable the Kubernetes Secrets Engine:
...
4. Configure the mount point. An empty config is allowed.
...
5. We can now configure Kubernetes Secrets Engine to create a Vault role (**not** the same as a
...
After a user has authenticated to Vault and has sufficient permissions, a read request to the
...
We can use the service account token above (`eyJHbG...`) with any Kubernetes API request that
its service account is authorized for (through role bindings).

```shell-session
$ curl -sk $(kubectl config view -o 'jsonpath={.clusters[0].cluster.server}')/api/v1/namespaces/test/pods \
...
When the lease expires, we can verify that the token has been revoked.
```shell-session
$ curl -sk $(kubectl config view -o 'jsonpath={.clusters[0].cluster.server}')/api/v1/namespaces/test/pods \
    --header "Authorization: Bearer eyJHbGci0iJSUzI1Ni..."
{
...
You can set a default (`token_defaul_ttl`) and a maximum TTL (`token_max_ttl`) when
...
You can also set a TTL (`ttl`) when generate the token from the credentials endpoint.
...
You can verify the token's TTL by decoding the JWT token and extracting the `exp` field.

```shell-session
$ echo 'eyJhbGc...' | cut -d'.' -f2 | base64 -d  | jq -r '.exp|todateiso8601'
2022-05-20T10:18:05Z
```

## Automatically Managing Roles and Service Accounts

When configuring the Vault role, we can pass in parameters to specify that we want to
automatically generate the Kubernetes service account and role binding,
and optionally generate the Kubernetes role itself.

If we want to configure the Vault role to use a pre-existing Kubernetes role, but generate
the service account and role binding automatically, we can set the `kubernetes_role_name`
parameter.
...
We can then get credentials with the automatically generated service account.
```shell-session
...
```
We can then get credentials in the same way as before.
```shell-session
$ vault read kubernetes/creds/auto-managed-sa-and-role \
    kubernetes_namespace=test
Key                          Value
---                          -----
