...
~> **Note**: As of Vault 1.8.0, tokens and keys can be generated for rolesets using
`/gcp/roleset/:roleset/token` and `/gcp/roleset/:roleset/key`. If wildcards are used
in Vault policies granting access to read rolesets, users may inadvertently be given
access to generate tokens and keys. Wildcards should be avoided when creating policies
for rolesets.
