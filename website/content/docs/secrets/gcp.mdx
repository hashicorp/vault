...
[Google Cloud Service Accounts][service-accounts]. Take note of the email address of the service
...
The credentials given to Vault must have the following permissions when using rolesets:
...
When using static accounts, Vault must have the following permissions:
...
- Create a [custom role][custom-roles] using these permissions, and assign this
  role at a project-level for rolesets or at the service account level when using static accounts.
...
- Vault requires permissions to IAM bindings and service accounts
