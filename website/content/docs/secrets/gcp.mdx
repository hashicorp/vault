...
`/gcp/roleset/:roleset/token` and `/gcp/roleset/:roleset/key`. If globs are used
in Vault policies granting access to read rolesets, users may inadvertently be given
access to generate tokens and keys. Globs should be avoided when creating policies
for rolesets.
