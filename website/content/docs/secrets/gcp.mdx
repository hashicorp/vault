...
Starting with Vault 1.8.0, existing permissive policies containing wildcards/blobs 
for the GCP Secrets Engine may grant additional privileges due to the introduction 
of `/gcp/roleset/:roleset/token` and `/gcp/roleset/:roleset/key` endpoints. If you 
are using the GCP Secrets Engine, and if these permissions contain globs (`*`) we 
recommend you carefully review these policies to ensure they work as expected. 
If possible, consider using more restrictive policies. If you are not using globs (`*`) 
in your GCP Secrets Engine policies, the behavior has not changed. For more more 
information on policy syntax, see the [policy documentation](/docs/concepts/policies#policy-syntax).
