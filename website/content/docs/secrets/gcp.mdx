...
To allow the GCP Secrets engine to impersonate the service account, filter using the the unique `sub` claim issued by plugin identity tokens.
This will be of the form `plugin-identity:<NAMESPACE>:secret:<GCP_SECRETS_MOUNT_ACCESSOR>`.
