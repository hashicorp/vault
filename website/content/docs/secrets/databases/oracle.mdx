...
~> **IMPORTANT:** v0.current has changed the way statements are handled in order to better support additional
environments. See [Revocation Statements](/docs/secrets/databases/oracle#revocation-statements) for more information.
...
1. Configure a role that maps a name in Vault to SQL statements to execute to
   ...
   $ vault write database/roles/my-role \
       ...
       creation_statements='GRANT CREATE SESSION TO "{{username}}"' \
       default_ttl="1h" \
       max_ttl="24h"
...
In order for Vault to delete users within Oracle databases, those users cannot have any open sessions. Prior to v0.current,
the Oracle database plugin attempted to disconnect those open sessions prior to running any revocation statements
(either custom or default statements). This presented a problem because it only supported a particular
subset of Oracle environments. For instance, it did not support dropping users in a RAC cluster or when using
[AWS RDS](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/Appendix.Oracle.CommonDBATasks.System.html#Appendix.Oracle.CommonDBATasks.DisconnectingSession).

In order to support these additional environments, a change was made in v0.current that adds two new feature flags:
`split_statements` and `disconnect_sessions`.

`split_statements` indicates whether the provided statements (all
statements, not just revocation statements) will be split on semi-colons prior to executing them
(`"FOO BAR; BAZ BIZ"` -> `["FOO BAR", "BAZ BIZ"]`. This was originally done so the statements could be specified as
a single string rather than as multiple strings. However, this can prevent the ability to run more complex queries
such as disconnecting all open sessions for a user since the semi-colons may need to be preserved.

`disconnect_sessions` indicates whether the default disconnect sessions logic should be run prior to executing the
revocation statements. The current behavior may not work for your environment and may need to be customized.

**IMPORTANT NOTE:** Both of these flags are temporary and will be removed in v0.next in order to require explicit
session disconnection logic within the revocation statements. Revocation statements will have a reasonable default,
however that may or may not work for your particular environment.

See the [API](/api/secret/databases/oracle#revocation-statements) for migration steps and general recommendations
