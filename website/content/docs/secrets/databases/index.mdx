...
1. From Secret Engines List, click on **database/**
1. Click on **Connections** tab and click **Create connection +**
1. Click on Database plugin dropdown and select desired plugin ie. **postgreSQL**
...
After configuring the root user, it is highly recommended you rotate that user's
...
1. In the **Connection name** dropdown, choose the desired DB connection for this role to be created in
1. In the **Type of role** dropdown, choose between creating a static or dynamic role
...
1. Manage desired role's TTL and Max TTL expiration or keep role roleDefaultState
1. Enter items as desired in statements fields or leave blank
...
1. Enter a username from the database that this role will be mapped to
1. Enter a password for this role if desired or leave blank
1. Specify the **Rotation period of this role**, or keep the default of 24 hours
1. Toggle whether or not to rotate this roles password immediately or to bypass the first rotation
1. Click **Create role** to create the role
...
Setting the password on the static role creation page and toggling **Rotate immediately** off will give you the ability to retrieve the static user's existing password after saving.
