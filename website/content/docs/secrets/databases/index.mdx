...
1. Configure Vault with the proper plugin and connection information.
  ...
  The recommendation is to create a user within the database specifically for
  Vault to use. You should use this user to manipulate dynamic and static users
  within the database. The documentation refers this user as the "root" user.
...
1. Click **Enable engine**.
  ![Partial screenshot of the Vault GUI showing the "Enable database" page](/img/gui/databases/enableDatabase.png) 
   By default, the secrets engine will enable at the name of the engine. To enable the secrets engine at a different path, enter the desired path in the **Path** field.
1. Click **Save** to enable the plugin.
1. Click **Create connection +** from the **Connections** tab to configure the database connection.
  ![Partial screenshot of the Vault GUI showing the "Create connection" page](/img/gui/databases/configureDBConnection.png)
1. Click on Database plugin dropdown and select desired plugin, for example **postgreSQL**.
  ![Partial screenshot of the Vault GUI showing the populated "Plugin config" section](/img/gui/databases/pluginConfig.png)
1. Fill in the chosen plugin's specific fields.
1. Click **Create database** to create the database connection.
1. Click on **Roles** tab and click **Create role +**
  ![Screenshot of the Vault GUI showing the "Create Role" page](/img/gui/databases/roleDefaultState.png)
1. Enter desired name of role in **Role name** field.
1. In the **Connection name** drop-down, choose the desired DB connection to create this role in.
1. In the **Type of role** drop-down, choose between creating a static or dynamic role.

<Tabs>
<Tab heading="Dynamic role">

Enter the role settings:
- Role's TTL and Max TTL expiration if different from the defaults
- Populate the statements fields or leave blank

![Partial screenshot of the Vault GUI showing the "Role settings" section for a Dynamic role](/img/gui/databases/roleDynamic.png)

Click **Create role** to create the role.

</Tab>
<Tab heading="Static role">

Enter the role settings:
- A username from the database that this role will be mapped to
- A password for this role if desired or leave blank
- Specify the **Rotation period of this role**, or keep the default of 24 hours
- Toggle whether or not to rotate this roles password immediately or to bypass the first rotation

![Partial screenshot of the Vault GUI showing the "Role settings" section for a static role](/img/gui/databases/roleStatic.png)
...
On the DB connection creation page, this toggle button will set the default value for static roles whether to rotate immediately or to skip the initial rotation if toggled off.

![Partial screenshot of the Vault GUI showing the role level "Rotate immediately" toggle](/img/gui/databases/roleRotation.png)
On the static role creation page, this toggle button will control whether this individual role password will be rotated immediately or skipped if toggled off.

![Partial screenshot of the Vault GUI showing a provided password and toggled off Rotate immediately input](/img/gui/databases/rolePasswordNonRotate.png)

To set the initial password during onboarding:

On the static role creation page provide a password and toggle **Rotate immediately** off before saving.
