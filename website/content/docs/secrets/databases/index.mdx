...
3. After configuration, we strongly recommend rotating the root user password such that the vault user is not accessible by any users other than
   Vault itself:
...
4. Configure a role that maps a name in Vault to a set of creation statements to
