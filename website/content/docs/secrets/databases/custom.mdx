...
1. [Password policies](/docs/concepts/password-policies) introduced in Vault 1.5 required
   ...
   password policies.
1. Passwords needed to be generated by database plugins. This meant that plugin authors
...
1. There were a number of inconsistencies within the version 4 interface that made it
...
1. The `SetCredentials` and `RotateRootCredentials` used for static credential rotation,
...
1. The old interface included both `Init` and `Initialize` adding to the confusion.
...
1. Version 4 has the following functions: `Initialize`, `Init`, `CreateUser`, `RenewUser`,
...
1. Version 5 has the following functions: `Initialize`, `NewUser`, `UpdateUser`, `DeleteUser`,
...
1. An easy way to see what functions need to be implemented is to put the following as a
...
1. Replace `Init` and `Initialize` with the new `Initialize` function definition. The fields that
...
1. Update `CreateUser` to `NewUser`. The `NewUserRequest` object contains the username and
...
1. `SetCredentials`, `RotateRootCredentials`, and `RenewUser` are combined into `UpdateUser`.
...
1. Update `RevokeUser` to `DeleteUser`. This is the simplest change. The username to be
