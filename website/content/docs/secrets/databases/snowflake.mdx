...
~> **Note:** The user being provided to Vault should have `ACCOUNT_ADMIN` privileges, and should be different
from the root user provided when making the Snowflake account and from user accounts. This allows Vault
to rotate its root credentials while users still have continued access to their accounts.
