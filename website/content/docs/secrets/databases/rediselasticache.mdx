...
~> **Note**: the username and password parameters are optional. If omitted, authentication fallbacks on the AWS credentials provider chain.
...
After the secrets engine is configured, configure static roles to enable generating credentials.
...
~> **Note**: New passwords may take up to a couple of minutes before ElastiCache has the chance to complete their configuration.
  It is recommended to use a retry strategy when establishing new Redis ElastiCache connections to prevent errors from trying to use
  a password that is yet to be live on the targeted ElastiCache cluster.
