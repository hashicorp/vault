...
Certificate Authority (CA). This CA is used when client certificates are generated  
for authenticating with Vault using the KMIP protocol. This CA is always generated 
by Vault and is not able to be imported to Vault. The CA's only purpose is for 
authenticating KMIP Client certificates.
