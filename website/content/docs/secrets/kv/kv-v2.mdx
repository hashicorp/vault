...
3. Write another version, the previous version will still be accessible. The
...
5. Partial updates can be accomplished using the `vault kv patch` command. A
...
6. The `vault kv patch` command also supports a `-method` flag which can be
...
7. Reading after a patch will return the newest version of the data in which
...
8. Previous versions can be accessed with the `-version` flag:
