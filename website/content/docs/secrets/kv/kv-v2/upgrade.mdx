...
1. Create a payload file for the request body to set `version`:
    ```json
    {
      "options": {
          "version": "2"
...
2. Call the [TuneMountConfiguration](/vault/api-docs/system/mounts#tune-mount-configuration)
   endpoint with the payload file:
    ```shell-session
    $ curl                            \
