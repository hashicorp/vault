...
Write custom metadata to a `kv` v2 secret path for ???.

<Tip title="Assumptions">

- You have [set up a `kv` v2 plugin](/vault/docs/secrets/kv/kv-v2/setup). 
- Your authentication token has `metadata` permissions for the `kv` v2 plugin.
...
```shell-session
$ vault kv metadata put                                              \
   -custom-metadata "desc=API keys for different dev environments"   \
...
current_version         9
custom_metadata         map[desc:API keys for different dev environments renew-date:2026-11-14]
...
{
  "custom_metadata": {
    "desc": "API keys for different dev environments",
