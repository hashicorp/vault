...
description: >-
  The Active Directory secrets engine allowing Vault to generate dynamic
  credentials.
---
...
1. `length` and `formatter` fields within the [configuration](/api-docs/secret/ad#password-parameters)
...
1. Configure the credentials that Vault uses to communicate with Active Directory
...
1. Configure a role that maps a name in Vault to an account in Active Directory.
...
1. Grant "my-application" access to its creds at `ad/creds/my-application` using an
