...
1.  Configure Vault to connect and authenticate to Consul.
    ...
    Vault can bootstrap the Consul ACL system for you if it hasn't already been done. If you
    have already bootstrapped the Consul ACL system, then you will need to provide
    ...
    1.  Configuring Vault without previously bootstrapping the Consul ACL system:
        ...
        ~> Note: When you let Vault perform the bootstrap for you, it will silently store the bootstrap token as the
        configuration token; it will not be presented to the user. If you need another management token,
        you will need to generate one by writing a Vault role with the `global-management` policy and
        then reading new creds back from it.
