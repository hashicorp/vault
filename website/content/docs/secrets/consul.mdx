...
2.  Bootstrap the Consul ACL system if not already done. To begin configuring the secrets engine, we must give Vault
...
5.  For Enterprise users, you may further limit a role's access by adding the optional parameters `consul_namespace` and/or
