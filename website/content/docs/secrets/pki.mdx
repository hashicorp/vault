...
Unlike many secrets engines which replicate their state to all nodes
in enterprise replicated clusters, the PKI secrets engine stores issued certificates (not
root/intermediate CA certificates) local to the node that issued them. 
This allows for both active and performance secondary nodes to issue
certificates for greater scalability.  As a result, these certificates
are visible only on the issuing node.
