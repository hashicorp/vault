...
do a statistical analysis of the tokens as they related to some other field
...
one can create a tokenization transform that is *convergent*.  

When enabled at transform creation time, Vault alters the calculation so that
...
use cases that require convergence and some that do not, create two
different tokenization transforms with convergence enabled on only one. 

### Token Lookup

Some use cases may want to lookup the value of a token given plaintext.  Ordinarily
...
transform.  Token lookup is supported when convergence is enabled, or
