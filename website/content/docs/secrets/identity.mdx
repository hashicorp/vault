...
to form the alias name that is used to match or create an entity. That is,
for each entry in the table, there will by default be one entity per object
specified in the right-hand side.

| Auth method | Name reported by auth method (1 entity per *) |
| ----------- | --------------------------------------------- |
