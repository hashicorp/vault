...
It is possible for a PKI mount to use different static challenge values from a PKI mount. The SCEP auth
mount will need multiple roles configured of `auth_type` `static-challenge`, each with a different `challenge` value.
Within the PKI mount's SCEP configuration, the `scep_role` field will need to be left blank. With that the login
attempt to attempt the various SCEP auth roles.
