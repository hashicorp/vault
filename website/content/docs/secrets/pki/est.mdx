...
A number of steps needs to occur to properly configure an existing PKI mount
to serve EST clients. Each of which can be broken down within two main
categories.
...
EST protocol specifies a few different authentication mechanisms, of which
Vault supports two.
...
along with providing the client's ACL policy to enforce. While both authentication
schemes can be enabled at once, only a single mount will be used to authenticate
a client based on what credentials a client provides. If an EST client sends
HTTP-Based authentication credentials, they will be preferred over TLS client
certificates.

When setting up the authentication mounts for EST clients, the token type must
be configured to return [batch tokens](/vault/docs/concepts/tokens#batch-tokens). They are required to avoid lease
explosions as every EST incoming request needs to be authenticated.

For ACL policies, the path's should list the internal redirected path including
the mount and not the `.well-known/est/` path space. The path to use within the
plugin depends on the path policy that is in configured.

If using the `sign-verbatim` as a path policy within the EST configuration, the following
ACL policy will allow an authenticated client access the required PKI paths.
```
...
For a role base path policy, this sample can be used
```
...
For EST to work within certain clients, a few response headers need to be permitted along with allowing
the mount to delegate authentication. The following will grant the required
response headers, you will need to replace the values for the `delegated-auth-accessors`
...
EST client's won't provide any sort of configuration
for paths, and will default to hitting the host on the path
`https://<hostname>:<port>/.well-known/est/`.
...
to register the .well-known/est default path, along with two est labels, test-label and sign-all.
The test-label would use existing est-clients PKI role for restrictions and defaults,
also specifying the issuer to use. The other two labels, default and sign-all will
...
paths within the .well-known path space. The following limitations apply:
 ...
 - Care must be taken if enabling EST on a [local](/vault/docs/commands/secrets/enable#local) PKI mount on
   ...
   on the local performance secondary might point to a different mount across servers in different clusters.
