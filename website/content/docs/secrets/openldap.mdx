...
description: The OpenLDAP secret engine manages OpenLDAP entry passwords.
---
...
1. Configure the credentials that Vault uses to communicate with OpenLDAP
...
1. Rotate the root password so only Vault knows the credentials:
...
1. Request credentials for the "hashicorp" role:
