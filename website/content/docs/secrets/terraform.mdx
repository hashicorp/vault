...
The HCP Terraform API limits Organization to **one active
token at any given time**. Generating a new Organization API token by
...
existing API token for that Organization.
...
taken according to ttl & max_ttl settings. In addition to that behavior,
if `max_ttl` is set, Vault will set an `ExpiredAt` date in HCP Terraform also
which will ensure the token expires at the Max TTL time. This prevents Team tokens
living past their max ttl if Vault is unable to revoke the token. Omitting a 
max_ttl will create a team token with no expiration date.

In HCP Terraform, team tokens cannot have matching descriptions. In order to avoid
collissions, we generate a random string as a suffix to the description. It is
