...
also which will ensure the token expires at the Max TTL time. This prevents 
Team tokens living past their max ttl if Vault is unable to revoke the token.
Omitting a max_ttl will Vault's system max_ttl
