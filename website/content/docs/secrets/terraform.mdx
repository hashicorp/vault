---
layout: docs
page_title: HCP Terraform Secret Backend
description: The HCP Terraform (previously Terraform Cloud) secrets engine for Vault generates tokens for HCP Terraform dynamically.
---

# HCP Terraform secret backend

Name: `HCP Terraform`

The HCP Terraform (previously known as Terraform Cloud) secrets engine for Vault
generates [HCP Terraform](https://cloud.hashicorp.com/products/terraform) API
tokens dynamically for Organizations, Teams, and Users.
...
for creating dynamic users. Instead, the HCP Terraform secrets engine creates
