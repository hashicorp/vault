...
<Note title="Lazy rotation vs automatic rotation">
  ...
  During a migration to the LDAP secret engine, this difference in behaviour
  may need to be handled by clients.  For example, where a client assumes the
  password does not change until its next request to Vault.
