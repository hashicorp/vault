...
##### Lazy vs Auto password rotation

~> **Note**: The AD secrets engine "lazy rotates" the passwords on the condition that both
the rotation period has been exceeded _AND_ the engine is receives a request for the role.
In contrast, the LDAP secret engine automatically rotates the passwords when the
`rotation_period` is met (client requests not required).  This difference in behaviour might
need to be taken into account by consuming clients.
