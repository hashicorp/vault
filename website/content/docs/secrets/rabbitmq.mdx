...
1.  Configure a role that maps a name in Vault to virtual host permissions:
  ...
  Alternatively, if you require a static username, configure a static role
  that maps a name in Vault to virtual host permissions:

    ```text
    $ vault write rabbitmq/static-roles/my-static-role \
        username="vhost" \
        rotation_period=300 \
        vhosts='{"/":{"write": ".*", "read": ".*"}}'
    Success! Data written to: rabbitmq/static-roles/my-static-role
    ```

    By writing to the `static-roles/my-static-role` path we are defining the
    `my-static-role` static role. The way this role is created is similar to
    the role in the example above. However, it will always keep `foobar` as a
    username, and automatically rotate the password every 300 seconds. Hence
    it will not create a new password every time credentials are requested.
...
1.  Alternatively, request credentials for a static role by reading from the
    `/static-creds` endpoint with hte name of the static role:
    of the role:

    ```text
    $ vault read rabbitmq/static-creds/my-static-role
    Key                    Value
    ---                    -----
    last_vault_rotation    2023-07-10T13:52:51.007373395Z
    password               5KZYIzbodxzyE5t1CzpPNeTe9t4gz9Wdx023
    rotation_period        5m
    ttl                    4m54s
    username               foobar
    ```
