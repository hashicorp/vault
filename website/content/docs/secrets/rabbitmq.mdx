...
Writing to the `static-roles/my-static-role` defines a static role called
`my-static-role`. The creation process is similar to the previous example,
but the role keeps `vhost` as a username and automatically rotate the
password every 300 seconds rather than creating a new password every time
Vault receives a credential request.
...
1.  Alternatively, request credentials for a static role by reading from the
    `/static-creds` endpoint with the name of the static role:
    of the role:
