...
@include 'alerts/enterprise-and-hcp.mdc'
...
1. Set the mount path and namespace for the SAML plugin.
...
## Step 2: Create a new relying party trust in ADFS
...
1. Follow the prompts to create a new party trust:
  ...
  1. Check **Claims aware** and select **Start**.
  1. Check **Enter data about relying party manually** and select **Next**.
  1. Set **Display name** to "Vault" and select **Next**.
  1. Select **Next** without adding a certificate.
  1. Check **Enable support for the SAML 2.0 WebSSO protocol**.
  1. Enter the SAML callback endpoint of your Vault instance:
      `https://<vault-address>/v1/<namespace/><mount-path>/auth/saml/callback`. For example,
      if you mounted the plugin under the `ns_admin` namespace on the path
      `org/security`, the callback endpoint URL would be
      `https://myvault.example.com:8200/v1/ns_admin/auth/org/security/saml/callback`
  1. Provide a one-word name for **Relying party trust identifier**. For example,
     "MyVaultIdentifier". **You need the identifier to finish configuring the
     SAML plugin**.
  1. Select the desired policy (or `Permit everyone`) for the **Access control policy**.
  1. Review the information and click **Next**.
  1. Check **Configure claims issuance policy for this application** and click
     **Close**.
...
1. Click **Add Rule...** and follow the prompts to create a new **Transform
   ...
   1. Set **Rule name** to a meaningful string. For example, "Vault SAML Claims"
   1. Set **Attribute store** to `Active Directory`.
   1. Complete the settings array as follows:
...
## Step 4: Modify the SAML signature in ADFS
...
1. Set the SAML signature for your relying party trust identifier to `false`:
   ...
   For example: NOT SURE ABOUT THIS!!!!
...
the configuration of the SAML plugin with the Vault CLI:

```shell-session
$ vault write                           \
  auth/saml/config                      \
  default_role="<adfs_role_name>"       \
  entity_id="<relying_party_trust_ID>"  \
  acs_urls="<saml_callback_url>"        \
  idp_metadata_url="<adfs_instance_url>/<federation_metadata_file>"
```
...
If you want to use `Group` for your token group setting instead of `groups`, you
must use `http://schemas.xmlsoap.org/claims/Group` instead of `groups` **in Step 6**.
