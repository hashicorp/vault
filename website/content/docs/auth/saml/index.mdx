...
1. Use the `/config` endpoint to save the configuration of your SAML identity provider and
   ...
   ```
<Note>
  The config has boolean options to validate SAML response signature, as `validate_response_and_assertion_signatures`
  , `validate_response_signature` and `validate_assertion_signature`. By default, its validated that at least one out of
  response or assertion is signed. If your IDP allows signing both then recommended secure approach is to enable
  `validate_response_and_assertion_signatures` option.
</Note>
