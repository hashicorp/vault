...
The Azure auth engine supports configuring schedule-based automatic
credential rotation for the root credential with the
[rotation_schedule](/vault/api-docs/auth/azure#rotation_schedule) field.
For example:
...
[rotation_window](/vault/api-docs/auth/azure#rotation_window) in which
...
Root rotation can be temporarily disabled by setting [disable_automated_rotation](/vault/api-docs/auth/azure#disable_automated_rotation)
