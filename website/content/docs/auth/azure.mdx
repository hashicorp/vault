...
1.  Alternatively, configure the audience claim value and the Client, Tenant IDs for plugin workload identity federation:
   ...
   Given a trust relationship is configured between Vault and Azure via
   Workload Identity Federation, the auth engine can exchange this identity token to obtain
   federated access tokens.

   ~> **Notice:** For this trust relationship to be established, Azure must have an
   a [Federated Identity Credential](https://learn.microsoft.com/en-us/entra/workload-id/workload-identity-federation-create-trust?pivots=identity-wif-apps-methods-azp#configure-a-federated-identity-credential-on-an-app)
   ...
   This is to ensure that Azure can fetch the JWKS [public keys](/vault/api-docs/secret/identity/tokens#read-active-public-keys)
...
The Azure auth engine supports the Plugin WIF workflow, and has a source of identity called
...
Exchanging identity tokens for access tokens lets the Azure auth engine
...
1. Configure a
   ...
   This will be of the form `plugin-identity:<NAMESPACE>:auth:<AZURE_MOUNT_ACCESSOR>`.
   1. The audience should be under 600 characters. The default value in Azure is `api://AzureADTokenExchange`.

1. Configure the Azure auth engine with the OIDC audience value and the Client, Tenant IDs.
