...
From Vault 1.13.0, an operator can configure a name for an MFA method. This name should be unique in the namespace in which the MFA method is configured.
Then, one can use the MFA method name instead of an MFA method ID in the MFA header.
