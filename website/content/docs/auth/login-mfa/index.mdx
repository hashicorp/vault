...
Configure the Login MFA TOTP method and assign its method ID to the `TOTP_METHOD_ID` environment variable.
...
Extract the TOTP `method_id` and after a sucessful MFA login authentication extract the `entity_id`.  Use these to generate a QR code.
...
```shell-session
$ VAULT_TOKEN=root vault auth list -format=json \
