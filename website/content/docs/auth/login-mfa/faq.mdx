...
- [Q: I am a Vault Enterprise MFA user using MFA for login. Should I migrate to the new Login MFA?](#q-i-am-a-vault-enterprise-mfa-user-using-mfa-for-login-should-i-migrate-to-the-new-login-mfa)
- [Q: What are the steps to migrate to Login MFA?](#q-what-are-the-steps-to-migrate-to-login-mfa)
...
~> **Note**: [The Legacy MFA](/docs/auth/mfa) is a **deprecated** MFA workflow in Vault OSS. Refer to the [Q: What is the Legacy MFA feature?](#q-what-is-the-legacy-mfa-feature) for more details.
...
The Step-up Enterprise MFA uses the combination of mount accessors plus a `username_format` template string, whereas in Login MFA, these are combined into a single field `username_template`, which uses the same identity [templating format](/docs/concepts/policies#templated-policies) as used in policies.
...
The Step-up Enterprise MFA is not namespace-aware; MFA methods cannot be configured at a namespace level.
However, the Login MFA supports namespaces. Users will need a Vault Enterprise license to use and configure namespaces with Login MFA. MFA method configurations can be defined per namespace with Login MFA, and used in enforcements defined in that namespace and its children. Everything operates in the root namespace in Vault OSS. MFA login enforcements can also be defined per namespace, and applied to that namespace and its children.
...
### Q: I am a Vault Enterprise MFA user using MFA for login. Should I migrate to the new Login MFA?
...
- We plan to remove Legacy MFA in Vault version 1.11.
...
### Q: What are the steps to migrate to Login MFA?

If you are a Vault Enterprise MFA user and wish to migrate to Login MFA, follow these steps and guidelines to migrate successfully.
...
1. First, create new MFA methods using the `identity/mfa/method` endpoints. These should mostly use the same fields as the MFA methods you defined using the `sys/mfa` method while keeping the following in mind:
   ...
   -the new non-TOTP endpoints have a user_name field instead of username_format+mount_accessor fields; see [Templated Policies](/docs/concepts/policies#templated-policies) for the username_template format.
