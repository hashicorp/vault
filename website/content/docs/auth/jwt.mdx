...
- `mode` (default: "browser"). This value is used to toggle between automatic and manual OIDC authentication.
   In `manual` mode, the user's browser is not automatically launched, rather the user copies the URL and
   completes browser authentication out of band. Upon successful authentication, the user is presented
   with a code that is copied and manually supplied to the authenticating client. `manual` mode requires
   that the role being used has the `oidc/manual` path listed in the role's `allowed_redirect_uris`. For example:
   `https://vault.server:8200/v1/oidc/oidc/manual`.

Example of manual mode:

```shell-session
$ vault login -method=oidc mode=manual callbackmethod=https
