...
```shell-session
# This for authentication to vault outside of Google Cloud
...
```shell-session
# This for authentication to vault inside of Google Cloud.
$ vault login -method=gcp \
    role="my-role" \
```
...
infrastructure running on Google Cloud**. These steps will not work from your local laptop or
...
You can autogenerate this token in Vault versions newer than 1.8.2.
