...
1. Alternatively configure the audience claim value and the service account email to assume for plugin workload identity federation:
   ...
   Given a trust relationship is configured between Vault and GCP via
   Web Identity Federation, the auth engine can exchange this identity token to obtain
   [federated access tokens](https://cloud.google.com/docs/authentication/token-types#access).

   ~> **Notice:** For this trust relationship to be established, GCP must have
   a [Workload Identity Pool and Provider](https://cloud.google.com/iam/docs/manage-workload-identity-pools-providers)
   ...
   This is to ensure that GCP can fetch the JWKS [public keys](/vault/api-docs/secret/identity/tokens#read-active-public-keys)
...
The GCP auth engine supports the Plugin WIF workflow, and has a source of identity called
...
the auth engine can exchange its identity token for short-lived access tokens needed to
perform its actions.

Exchanging identity tokens for access tokens lets the GCP auth engine
...
1. Create a
    ...
    token. This can be the [default audience](https://cloud.google.com/iam/docs/workload-identity-federation-with-other-providers#prepare)
...
To allow the GCP Auth engine to impersonate the service account, filter using the the unique `sub` claim issued by plugin identity tokens.
This will be of the form `plugin-identity:<NAMESPACE>:auth:<GCP_AUTH_MOUNT_ACCESSOR>`.

1. Configure the GCP auth engine with the OIDC audience value and service account
email.
