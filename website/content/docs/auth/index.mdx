...
at the time of authentication and for any subsequent token renewals.  This means that when the status of an
entity is changed in an external system (e.g. an account expires or is disabled), Vault will deny requests to
renew tokens already issued for that entity, but existing tokens will remain vaild for the entire duration for
which they were originally granted unless they are expicitly revoked within Vault.
Operators should ensure appropriate [token TTLs](/vault/docs/concepts/tokens#the-general-case)
