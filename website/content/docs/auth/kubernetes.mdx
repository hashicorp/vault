...
When configuring Kubernetes auth, you can set `use_annotations_as_alias_metadata=true`
to use Kubernetes Service Account annotations for
[Vault alias](vault/docs/concepts/identity#entities-and-aliases) metadata. This will
allow you to utilize the `identity.entity.aliases.<mount accessor>.metadata.<metadata key>`
templating parameter in creating
[templated policies](vault/docs/concepts/policies#templated-policies). Note that
Vault will need permission to read service accounts from the Kubernetes API.

#### Scenario Introduction
Assume that the following policy requirement was given:

Each application can perform read operations on its allocated key/value secret path
`(env-kv/data/<env>)`
...
The annotation will be rendered to the alias metadata as `env : demo/app`
upon Vault login using `app` ServiceAccount's JWT.

#### Create a templated ACL policy

The `env-tmpl` policy allows applications to read their secret that
are defined in KV v2 secret engine. The mount accessor value (auth_kubernetes_bcecb1e1
in this example) can be read from the sys/auth endpoint.
...
This allows users that login as the `env-reader` role to read at the secret path
described in the `env-tmpl` policy.

```bash
vault write auth/kubernetes/role/env-reader bound_service_account_names=app
bound_service_account_namespaces=demo policies=default,env-tmpl ttl=1h
```
