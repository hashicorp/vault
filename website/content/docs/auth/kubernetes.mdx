...
#### Use the system's trust store for CA certificates

Omitting `kubernetes_ca_cert` and setting `disable_local_ca_jwt` to `true` will lead to Vault
using the system's trust store for TLS certificate verification.
