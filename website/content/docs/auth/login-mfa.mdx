...
There are two ways to validate a login request that is subject to MFA validation.
 ...
 that identify which method IDs need to be used to validate the login request. MFA constraints
...
#### Sample CLI Request
A user can use the CLI write command to validate a Login MFA.
