...
There are two ways to validate a login request that is subject to MFA validation.
 ...
 and represent all MFA enforcements that match a login request.
...
```
 ...
 To validate the MFA restricted login request, the user sends a second request to the [validate](/sys/mfa/validate)
