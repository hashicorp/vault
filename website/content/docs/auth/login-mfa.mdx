...
Note that both MFA methods like (Okta, Duo, and PingID) and MFA enforcement configuration
could use mount accessors. However, there is no particular correlation between the two accessors. In other words, the
mount accessors in an MFA method config don't need to match mount accessors in an MFA enforcement config.
