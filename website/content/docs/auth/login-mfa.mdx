...
this feature and the MFA feature used strictly in Vault Enterprise.
...
on how to configure an MFA method. Once an MFA method is configured, using the returned unique MFA method identification,
...
either of the MFA enforcement restrictions is subject to further MFA validation,
...
There are two ways to validate a login request that is subject to MFA validation.
 ...
 the header is `mfa_method_id[:key[=value]]`. The item in the `[]` is optional. If there are more than
 ...
 ## Two-phase Login
 The more conventional, and prevalent MFA method is a two request mechanism, also referred to as the Two-phase Login MFA.
 In Two-phase login the `X-Vault-MFA` header is provided in the request. In this case, after sending a regular login request,
 the user receives an auth response which includes the MFA requirements. MFA requirements contain an MFA request ID
 which identifies the login request that needs to get validated. In addition, the MFA requirement contains MFA constraints
 that identifies which method IDs need to be used to validate the login request. MFA constraints
 form a nested map in MFA Requirement, and represent all MFA enforcements that match a login request.
 Please see [Login MFA Enforcement API](/api/secret/identity/mfa/login-enforcement)
 for details on the parameters of an MFA enforcement configuration.
...
```
 ...
 To validate the MFA restricted login request, the user sends a second request to `"/sys/mfa/validate"` [API endpoint](/sys/mfa/validate)
 including the MFA request ID, and MFA payload. MFA payload contains a map of methodIDs and their associated credentials.
 If the configured MFA methods (like PingID, Okta, Duo) don't need a passcode, the associated
 credentials will be a list of an empty string.
