...
1. Vault validates that the signature is no more than 300 seconds old, or 60 seconds in the future.
1. Vault validates that the cert was issued by the CA certificate you've pre-configured.
1. Vault validates that the request was signed by the private key for the `CF_INSTANCE_CERT`.
1. Vault validates that the `CF_INSTANCE_CERT` application ID, space ID, and org ID presently exist.
1. If all checks pass, Vault issues an appropriately-scoped token.
