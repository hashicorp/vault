...
- `rotation_schedule` (string, optional) - The schedule, in [cron-style time format](https://en.wikipedia.org/wiki/Cron), defining the schedule on which Vault should rotate the root token. See [rotation_schedule](/vault/api-docs/auth/ldap#rotation_schedule) and.
- `rotation_window` (string, optional) - The maximum amount of time, in [duration format](https://pkg.go.dev/time#ParseDuration), allowed to complete a rotation when a scheduled token rotation occurs. See [rotation_window](/vault/api-docs/auth/ldap#rotation_window).
- `rotation_period` (string, optional) - The amount of time, in [duration format](https://pkg.go.dev/time#ParseDuration), Vault should wait before rotating the root credential. See [rotation_period](/vault/api-docs/auth/ldap#rotation_period).
...
The LDAP auth engine supports configuring schedule-based automatic
credential rotation for the root credential with the
[rotation_schedule](/vault/api-docs/auth/ldap#rotation_schedule) field.
For example:
...
[rotation_window](/vault/api-docs/auth/ldap#rotation_window) in which
...
Root rotation can be temporarily disabled by setting [disable_automated_rotation](/vault/api-docs/auth/ldap#disable_automated_rotation)
to `true`. This will prevent any rotation of the root credential until the field is set to `false`. Note: This will reset
the TTL if `rotation_period` is being used.
