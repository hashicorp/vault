...
1. Follow the prompts to create a new application group with the following
   information: 
    ...
    - **Description**: a short description explaining the purpose of the application
      group. For example, "Read/write access to Vault".
...
1. Confirm the application group details and close are correct information before closing.
...
## Step 3: Create a default ADFS role in Vault

Use the `vault write` CLI command to create a default roll for users
...
```shell-session
$ vault write auth/adfs/role/<ADFS_ROLE> \
...
associated with the ADFS authentication token.
...
## OPTIONAL: Link Active Directory groups to ADFS
...
Now you can create an Active Directory group for Vault and link the AD group to
your ADFS application group:
...
1. Create an external group in Vault and save the group ID to a file named
  ...
  $ vault write \
    -format=json \
    identity/group name="<YOUR_NEW_AD_GROUP_NAME>" \
...
1. Create a group alias:
   ...
   $ vault write identity/group-alias \
     name="<YOUR_EXISTING_ADFS_GROUP>"  \
     mount_accessor=$(cat accessor_adfs.txt) \
     canonical_id="$(cat group_id.txt)"
   ```
1. Login to Vault as an ADFS user who is a member of YOUR_EXISTING_ADFS_GROUP.
...
1. Follow the prompts to create an external group with the following
   information:
     - Name: your new AD group name
     - Type: `external`
     - Policies: the read-only ADFS policy you created. For example,
       `read-adfs-test`.
1. Click on **Add alias** and follow the prompts to map the Vault AD group name
   to an existing group on your ADFS:
   - Name: the name of an existing ADFS group (**must match exactly**).
   - Auth Backend: `<YOUR_OIDC_MOUNT_PATH>/ (oidc)`
1. Login to Vault as an ADFS user who is a member of the aliased ADFS group.
1. Read your test value from the KV plugin.
