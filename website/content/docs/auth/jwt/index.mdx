...
contains an `aud` claim (more often the case than not), the associated `bound_audiences` for the "jwt" role
must **exactly** match at least one of the `aud` claims declared for the JWT. For
