...
Vault JWT/OIDC auth method. The configuration allows Vault to obtain Google Workplace group membership and
user information during the JWT/OIDC authentication flow. The group membership obtained from Google Workplace
may be used for Identity group alias association. The user information obtained from Google Workplace can be
...
- A Google Workplace account with the [super admin role](https://support.google.com/a/answer/2405986?hl=en)
...
The Google-specific handling that's used to fetch Google Workplace groups and user information in Vault uses
[Google Workplace Domain-Wide Delegation of Authority](https://developers.google.com/admin-sdk/directory/v1/guides/delegation)
...
to obtain the key file for a Google service account capable of making requests to the Google Workplace
...
- `gsuite_admin_impersonate` `(string: <required>)` - Email address of a Google Workplace admin to impersonate.
- `fetch_groups` `(bool: false)` - If set to true, groups will be fetched from Google Workplace.
- `fetch_user_info` `(bool: false)` - If set to true, user info will be fetched from Google Workplace using the configured [user_custom_schemas](#user_custom_schemas).
- `groups_recurse_max_depth` `(int: <optional>)` - Group membership recursion max depth. Defaults to 0, which means don't recurse.
- `user_custom_schemas` `(string: <optional>)` - Comma-separated list of Google Workplace [custom schemas](https://developers.google.com/admin-sdk/directory/v1/guides/manage-schemas).
  Values set for Google Workplace users using custom schema fields will be fetched and made available as claims that can be used with [claim_mappings](/api/auth/jwt#claim_mappings). Required if [fetch_user_info](#fetch_user_info) is set to true.
