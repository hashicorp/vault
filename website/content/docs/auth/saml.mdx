...
1. Create a named role:
   ...
   [API documentation](/vault/api-docs/auth/saml).
...
that both the [`bound_subject`](/vault/api-docs/auth/saml#bound_subject) and
...
For example, a role with `bound_subject=*@hashicorp.com` and
...
controlled by the [`bound_subject_type`](/vault/api-docs/auth/saml#bound_subject_type) and
[`bound_attributes_type`](/vault/api-docs/auth/saml#bound_subject_type) parameters.

## API

The SAML Auth Plugin has a full HTTP API. Please see the
[API documentation](/vault/api-docs/auth/saml) for more details.
