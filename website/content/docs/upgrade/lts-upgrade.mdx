---
layout: docs
page_title: LTS Vault upgrades
description: >-
  General guidance for LTS upgrades
---

# LTS Vault upgrades

Current LTS release | Extended maintenance release
------------------- | ----------------------------
1.19.x              | 1.16.x

Before you perform a direct LTS upgrade from 1.16.x to 1.19.x, we recommend
reviewing the following summary of functional changes, unaddressed known issues,
and open bugs to determine the likely impact on your Vault deployment.

Functional changes affect how Vault works including new requirements, new
defaults, new behavior, and breaking changes. In some cases, we recommend specific
actions before or after upgrading to mitigate the impact of a functional change.

Unaddressed bugs or known issues may have suggested workarounds or mitagation
stratagies you should consider before upgrading.

@include 'release-notes/change-summary/lts/1_16-to-1_19.mdx'