...
1. Initialize a root token generation.
   ...
   Vault generates a nonce and one-time password (OTP) value, and displays these values in the command output. The nonce value should be
...
1. Initialize a root token generation, providing the path to a GPG public key or
   Keybase username of a user to encrypt the resulting token.
...
1. Each unseal key holder provides their unseal key.
   ...
   If there is a tty, Vault prompts for the key and automatically
   completes the nonce value. If there is no tty, or if the value is piped
   from stdin, the user must specify the nonce value from the `-init`
   operation.
