...
the `development_cluster` field to the `license` parameter within the `reporting` stanza. Only set the field to `true` if
the cluster is not used for production purposes — there are EULA implications if set on a production cluster.
...
If the inherited value from the primary differs from the HCL configuration on the secondary, the secondary will log the
error `"development cluster setting in config does not match that of the primary, updating to follow the primary config setting"`
to warn that it will be reporting a different value than what is set in its config.

</Note>

If the secondary cluster is promoted to a primary, it will begin to report its own `development_cluster` value determined by its HCL configuration.
