...
reported `development_cluster` value when the primary cluster changes. The error message:

> "development cluster setting in config does not match that of the primary, updating to follow the primary config setting"

is logged on the secondary when the cluster's reported value is different from its HCL configuration, to warn of the inconsistency
between the primary and secondary configurations.
