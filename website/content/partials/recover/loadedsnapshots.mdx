<Note>
  ...
  Vault has restrictions for loading snapshots:
  * The snapshot must be from the same cluster and have the same unseal keys as
  the current cluster.
  * You can load only one snapshot into a cluster at a time.
  * Loaded snapshots are automatically unloaded 72 hours after being loaded.
