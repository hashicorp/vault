...
Autopilot gets initialized with the following default values. If these default values do not meet your expected autopilot behavior, don't forget to set them to your desired values.

- `cleanup_dead_servers` `(bool: false)`
...
- `dead_server_last_contact_threshold` - `(string: "24h")`
...
result in removal of nodes that aren't actually dead.** When adding new nodes
...
- `min_quorum` `(int)` - This doesn't default to anything and should be set to the expected
...
- `last_contact_threshold` `(string "10s")`
