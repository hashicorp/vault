...
In environment variable definition conflicts between the main Vault process and
external plugins, Vault gives precedence to the plugin variables when loading
the plugin. The behavior for builtin plugins and plugins that do not specify
...
<Highlight title="Avoid conflicts with containerized plugins">
  ...
  Containerized plugins are fully isolated from the main Vault process and do
  not inherit system-defined environment variables. As a result, containerized
