...
Being able to close listeners in this way leaves the Vault API vulnerable to a
denial-of-service (DoS) attack by requests which are intentionally made by untrusted
upstream connections, causing it to become unavailable on that node.
