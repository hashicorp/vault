...
**Splunk App for Monitoring Vault (Enterprise Only):**  A new Splunk App providing sample dashboards for Vault operational and usage metrics that will include operational metrics, usage metrics and some interpretive data from Vault’s audit logs.
* **Operational metrics (telemetry)** include Disk I/O, memory and CPU usage. Storage backend metrics are also included (if using Consul or Integrated Storage).
* **Usage metrics (telemetry)** include number of identity entities, number of service tokens, token TTL distribution, and secret KV count. The usage metrics are new to Vault telemetry (and are supported by all versions of Vault).
* **Audit log interpretation** data includes requests by path, KV operations by path, distinct tokens by auth method, and leases created by path. 
...
**Vault Monitor Command:** This adds a new command, “vault monitor”, which lets users to stream logs of a running Vault server. The log level selected can be different from the log level used by the server logs.
