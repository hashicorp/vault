...
**Vault Monitor Command:** A new command, “vault monitor”, which lets users stream logs of a running Vault server. The log level selected can be different from the log level used by the server logs.
