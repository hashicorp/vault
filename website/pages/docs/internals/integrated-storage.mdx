...
As soon as a node joins a Raft cluster, it attempts to catch up on the data that
has been applied on the nodes in the rest of the cluster. While in this state,
the node cannot vote but is counted for purposes of quorum. If a number of new
nodes have joined the cluster at a similar time, and they exceed the failure
tolerance of the cluster, quorum can be lost and the cluster can fail.

As an example, say you have the following:

* A 3 node cluster with a fair amount of data (failure tolerance: 1)
* 3 new nodes attempt to join the cluster at the same time
* Now, you have a 6 node cluster with a failure tolerance of 2, but all 3 new
  nodes are still catching up, so quorum has been lost
