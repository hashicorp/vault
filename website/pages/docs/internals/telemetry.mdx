...
| `vault.core.leadership_lost`         | Measures the amount of time that the HA cluster leader was the leader, at the point when it lost leadership. This should be monitored and alerted on for overall cluster leadership status. Any count greater than zero means that Vault experienced a leadership change and could potentially be cause for alerting.  Low values are a signal of leadership flapping. | ms   | summary |
