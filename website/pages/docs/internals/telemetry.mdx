...
| `vault.core.leadership_lost`         | Duration of time taken by cluster leadership losses which have occurred in a highly available Vault cluster. This should be monitored and alerted on for overall cluster leadership status. Any count greater than zero means that Vault experienced a leadership change and could potentially be cause for alerting.  Do note that a high mean value here is considered better than a low value, because a low value means there is leadership flapping. The measured value of this metric answers the question "how long was this server the leader, when it lost leadership?" | ms   | summary |
