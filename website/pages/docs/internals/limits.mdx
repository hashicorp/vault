...
The following table assumes a conservative (but not worst-case)
estimate of 1KB per entity or group; the number is slightly less than
512 * 256 to reflect the fact that the first shard to fill up will
start inducing failures.  This maximum will decrease if each entity
has a large amount of metadata, or if each group has a large number of
members.

|   | Consul default (512 KiB) | Integrated storage default (1 MiB) |
|---|--------------------------|------------------------------------|
| Maximum number of identity entities | ~100,000 | ~220,000 |
...
| Maximum cluster size | no limit |
| Maximum number of DR replicas | no limit |
| Maximum number of performance replicas | no limit |
