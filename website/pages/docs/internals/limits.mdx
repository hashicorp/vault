...
read-modify-write cycle on the entire entry.
...
takes about 500 bytes. Each of (1) auth mounts, (2) secret engine mount
points, (3) local-only auth methods, and (4) local-only secret engine mounts
are stored separately, so the limit applies to each independently.

|   | Consul default (512 KiB) | Integrated storage default (1 MiB) |
|---|--------------------------|------------------------------------|
| Maximum number of secret engine mount points | ~1000 | ~2000 |
| Maximum number of enabled auth methods | ~1000 | ~2000 |
...
entry. However, the effective limit is generally much smaller because
each namespace must have at least two secret engine mounts (for `sys`
and `identity`) and at least one auth engine mount (for `token`).

|   | Consul default (512 KiB) | Integrated storage default (1 MiB) |
|---|--------------------------|------------------------------------|
| Maximum number of namespaces | ~500 | ~1000 |
| Maximum number of namespaces with at least one additional secret engine | ~330 | ~650 |
...
aliases and the amount of metadata.
...
membership information is stored inline in each Group object.

Entities consume about 450 bytes of storage in isolation, but compression
performs better across many entities.  The following table shows a best-case
estimate of 250 bytes per entity and a a conservative (but not worst-case)
estimate of 1KB per entity or group.  The number is slightly less than the
amount that fits in one shard, to reflect the fact that the first shard to fill up will start inducing failures.  This maximum will decrease if each entity
has a large amount of metadata, or if each group has a large number of
members.

|   | Consul default (512 KiB) | Integrated storage default (1 MiB) |
|---|--------------------------|------------------------------------|
| 
| Maximum number of identity entities (best case) | ~450,000 | ~950,000 |
| Maximum number of identity entities (conservative) | ~100,000 | ~220,000 | 
| Maximum number of groups | ~100,000 | ~220,000 |
| Maximum number of members in a group | ~12,000 | ~24,000 |
 
The number of entities can be monitored using Vault's [telemetry](/docs/internals/telemetry#token-identity-and-lease-metrics); see `vault.identity.num_entities metric` (total) or `vault.identity.entities.count metric` (by namespace).
...
`vault.identity.upsert_entity_txn` metric and
...
entity belongs to, and recursively to any parents of that entity.
...
| Size of one version of a secret | slightly less than one storage entry (512 KiB or 1024 KiB)> |
...
the `maxium_request_size` option in the [listener stanza](/docs/configuration/listener/tcp). It defaults to 32 MiB.  This value, minus the overhead of
the HTTP request itself, places an upper bound on any Transform operation,
and on the maximum size of any key-value secrets.

### Request duration

The default maximum duration of a Vault operation is
[`max_request_duration`](/configuration/listener/tcp), which defaults to
...
each replica adds considerable overhead to the active node, as
each write must be duplicated to all standbys. The overhead of
...
| Maximum number of performance replicas | no limit, up to actcive node capability |
