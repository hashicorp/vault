...
!> **Expired token rotation:** Currently, once the TTL expires for the tokens and unless you set a workflow in place to manually rotate the tokens,
Consul operations will start failing because of the tokens being expired. At this time, the recommended approach for operators is to rotate the 
tokens manually by creating a new token using the `vault read consul/creds/my-role` command and then, once the token is synchronized with Consul 
apply the token to the agents using the Consul API or CLI.
