...
Anytime Vault needs to connect to the [EC2 instance metadata
...
docker on an EC2 instance, and the instance hop limit is set to 1 which prevents
connections from docker to IMDSv2.

To avoid the timeout altogether, the hop limit may be [adjusted on the
underlying EC2 instances][aws-ec2-imdsv2-config]. If Vault is running in a docker
container on an EC2 host, setting the hop limit to 2 will allow the AWS SDK to
connect to IMDSv2 without delay.
