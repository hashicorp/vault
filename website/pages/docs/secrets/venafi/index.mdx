...
1. Configure a Venafi secret that maps a name in Vault to connection and authentication
   ...
   To see all of the options available for venafi secrets&mdash;including `ttl`, `max_ttl`
   and `issuer_hint` (for validity)&mdash;use `vault path-help venafi-pki/venafi/:name`
...
1. Lastly, configure a [role](/docs/secrets/pki)
   ...
   **Trust Protection Platform**:
   ...
   $ vault write venafi-pki/roles/tpp \
       venafi_secret="tpp" \
       store_by_serial=true store_pkey=true \
...
To see all of the options available when requesting a certificate&mdash;including
`ttl` (for validity), `key_password`, `custom_fields`&mdash;use
