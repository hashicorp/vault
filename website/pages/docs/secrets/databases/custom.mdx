...
-> In version 4, password generation was the responsibility of the plugin via the
   `GenerateCredentials` function. This is no longer the case with version 5. Vault is
   responsible for generating passwords and passing them to the plugin via
   `NewUserRequest.Password` and `UpdateUserRequest.Password.NewPassword`.
