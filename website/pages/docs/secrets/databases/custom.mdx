...
In each of the requests, you will see a `Statements` object. This object represents the set of
commands to run for that particular operation. For the `NewUser` function, this is a set of
commands to create the user (and often set permissions for that user). These statements are
from the following fields in the API:
...
It is also passed a boolean value indicating if your plugin should initialize a connection to
the database during the `Initialize` call. This function is called when the configuration is
written. This allows the user to know whether the configuration is valid and able to connect
to the database in question. If this is set to false, no connection should be made during the
`Initialize` call, but subsequent calls to the other functions will need to open a connection.
...
plugins will be removed in a future release. If you are using a version 4 custom database
plugin, the following are instructions for upgrading to version 5.
...
2. An easy way to see what functions that need to be implemented is to put the following as a
...
3. Update the initialization function. This step can be a little confusing since there are
   three functions to keep in mind: In the version 4 interface, there was `Initialize` and
   `Init`. The `Initialize` function was replaced with the `Init` function and was not used
   some time before Vault 1.6. It was kept in the interface originally for backwards
   compatibility. The version 5 interface has an `Initialize` function that will need to replace
   the `Init` function. The two arguments passed into `Init` (`config map[string]interface{}`
   and `verifyConnection bool`) are included in the `InitializeRequest` object and should be
   referenced accordingly. The verison 4 `Init` function returned a `map[string]interface{}`
   indicating the configuration that should be saved in Vault. This is wrapped inside the
   `InitializeResponse` object within the version 5 interface.
4. `SetCredentials`, `RotateRootCredentials`, and `RenewUser` are combined into `UpdateUser`.
...
5. Update `RevokeUser` to `DeleteUser`. This is the simplest change. The username to be
