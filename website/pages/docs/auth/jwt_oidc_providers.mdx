...
information, see the [Vault JWT/OIDC method documentation](/docs/auth/jwt.html).
...
are required for [external groups](/docs/secrets/identity/index.html#external-vs-internal-groups) to work.
...
- In the [OIDC Role config](/api/auth/jwt/index.html#create-role)
...
- Finally Azure AD group can be referenced by using the groups `objectId` as the [group alias name](/api/secret/identity/group-alias.html) for the external group.
