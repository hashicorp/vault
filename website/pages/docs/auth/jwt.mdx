...
However, if [`pass_namespace_in_state`](/api-docs/auth/jwt#pass_namespace_in_state)
is set to `true`, the namespace query parameter should not be included in the
allowed redirect URI, both in Vault and the OIDC provider configs.
