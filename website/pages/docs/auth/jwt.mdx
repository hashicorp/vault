...
However, as of Vault 1.6, if
[`pass_namespace_in_state`](/api-docs/auth/jwt#pass_namespace_in_state) is set
to `true`, the namespace query parameter should not be included in the allowed
redirect URI, either in Vault or in the OIDC provider configs.
