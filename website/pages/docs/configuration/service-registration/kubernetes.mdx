...
you're using Kubernetes must still exist to indicate your intentions, like so:
...
- `vault-active` `(string: "true"/"false")` – Vault active is updated dynamically each time Vault's active status changes.
  **This field is only valuable if the pod is NOT a member of a performance standby cluster**,
  it will simply be set to "false" when it's not applicable. True indicates that this Vault pod is currently the leader.
  False indicates that this Vault pod is currently a standby.
