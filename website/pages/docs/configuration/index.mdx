...
- `disable_mlock` `(bool: false)` – Disables the server from executing the
  ...
  storage](https://www.vaultproject.io/docs/internals/integrated-storage) due to
  the fact that `mlock` does not interact well with memory mapped files such as
  those created by BoltDB, which is used by Raft to track state. Additionally,
  swap should be disabled to prevent these files from being dumped into disk.
