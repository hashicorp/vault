...
method](/docs/auth/aws.html). Both `ec2` and `iam`
...
- `region` `(string: "us-east-1")` - The region to use for signing the authentication request. The
  ...
  [`sts_endpoint`](/api/auth/aws/index.html#sts_endpoint),
...
- `header_value` `(string: optional)` - If configured in Vault, the value to use for
  [`iam_server_id_header_value`](/api/auth/aws/index.html#iam_server_id_header_value).
