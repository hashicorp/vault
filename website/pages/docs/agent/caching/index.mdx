...
existing Vault token in the request and instead uses the auth-auth token. 
...
-> **Note:** Vault Agent does not require configuring auto-auth in order to use the
  caching functionality. If no auto-auth is configured or Vault is otherwise
  unable to authenticate and retrieve a token, using `force` will still remove
  any token included in a request and replace it with an empty token value,
  likely resulting in failures due to the requests now being unauthenticated.
