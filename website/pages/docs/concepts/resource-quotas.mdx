...
By default, no paths are exempt from rate limiting. However, Vault operators can
set paths that are exempt from all rate limit resource quotas by updating the
resource quota `rate_limit_exempt_paths` configuration field.
