...
A rate limit can be created with an optional `block`, such that when set to a non-zero
value, any client that hits a rate limit threshold will be blocked from all subsequent
requests for a duration of `block` seconds.
