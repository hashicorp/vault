...
API. When joining a node, the API address of the leader node must be used. We
...
If the failed servers are not recoveryable partial recovery is possible using
...
* id (string: <required>) - Specifies the node ID of the server. This can be
  found in the logs when the server starts up if it was auto-generated, and it
  can also be found inside the node-id file in the server's data directory.
...
* non_voter (bool: <false>) - This controls whether the server is a non-voter.
  If omitted, it will default to false, which is typical for most clusters.
