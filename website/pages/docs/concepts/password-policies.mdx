...
As more characters are generated, the amount of time increases (as seen in the `No Minimum Characters` example).
...
`abcde` you the chances of selecting `c` from it is 1/5. However if you have a 2 character password, the chances
are greater because you have a second chance to select `c` from the charset.
...
(26/94 for lowercase characters). This results in a dramatic loss of performance.
