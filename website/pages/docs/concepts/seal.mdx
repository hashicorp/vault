...
Once a Vault node is unsealed, it remains unsealed until one of two things happens:

1. It is resealed via the API (see below).

2. The server is restarted.

3. Something sufficiently bad happens at the storage layer that Vault decides
it can't continue operation.
