...
- `server` - Values that configure running a Vault server within Kubernetes.
  ...
  - `image` - Values that configure the Vault Docker image.
    ...
    * `ingress` - Values that configure Ingress services for Vault.
      ...
      If [`ha`](#ha) is enabled the Ingress will point to the active vault server via the `active` Service. This requires vault 1.4+ and [service_registration](https://www.vaultproject.io/docs/configuration/service-registration/kubernetes) to be set in the config.
    ...
    * `route` - Values that configure Route services for Vault in OpenShift
      ...
      ~> If [`ha`](#ha) is enabled the Route will point to the active vault server via the `active` Service (requires vault 1.4+ and [service_registration](https://www.vaultproject.io/docs/configuration/service-registration/kubernetes) to be set in the config).
