  ...
  - `image` - Values that configure the Vault Docker image.
    ...
    * `readinessProbe` - Values that configure the readiness probe for the Vault pods.
    ...
    - `standalone` - This configures `standalone` mode for the Vault server.
      ...
      - `config` -
        A list of values to define the configuration for Vault servers.
        This will be saved as a JSON into a ConfigMap that is read by the Vault servers.
      ...
      New parameters can be set using Helm's `--set` flag (vault-helm v0.1.0 and later), using the following syntax:

      ```shell
      --set server.standalone.config.ui=true --set server.standalone.config.listener.tcp.tls_disable=0
      ```
