...
- `vault.hashicorp.com/agent-init-first` - configures the pod to run the Vault Agent 
  init container first.  This is useful when other init containers need pre-populated 
  secrets.  This should be set to a `true` or `false` value.  Defaults to `false`.
...
- `vault.hashicorp.com/agent-inject-token` - configures Vault Agent to share the Vault 
  token with other containers in the mode.  This is helpful when other containers 
