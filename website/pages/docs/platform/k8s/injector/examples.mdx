...
- the master Kubernetes nodes can connect to worker nodes on port `8080`,
...
Depending on where Vault is running (e.g. outside the Kubernetes cluster), there
are some other connectivity requirements to keep in mind:
...
Agent configuration settings, [see the Agent documentation](/docs/agent/template#vault-agent-templates).
