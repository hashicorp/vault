...
This command is used to join a new node as a peer to the raft cluster.
...
functionality of the raft storage backend. There are 2 subcommands supported:
`save` and `restore`.
...
Usage: vault operator raft snapshot <subcommand> [options] [args]
  ...
  This command groups subcommands for operators interacting with the snapshot
  functionality of the raft storage backend.
...
Takes a snapshot of the raft cluster state.
...
Restores a snapshot of the raft cluster state.
