...
defined `path`. NodeID can optionally be set to identify this node.
`cluster_addr` must be set to the cluster hostname of this node. For more
configuration options see the [raft storage configuration
documentation](/docs/configuration/storage/raft).
