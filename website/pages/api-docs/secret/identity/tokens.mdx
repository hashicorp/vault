...
- `allow_impersonation` `(bool: false)` - Specifies if the token generation endpoint allows impersonation for this key. Must also be enabled for applicable roles.
...
- `allow_impersonation` `(bool: false)` - Specifies if the token generation endpoint allows impersonation for this role. Must also be enabled for the associated key.
...
- `subject` `(string: "")` - Subject to use for impersonation

- `issuer` `(string: "")` - Issuer to use for impersonation

- `claims` `(string: "")` - Custom claims to override for impersonation. This may be in string-ified JSON or base64 format.
