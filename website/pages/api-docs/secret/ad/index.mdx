...
- `auto_disable_account` (bool: false, optional): Auto disable account will disable the services accounts on 
ckeck-in and enable them on check-out. The UserAccountControl attribute is used with the ACCOUNT_DISABLE flag. 
Defaults to false.
