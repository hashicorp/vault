...
This endpoint is used to create a lease count quota by `name`. A lease count quota
must include a `max_leases` value with an optional `path` that can either be a
namespace or mount.
