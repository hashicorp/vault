...
has completed. The response included the actual time period covered, if a query covers more time, or
less time, than requested.

All namespaces will be included, that are children of the namespace where the request was made.
...
Times must be given as RFC3339 timestamps; both `start_time` and `end_time` are optional.
