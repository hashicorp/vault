...
- `block` `(string: "")` - If set, when a client reaches a rate limit threshold,
the client will be prohibited from any further requests until after the 'block' interval.
...
```json
{
  ...
  "renewable": false,
  "data": {
    // TODO: Ensure the response here is valid
    "interval": "2m0s",
    "block": "5m",
