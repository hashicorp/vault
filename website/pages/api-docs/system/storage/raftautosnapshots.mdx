...
has an interval controlling how often snapshots are taken, a destination to
which to write the snapshots, as well as a retention policy governing when
...
- `aws_s3_disable_tls` `(string)` - Disable TLS for the S3 endpoint.  This
