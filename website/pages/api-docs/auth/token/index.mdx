...
be cached.  Listing the `/auto/token/accessors` endpoint is a good way to get
...
For each accessor listed in the accessor index, tidy will check if the corresponding
token still exists in storage, and if not will delete the accessor from the index.
If the token still exists in storage but shouldn't, tidy will try to revoke it
and any child leases it might have, then delete the accessor entry.
