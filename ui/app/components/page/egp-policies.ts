...
import type { PoliciesEgpIndexModel } from 'vault/routes/vault/cluster/policies/egp/index';

/**
 * @module Page::EgpPolicies
 *
 * Renders the EGP policies list view. Wraps Page::ListView with the
 ...
 *
 * @param {PoliciesEgpIndexModel} model - route model: policies array, listViewConfig, page, pageSize
...
interface Args {
  model: PoliciesEgpIndexModel;
  page: number;
  pageSize: number;
}

export default class PageEgpPoliciesComponent extends Component<Args> {
  ...
  async downloadPolicy(item: PolicyRow) {
    try {
      const res = await this.api.sys.systemReadPoliciesEgpName(item.name);
  ...
  async deletePolicy(item: PolicyRow) {
    try {
      await this.api.sys.systemDeletePoliciesEgpName(item.name);
