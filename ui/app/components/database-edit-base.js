// ARG TODO copied from transform
...
export default Component.extend(FocusOnInsertMixin, {
  ...
  modelPrefixFromType(modelType) {
    let modelPrefix = '';
    if (modelType && modelType.startsWith('database/')) {
      modelPrefix = `${modelType.replace('database/', '')}/`;
    }
    return modelPrefix;
  },
  ...
  listTabFromType(modelType) {
    let tab;
    if (modelType && modelType.startsWith('database/')) {
      tab = `${modelType.replace('database/', '')}`;
    }
    return tab;
  },
  ...
  applyChanges(type, callback = () => {}) {
    const modelId = this.model.id || this.model.name; // database comes in as model.name
