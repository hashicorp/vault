...
module('Integration | Component | namespace-picker', function (hooks) {
  ...
  test('it filters namespace options based on search input', async function (assert) {
    ...
    // Verify filtered namespaces are displayed
    assert.strictEqual(
      findAll(NAMESPACE_PICKER_SELECTORS.link()).length,
      1,
  ...
  test('it resets the namespace list when the search input is cleared', async function (assert) {
    assert.expect(2);
    ...
    // Verify all namespaces are displayed again
    assert.strictEqual(
      findAll(NAMESPACE_PICKER_SELECTORS.link()).length,
      3,
  ...
  test('it updates the namespace list after clicking "Refresh list"', async function (assert) {
    ...
    assert
      .dom(NAMESPACE_PICKER_SELECTORS.link('parent2'))
      .exists('The new namespace "parent2" is displayed after refreshing');
  });
