...
module('Integration | Component | get-credentials-card', function (hooks) {
  ...
  test('it shows a disabled button when no item is selected', async function (assert) {
    ...
    assert.dom(GENERAL.buttonByAttr('get-credentials')).isDisabled();
    assert.dom(GENERAL.buttonByAttr('get-credentials')).hasText('Get credentials', 'Button has default text');
  });
  ...
  test('it shows button that can be clicked to credentials route when an item is selected', async function (assert) {
    ...
    assert.dom(GENERAL.buttonByAttr('get-credentials')).isEnabled();
    await click(GENERAL.buttonByAttr('get-credentials'));
    assert.propEqual(
