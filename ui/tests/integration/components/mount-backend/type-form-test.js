...
const secretTypes = filterEnginesByMountType({ mountGroup: 'secret', isEnterprise: false }).map(
  (engine) => engine.type
);
const allSecretTypes = filterEnginesByMountType({ mountGroup: 'secret', isEnterprise: true }).map(
  (engine) => engine.type
);
