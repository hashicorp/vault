...
module('Integration | Component | details/policies', function (hooks) {
...
path "kv/data/appB/*" {
  ...
  test('it renders ceiling policy table and snippets tabs', async function (assert) {
    await render(hbs`
      <Details::Policies @agent={{this.data.agent}} @entity={{this.data.entity}} @groups={{this.data.groups}} @aggregatePolicy={{this.data.aggregatePolicy}} />
    `);
