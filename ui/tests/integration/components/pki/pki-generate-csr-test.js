...
module('Integration | Component | pki-generate-csr', function (hooks) {
  ...
  test('it should show generated CSR for type=exported', async function (assert) {
    ...
    assert
      ...
      .hasAttribute('data-clipboard-target', this.model.csr, 'it renders copyable csr');
    assert
    ...
    assert
      ...
      .hasAttribute('data-clipboard-target', this.model.privateKey, 'it renders copyable private_key');
    assert
