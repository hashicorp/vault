    ...
    module('isEnterprise', function (hooks) {
      ...
      module('AWS specific', function () {
        test('it shows validation error if default lease is entered but max lease is not', async function (assert) {
          ...
          await render(hbs`
                <SecretEngine::ConfigureCreateEdit @backendPath={{this.id}} @displayName={{this.displayName}} @type='aws' @model={{this.config}} @issuerConfig={{this.issuerConfig}} @secondModel={{this.secondConfig}}/>
              `);
