...
module('Integration | Component | kv-v2 | Page::Configuration', function (hooks) {
  ...
  test('it renders kv configuration details', async function (assert) {
    ...
    assert.dom(GENERAL.title).includesText(this.mountData.path, 'renders engine path as page title');
    assert.dom(GENERAL.infoRowValue('Require check and set')).hasText('No');
    assert.dom(GENERAL.infoRowValue('Automate secret deletion')).hasText('Never delete');
    assert.dom(GENERAL.infoRowValue('Maximum number of versions')).hasText('0');
    assert.dom(GENERAL.infoRowValue('Accessor')).hasText(this.mountData.accessor);
    assert.dom(GENERAL.infoRowValue('Path')).hasText(this.mountData.path);
    assert.dom(GENERAL.infoRowValue('Type')).hasText(this.mountData.type);
    assert.dom(GENERAL.infoRowValue('Description')).doesNotExist();
    assert.dom(GENERAL.infoRowValue('Seal wrap')).hasText('No');
    assert.dom(GENERAL.infoRowValue('Default Lease TTL')).hasText('3 days');
    assert.dom(GENERAL.infoRowValue('Max Lease TTL')).hasText('5 days 3 hours');
  });
  ...
  test('it renders non default kv engine config data', async function (assert) {
    ...
    assert.dom(GENERAL.infoRowValue('Require check and set')).hasText('Yes');
    assert.dom(GENERAL.infoRowValue('Automate secret deletion')).hasText('10 days');
    assert.dom(GENERAL.infoRowValue('Maximum number of versions')).hasText('10');
  });
});
