...
module('Integration | Component | kv-v2 | Page::Secret::Paths', function (hooks) {
  ...
  hooks.beforeEach(async function () {
    ...
    this.assertClipboard = (assert, element, expected) => {
      assert.dom(element).hasAttribute('data-test-clipboard-text', expected);
    };
  });
...
/>
    ...
    assert.dom(PAGE.paths.snippetCopy('cli')).hasAttribute('data-test-clipboard-text', expected.cli);
    assert.dom(PAGE.paths.codeSnippet('api')).hasText(expected.apiDisplay);
    assert.dom(PAGE.paths.snippetCopy('api')).hasAttribute('data-test-clipboard-text', expected.apiCopy);
  });
