...
module('Acceptance | vault-reporting', function (hooks) {
  ...
  test('dashboard card: Secret engines', async function (assert) {
    ...
    assert.dom('[data-test-dashboard-secret-engines]').exists('renders Secret engines card');
    assert
      .dom('[data-test-dashboard-secret-engines] [data-test-dashboard-card-title]')
      .exists('title is present')
      .hasText('Secret engines', 'title is correct');
    assert
      .dom('[data-test-dashboard-secret-engines] [data-test-dashboard-card-title-link]')
      .exists('title link is present')
      .hasAttribute('href', 'secrets', 'link points to secrets');
    assert
      .dom('[data-test-dashboard-secret-engines] [data-test-dashboard-card-description]')
      .exists('description is present')
      .hasText('Enabled secret engines for this cluster.', 'description is correct');
    assert.dom('[data-test-dashboard-secret-engines]').includesText('aws nomad cubbyhole 47 46 45');
  });
  ...
  test('dashboard card: Authentication methods', async function (assert) {
    ...
    assert.dom('[data-test-dashboard-auth-methods]').exists('renders Authentication methods card');
    assert
      .dom('[data-test-dashboard-auth-methods] [data-test-dashboard-card-title]')
      .exists('title is present')
      .hasText('Authentication methods', 'title is correct');
    assert
      .dom('[data-test-dashboard-auth-methods] [data-test-dashboard-card-title-link]')
      .exists('title link is present')
      .hasAttribute('href', 'access', 'link points to access');
    assert
      .dom('[data-test-dashboard-auth-methods] [data-test-dashboard-card-description]')
      .exists('description is present')
      .hasText('Enabled authentication methods for this cluster.', 'description is correct');
    assert.dom('[data-test-dashboard-auth-methods]').includesText('kubernetes userpass aws 44 43 42');
  });
  ...
  test('dashboard card: Global lease count quota', async function (assert) {
    ...
    assert.dom('[data-test-dashboard-lease-count]').exists('renders Global lease count quota card');
    assert
      .dom('[data-test-dashboard-lease-count] [data-test-dashboard-card-title]')
      .exists('title is present')
      .hasText('Global lease count quota', 'title is correct');
    assert
      .dom('[data-test-dashboard-lease-count] [data-test-dashboard-card-title-link]')
      .exists('title link is present')
      .hasAttribute(
        'href',
    ...
    assert
      .dom('[data-test-dashboard-lease-count] [data-test-dashboard-card-description]')
      .exists('description is present')
  ...
  test('dashboard card: Cluster replication status', async function (assert) {
    ...
    assert.dom('[data-test-dashboard-cluster-replication]').exists('renders Cluster replication status card');
    assert
      .dom('[data-test-dashboard-cluster-replication] [data-test-dashboard-card-title]')
      .exists('title is present')
      .hasText('Cluster replication', 'title is correct');
    assert
      .dom('[data-test-dashboard-cluster-replication] [data-test-dashboard-card-title-link]')
      .exists('title link is present')
      .hasAttribute('href', 'replication', 'link points to replication');
    assert
      .dom('[data-test-dashboard-cluster-replication] [data-test-dashboard-card-description]')
      .exists('description is present')
