...
module('Acceptance | transit (flaky)', function (hooks) {
  ...
  test('it rotates, encrypts and exports key type aes256-gcm96', async function (assert) {
    assert.expect(6);
    const keyData = {
      name: (uid) => `aes-2B-${uid}`,
      type: 'aes256-gcm96',
  ...
  test('it rotates, encrypts and signs key type ecdsa-p256', async function (assert) {
    assert.expect(6);
    const keyData = {
      name: (uid) => `ecdsa-3A-${uid}`,
      type: 'ecdsa-p256',
      exportable: true,
    ...
    // navigate back to actions tab
    await click(SELECTORS.actionsTab);
    await waitUntil(() => find(SELECTORS.card('sign')));
    assert.dom(SELECTORS.card('sign')).exists(`renders sign card for ${name}`);
    await click(SELECTORS.card('sign'));
