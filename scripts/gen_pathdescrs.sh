...
vault server -dev -dev-root-token-id=root -log-level=trace > /tmp/pathdescr.log 2>&1 &
sleep 2
VAULT_PID=$!
...
if [ "$1" == "-p" ]; then
  curl -H "X-Vault-Token: root" "http://127.0.0.1:8200/v1/sys/internal/specs/pathdescr" | jq > openapi.json
else
  curl -H "X-Vault-Token: root" "http://127.0.0.1:8200/v1/sys/internal/specs/pathdescr" > openapi.json
fi
