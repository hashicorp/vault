...
func genUsername(displayName, policyName, userType, usernameTemplate string) (ret string, err error) {
	...
	case "iam_user", "assume_role":
		...
		if len(ret) > 64 {
			return "", errors.New("the username generated by the template exceeds the IAM username length limits of 64 chars")
		}
	case "sts":
		up, err := template.NewTemplate(template.Template(usernameTemplate))
		if err != nil {
		...
		if len(ret) > 32 {
			return "", errors.New("the username generated by the template exceeds the STS username length limits of 32 chars")
		}
	}
	return
}
...
func (b *backend) secretAccessKeysRevoke(ctx context.Context, req *logical.Request, d *framework.FieldData) (*logical.Response, error) {
	...
	if ok {
		...
		if ok {
			...
			return nil, errors.New("secret has is_sts but value could not be understood")
		}
	}
