package ad

import (
	"context"
	"fmt"
	"testing"

	"github.com/hashicorp/go-hclog"
	"github.com/hashicorp/vault/builtin/logical/ad/config"
	"github.com/hashicorp/vault/logical"
)

var (
	ctx     = context.Background()
	storage = &logical.InmemStorage{}
)

func TestConfigWriteReadDelete(t *testing.T) {

	b, err := Factory(ctx, &logical.BackendConfig{
		Logger:      hclog.NewNullLogger(),
		StorageView: storage,
	})
	if err != nil {
		t.Fatal(err)
	}

	// Write
	req := &logical.Request{
		Operation: logical.UpdateOperation,
		Path:      config.BackendPath,
		Storage:   storage,
		Data: map[string]interface{}{
			"username":    "tester",
			"password":    "pa$$w0rd",
			"urls":        "ldap://138.91.247.105",
			"certificate": validCertificate,
			"dn":          "example,com",
		},
	}
	resp, err := b.HandleRequest(ctx, req)
	if err != nil {
		t.Fatal(err)
	}
	verifyResponse(t, resp)
	configIsStoredAsExpected(t)

	// Read
	req = &logical.Request{
		Operation: logical.ReadOperation,
		Path:      config.BackendPath,
		Storage:   storage,
	}
	resp, err = b.HandleRequest(ctx, req)
	if err != nil {
		t.Fatal(err)
	}
	verifyResponse(t, resp)
	configIsStoredAsExpected(t)

	// Delete
	req = &logical.Request{
		Operation: logical.DeleteOperation,
		Path:      config.BackendPath,
		Storage:   storage,
	}
	resp, err = b.HandleRequest(ctx, req)
	if err != nil {
		t.Fatal(err)
	}
	if resp != nil {
		t.Fatal("expected a nil resp, to provide a 204 with no body as the outer response")
	}
	entry, err := storage.Get(ctx, config.StorageKey)
	if err != nil {
		t.Fatal(err)
	}
	if entry != nil {
		t.Fatal("config should no longer be stored")
	}
}

func configIsStoredAsExpected(t *testing.T) {
	entry, err := storage.Get(ctx, config.StorageKey)
	if err != nil {
		t.Fatal(err)
	}

	engineConf := &config.EngineConf{}
	if err := entry.DecodeJSON(engineConf); err != nil {
		t.Fatal(err)
	}

	if engineConf.ADConf.Certificate != validCertificate {
		t.Fatalf("expected Certificate to be the given valid certificate but received \"%s\"", engineConf.ADConf.Certificate)
	}

	if engineConf.ADConf.RootDomainName != "example,com" {
		t.Fatalf("expected RootDomainName to be \"example,com\" but received \"%s\"", engineConf.ADConf.RootDomainName)
	}

	if engineConf.ADConf.InsecureTLS {
		t.Fatal("expected InsecureTLS to be false but received true")
	}

	if engineConf.ADConf.Password != "pa$$w0rd" {
		t.Fatalf("expected Password to be \"pa$$w0rd\" but received \"%s\"", engineConf.ADConf.Password)
	}

	if !engineConf.ADConf.StartTLS {
		t.Fatal("expected StartTLS to be true but received false")
	}

	if engineConf.ADConf.URLs[0] != "ldap://138.91.247.105" {
		t.Fatalf("expected the url to be \"ldap://138.91.247.105\" but received \"%s\"", engineConf.ADConf.URLs[0])
	}

	if engineConf.ADConf.TLSMinVersion != 771 {
		t.Fatalf("expected TLSMinVersion to be \"771\" but received \"%d\"", engineConf.ADConf.TLSMinVersion)
	}

	if engineConf.ADConf.TLSMaxVersion != 771 {
		t.Fatalf("expected TLSMaxVersion to be \"771\" but received \"%d\"", engineConf.ADConf.TLSMaxVersion)
	}

	if engineConf.ADConf.Username != "tester" {
		t.Fatalf("expected Username to be \"tester\" but received \"%s\"", engineConf.ADConf.Username)
	}

	if engineConf.PasswordConf.DefaultPasswordTTL != config.DefaultPasswordTTLs {
		t.Fatalf("received unexpected DefaultPasswordTTL of \"%d\"", engineConf.PasswordConf.DefaultPasswordTTL)
	}

	if engineConf.PasswordConf.MaxPasswordTTL != config.DefaultPasswordTTLs {
		t.Fatalf("received unexpected MaxPasswordTTL of \"%d\"", engineConf.PasswordConf.MaxPasswordTTL)
	}

	if engineConf.PasswordConf.PasswordLength != config.DefaultPasswordLength {
		t.Fatalf("received unexpected PasswordLength of \"%d\"", engineConf.PasswordConf.PasswordLength)
	}
}

func verifyResponse(t *testing.T, resp *logical.Response) {

	// Did we get the response data we expect?
	if resp.Data["certificate"] != "\n-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n" {
		t.Fatalf("expected certificate to be the given one but received \"%s\"", resp.Data["certificate"])
	}

	if resp.Data["dn"] != "example,com" {
		t.Fatalf("expected dn to be \"example,com\" but received \"%s\"", resp.Data["dn"])
	}

	if resp.Data["insecure_tls"].(bool) {
		t.Fatalf("expected insecure_tls to be false but received true")
	}

	if resp.Data["password"] != "pa$$w0rd" {
		t.Fatalf("expected password to be \"pa$$w0rd\" but received \"%s\"", resp.Data["password"])
	}

	if !resp.Data["starttls"].(bool) {
		t.Fatalf("expected starttls to be true but received false")
	}

	if fmt.Sprintf("%s", resp.Data["urls"]) != `[ldap://138.91.247.105]` {
		t.Fatalf("expected url to be \"[ldap://138.91.247.105]\" but received \"%s\"", fmt.Sprintf("%s", resp.Data["urls"]))
	}

	if resp.Data["tlsminversion"].(uint16) != 771 {
		t.Fatalf("expected tlsminversion to be \"771\" but received \"%s\"", resp.Data["tlsminversion"])
	}

	if resp.Data["tlsmaxversion"].(uint16) != 771 {
		t.Fatalf("expected tlsmaxversion to be \"771\" but received \"%s\"", resp.Data["tlsmaxversion"])
	}

	if resp.Data["username"] != "tester" {
		t.Fatalf("expected username to be \"tester\" but received \"%s\"", resp.Data["username"])
	}

	if resp.Data["default_password_ttl"] != config.DefaultPasswordTTLs {
		t.Fatalf("received unexpected default_password_ttl of \"%d\"", resp.Data["default_password_ttl"])
	}

	if resp.Data["max_password_ttl"] != config.DefaultPasswordTTLs {
		t.Fatalf("received unexpected default_password_ttl of \"%d\"", resp.Data["max_password_ttl"])
	}

	if resp.Data["password_length"] != config.DefaultPasswordLength {
		t.Fatalf("received unexpected default_password_ttl of \"%d\"", resp.Data["password_length"])
	}
}

const validCertificate = `
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`
