...
func validateRole(b *backend, entry *issuing.RoleEntry, ctx context.Context, s logical.Storage) (string, []string, error) {
	...
	if issuerId != issuing.IssuerRefNotFound {
		...
		if err != nil {
			warnings = append(warnings, fmt.Sprintf("Failed to fetch entry for issuer %q to validate against: %v", entry.Issuer, err))
		} else {
			issuerCertificate, err := issuerEntry.GetCertificate()
			if err != nil {
				warnings = append(warnings, fmt.Sprintf("Unable to get certificate for issuer %s to validate against: %v", entry.Issuer, err))
			}
			issuerKeyType := strings.ToLower(issuerCertificate.PublicKeyAlgorithm.String())
			signatureBits, err = certutil.ValidateDefaultOrValueHashBits(issuerKeyType, entry.SignatureBits)
			if err != nil {
				warnings = append(warnings, fmt.Sprintf("The Issuing Certificate %v for this role has a key algorithm, %v, incompatible with the set role signature bits, %v", entry.Issuer, issuerKeyType, entry.SignatureBits))
			}
		}
	} else {
		// We still want to update to keep default value of 256 bits
		if signatureBits == 0 {
			signatureBits = 256
		}
	}
