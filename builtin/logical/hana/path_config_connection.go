	...
	return &framework.Path{
		...
		Fields: map[string]*framework.FieldSchema{
			...
			"verify_connection": &framework.FieldSchema{
				...
				Default:     true,
				Description: "If set to true, connection_string is verified by actually connecting to the database",
			},
		},
...
func (b *backend) pathConnectionWrite(req *logical.Request, data *framework.FieldData) (*logical.Response, error) {
	...
	// Don't check the connection_string if the verification is disabled
...
Configure the connection string to talk to your SAP HANA database.
`

const pathConfigConnectionHelpDesc = `
This path configures the connection string used to connect to the HANA DB Server.
The value of the string is a Data Source Name (DSN). An example of this string is:
"connection_string=hdb://<username>:<password>@<HANA_server_IP>:<port_number>"
