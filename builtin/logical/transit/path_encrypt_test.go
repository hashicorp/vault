...
func TestTransit_BatchEncryptionCase13(t *testing.T) {
	...
	batchInput := []any{
		map[string]any{"plaintext": "bXkgc2VjcmV0IGRhdGE=", "nonce": "YmFkbm9uY2U="},
	}
...
func TestTransit_decodeBatchRequestItems(t *testing.T) {
	tests := []struct {
		...
		{
			name: "src_nonce-dest",
			src:  []any{map[string]any{"nonce": "dGVzdGNvbnRleHQ="}},
			dest: []BatchRequestItem{},
		},
		{
			name:            "src_nonce_invalid-dest",
			src:             []any{map[string]any{"nonce": 666}},
			dest:            []BatchRequestItem{},
			wantErrContains: "expected type 'string', got unconvertible type 'int'",
		},
		{
		...
		},
		{
			...
			src:  []any{map[string]any{"plaintext": "dGhlIHF1aWNrIGJyb3duIGZveA==", "nonce": "null"}},
			dest: []BatchRequestItem{},
		},
		// required fields
		{
