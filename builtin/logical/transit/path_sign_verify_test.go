...
func testTransit_SignVerify_OpenPGP(t *testing.T, bitsize int) {
	...
	*/
	...
	signRequest := func(req *logical.Request, errExpected bool, postpath string, expectedErrorContains string) string {
		...
		if err != nil && errExpected {
			// This isn't great, but I was running into situations where the resp was nil
			// and resp.IsError() wasn't running properly on transit/sign requests
