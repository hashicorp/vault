...
func TestPersistUpgradableRoleEntries(t *testing.T) {
	...
	for _, role := range before {
		err = b.nonLockedSetAWSRole(ctx, storage, role.name, role.entry)
		if err != nil {
			t.Fatal(err)
		}
	}
	...
	err = b.persistUpgradableRoleEntries(ctx, storage)
	...
	for _, name := range names {
		entry, err := b.nonLockedAWSRole(ctx, storage, name)
		if err != nil {
			t.Fatal(err)
		}
