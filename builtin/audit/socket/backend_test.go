...
func TestBackend_Factory_Conf(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"write-duration-not-valid": {
			...
			isErrorExpected:      true,
			expectedErrorMessage: "socket.Factory: error configuring sink node: socket.(Backend).configureSinkNode: error creating socket sink node: event.NewSocketSink: error applying options: time: invalid duration \"qwerty\"",
		},
