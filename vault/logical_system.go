...
func (b *SystemBackend) handleRootPluginReloadUpdate(ctx context.Context, req *logical.Request, d *framework.FieldData) (*logical.Response, error) {
	...
	pluginType, err := consts.ParsePluginType(pluginTypeStr)
	if err != nil {
		return logical.ErrorResponse(`error parsing %q as a plugin type, must be one of "auth", "secret", or "database"`, pluginTypeStr), nil
	}
	if pluginType == consts.PluginTypeUnknown {
		return logical.ErrorResponse(`plugin type %q is not supported, must be one of "auth", "secret", or "database"`, pluginTypeStr), nil
	}
	if scope != "" && scope != globalScope {
		return logical.ErrorResponse("reload scope must be omitted or 'global'"), nil
	}
