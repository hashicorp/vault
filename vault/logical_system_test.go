...
func TestHandleListCustomMessages(t *testing.T) {
	// Setup a system backend with a custom message in it
	uiConfig := &UIConfig{
	...
	require.NoError(t, uiConfig.barrierStorage.Put(context.Background(), &logical.StorageEntry{
		Key:   fmt.Sprintf("%s/000", UICustomMessageKey),
		Value: []byte(fmt.Sprintf(`{"id":"000", "title":"title", "message":"message", "type":"banner", "authenticated": true, "start_time":"%s", "end_time":"%s"}`, startTime, endTime)),
	}))
	...
	for _, testcase := range testcases {
		resp, err := backend.handleListCustomMessages(context.Background(), &logical.Request{}, testcase.fieldData)
	...
	uiConfig.barrierStorage = &testingStorage{
		listFails: true,
	}

	resp, err := backend.handleListCustomMessages(context.Background(), &logical.Request{}, &framework.FieldData{
		Schema: fieldSchemas,
		Raw:    map[string]any{},
	})
...
func TestHandleDeleteCustomMessage(t *testing.T) {
	...
	resp, err := backend.handleDeleteCustomMessage(context.Background(), &logical.Request{}, &framework.FieldData{
		Schema: fieldSchemas,
		Raw:    fieldRaw,
	})
	assert.NoError(t, err)
...
func TestHandleReadCustomMessage(t *testing.T) {
	...
	entryValue, err := jsonutil.EncodeJSON(customMessageEntry)
	require.NoError(t, err)

	err = uiConfig.barrierStorage.Put(context.Background(), &logical.StorageEntry{
		Key:   fmt.Sprintf("%s/%s", UICustomMessageKey, customMessageEntry.Id),
		Value: entryValue,
	})
	require.NoError(t, err)
	...
	assert.Equal(t, resp.Data["id"], "abc")
...
func TestHandleUpdateCustomMessage(t *testing.T) {
	...
	fieldData := &framework.FieldData{
		...
		Raw: map[string]any{
			...
			"end_time":      time.Now().Add(time.Hour).Format(time.RFC3339Nano),
			"link": map[string]any{
				"title": "link-title",
	...
	resp, err := backend.handleUpdateCustomMessage(context.Background(), &logical.Request{}, fieldData)
	assert.NoError(t, err)
	assert.NotNil(t, resp)
	assert.NotNil(t, resp.Data)
	...
	for _, testcase := range testcases {
		...
		resp, err := backend.handleUpdateCustomMessage(context.Background(), &logical.Request{}, &framework.FieldData{
...
func TestHandleCustomMessageExistenceCheck(t *testing.T) {
	...
	// returned.
	fieldData.Raw["id"] = []int{0}

	found, err = backend.handleCustomMessageExistenceCheck(context.Background(), &logical.Request{}, fieldData)
	assert.Error(t, err)
	assert.False(t, found)

	// Check that when id is not provided, false and no error are returned.
	delete(fieldData.Raw, "id")
