...
func (c *Core) BarrierRekeyUpdate(ctx context.Context, key []byte, nonce string) (*RekeyResult, logical.HTTPCodedError) {
	...
	if existingConfig.SecretThreshold == 1 {
		...
		c.barrierRekeyConfig.RekeyProgress = nil
		if err != nil {
			return nil, logical.CodedError(http.StatusInternalServerError, errwrap.Wrapf("failed to compute root key: {{err}}", err).Error())
		}
	}
	...
	case c.seal.BarrierType() == wrapping.Shamir:
		if c.seal.StoredKeysSupported() == seal.StoredKeysSupportedShamirRoot {
			...
			if err != nil {
				return nil, logical.CodedError(http.StatusInternalServerError, errwrap.Wrapf("failed to read root key: {{err}}", err).Error())
			}
		...
		if err := c.barrier.VerifyRoot(recoveredKey); err != nil {
			...
			return nil, logical.CodedError(http.StatusBadRequest, errwrap.Wrapf("root key verification failed: {{err}}", err).Error())
		}
	}
	...
	if err != nil {
		...
		return nil, logical.CodedError(http.StatusInternalServerError, errwrap.Wrapf("root key generation failed: {{err}}", err).Error())
	}
