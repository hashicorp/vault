...
func TestExpiration_RegisterAuth_NoLease(t *testing.T) {
	...
	if err != logical.ErrInvalidRequest || resp.Error().Error() != "lease not found or lease is not renewable" {
		t.Fatalf("err: %v", err)
	}
...
func TestExpiration_RenewToken_NotRenewable(t *testing.T) {
	...
	if err != logical.ErrInvalidRequest || resp.Error().Error() != "lease is not renewable" {
		t.Fatalf("err: %v", err)
	}
}
