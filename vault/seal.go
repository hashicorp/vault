...
func (d *DefaultSeal) SetCore(core *Core) {
	d.core = core
	d.logger = logformat.DeriveModuleLogger(d.core.logger, "seal")
}
...
func (d *DefaultSeal) BarrierConfig() (*SealConfig, error) {
	...
	if err := jsonutil.DecodeJSON(pe.Value, &conf); err != nil {
		d.logger.Error("failed to decode seal configuration", "error", err)
	...
	case d.BarrierType():
	default:
		d.logger.Error("barrier seal type does not match loaded type", "barrier_seal_type", conf.Type, "loaded_seal_type", d.BarrierType())
		return nil, fmt.Errorf("barrier seal type of %s does not match loaded type of %s", conf.Type, d.BarrierType())
	}

	// Check for a valid seal configuration
	if err := conf.Validate(); err != nil {
		d.logger.Error("invalid seal configuration", "error", err)
