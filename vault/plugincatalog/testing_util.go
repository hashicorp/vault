...
// writeFileToZip adds a file with specified name and content to the ZIP writer
func writeFileToZip(t *testing.T, zipWriter *zip.Writer, name string, content []byte) {
	t.Helper()

	fileWriter, err := zipWriter.Create(name)
	assert.NoError(t, err, "expected to create file in ZIP writer", name)

	_, err = fileWriter.Write(content)
	assert.NoError(t, err, "expected to write content to file in ZIP writer", name)
}
