// Copyright (c) HashiCorp, Inc.
// SPDX-License-Identifier: BUSL-1.1

package plugincatalog

import (
	"archive/zip"
	"crypto/sha256"
	"errors"
	"fmt"
	"io"
	"os"
	"path"
	"path/filepath"
	"runtime"
	"strings"

	"github.com/ProtonMail/gopenpgp/v3/crypto"
	"github.com/hashicorp/vault/sdk/helper/consts"
	"github.com/hashicorp/vault/sdk/helper/pluginutil"
)

// hashiCorpPGPPubKey is HashiCorp's PGP public key at https://www.hashicorp.com/.well-known/pgp-key.txt.
// This key is used to verify the authenticity of HashiCorp plugins.
const hashiCorpPGPPubKey = `
-----BEGIN PGP PUBLIC KEY BLOCK-----


=7pIB
-----END PGP PUBLIC KEY BLOCK-----
`

var defaultPGPPubKey = hashiCorpPGPPubKey

const (
	artifactZipFmt = "%s_%s_%s_%s.zip" // vault-plugin-database-oracle_1.2.3+ent_linux_amd64.zip
	metadataFile   = "metadata.json"
	metadataSig    = "metadata.json.sig"
)

var (
	errInvalidExtractPath             = errors.New("invalid extract file path")
	errPluginArtifactUnzipDstNotFound = errors.New("plugin artifact unzip destination not found")
	errCreatePluginDst                = errors.New("failed to create plugin unzip destination")
	errReadMetadata                   = errors.New("failed to read metadata")
	errReadMetadataSig                = errors.New("failed to read metadata signature")
	errReadPlugin                     = errors.New("failed to read plugin binary")
	errReadPluginMetadata             = errors.New("failed to read plugin metadata")
	errReadPluginPGPSig               = errors.New("failed to read plugin binary PGP signature")
	errVerifyMetadataSig              = errors.New("failed to verify metadata detached signature")
	errVerifyPluginSig                = errors.New("failed to verify plugin binary PGP signature")
)

// unpackArtifact unpacks the plugin artifact and verifies it.
// It returns the plugin tier, unpacked artifact directory name, command, sha256sum, and error if any.
func (c *PluginCatalog) unpackArtifact(plugin pluginutil.SetPluginInput) (consts.PluginTier, string, string, []byte, error) {
	zipPath := filepath.Join(c.directory, zipName(plugin.Name, plugin.Version))

	tempDir, err := os.MkdirTemp(c.directory, "unzip-")
	if err != nil {
		return consts.PluginTierUnknown, "", "", nil,
			fmt.Errorf("failed to create temporary destination directory to unzip: %w", err)
	}
	defer os.RemoveAll(tempDir)

	tempPluginDir, err := unzip(plugin.Name, zipPath, tempDir)
	if err != nil {
		return consts.PluginTierUnknown, "", "", nil,
			fmt.Errorf("failed to unzip: %w", err)
	}

	if err = verifyPlugin(tempPluginDir, plugin.Name, defaultPGPPubKey); err != nil {
		return consts.PluginTierUnknown, "", "", nil,
			fmt.Errorf("failed to verify plugin: %w", err)
	}

	if err = c.copyExtractedPluginDirectory(tempPluginDir); err != nil {
		return consts.PluginTierUnknown, "", "", nil,
			fmt.Errorf("failed to write plugin directory: %w", err)
	}

	sha256sum, err := pluginSHA256Sum(path.Join(tempPluginDir, plugin.Name))
	if err != nil {
		return consts.PluginTierUnknown, "", "", nil,
			fmt.Errorf("failed to calculate SHA256 sum of plugin: %w", err)
	}

	unpackedArtifactDirName := path.Base(tempPluginDir)
	command := path.Join(unpackedArtifactDirName, plugin.Name)

	return consts.PluginTierOfficial, unpackedArtifactDirName, command, sha256sum, nil
}

// copyExtractedPluginDirectory copies the temporary extracted plugin directory
// to the plugin catalog directory
func (c *PluginCatalog) copyExtractedPluginDirectory(tempPluginDir string) error {
	pluginDir := path.Base(tempPluginDir)
	dst := path.Join(c.directory, pluginDir)

	// we overwrite the plugin directory if it already exists
	_ = os.RemoveAll(dst)

	err := os.CopyFS(dst, os.DirFS(tempPluginDir))
	if err != nil {
		return fmt.Errorf("failed to copy plugin directory: %w", err)
	}

	return nil
}

func zipName(command, version string) string {
	if strings.HasPrefix(version, "v") {
		version = version[1:]
	}

	return fmt.Sprintf(artifactZipFmt, command, version, runtime.GOOS, runtime.GOARCH)
}

// sanitizeExtractFilePath ensures the extracted file path is within the given directory
// avoiding path traversal attack
func sanitizeExtractFilePath(dir, filePath string) (string, error) {
	resolved := filepath.Join(dir, filePath)
	if !strings.HasPrefix(resolved, filepath.Clean(dir)+string(os.PathSeparator)) {
		return "", errInvalidExtractPath
	}
	return filePath, nil
}

// unzip extracts the plugin artifact to the destination directory
func unzip(pluginName, src, dst string) (string, error) {
	zipReader, err := zip.OpenReader(src)
	if err != nil {
		return "", err
	}
	defer zipReader.Close()

	if _, err := os.Stat(dst); err != nil {
		return "", fmt.Errorf("%w: %s", errPluginArtifactUnzipDstNotFound, dst)
	}

	// preserve the plugin archive structure when we unzip to the destination
	// Archive:  vault-plugin-secrets-example_1.2.3_darwin_arm64.zip
	// vault-plugin-secrets-example_1.2.3+ent_darwin_arm64/LICENSE
	// vault-plugin-secrets-example_1.2.3+ent_darwin_arm64/vault-plugin-secrets-example
	// vault-plugin-secrets-example_1.2.3+ent_darwin_arm64/metadata.json
	// vault-plugin-secrets-example_1.2.3+ent_darwin_arm64/metadata.json.sig

	pluginRelativePath := strings.Trim(path.Base(src), ".zip")
	pluginDst := path.Join(dst, pluginRelativePath)
	if err = os.Mkdir(path.Join(dst, pluginRelativePath), os.FileMode(0o770)); err != nil {
		return "", fmt.Errorf("%w: %w", errCreatePluginDst, err)
	}

	for _, file := range zipReader.File {
		if file.Mode().IsDir() {
			continue
		}

		var sanitizedPath string
		if sanitizedPath, err = sanitizeExtractFilePath(pluginDst, file.Name); err != nil {
			return "", err
		}

		pin, err := zipReader.Open(sanitizedPath)
		if err != nil {
			return "", err
		}

		rawBytes, err := io.ReadAll(pin)
		if err != nil {
			return "", err
		}
		pin.Close()

		var fileMode os.FileMode
		switch path.Base(sanitizedPath) {
		case pluginName:
			fileMode = os.FileMode(0o770)
		case metadataFile, metadataSig:
			fileMode = os.FileMode(0o660)
		default:
			// ignore other and LICENSE files
			continue
		}

		if err = os.WriteFile(path.Join(pluginDst, sanitizedPath), rawBytes, fileMode); err != nil {
			return "", err
		}
	}

	return pluginDst, nil
}

func verifyPlugin(pluginDir, pluginName, pubKey string) error {
	pgp := crypto.PGP()
	key, err := crypto.NewKeyFromArmored(pubKey)
	if err != nil {
		return err
	}

	verifier, err := pgp.Verify().VerificationKey(key).New()
	if err != nil {
		return err
	}

	// verify metadata.json is untampered
	metadataPath := path.Join(pluginDir, "metadata.json")
	metadataBytes, err := os.ReadFile(metadataPath)
	if err != nil {
		return fmt.Errorf("%w: %w", errReadMetadata, err)
	}

	metadataSigBytes, err := os.ReadFile(path.Join(pluginDir, "metadata.json.sig"))
	if err != nil {
		return fmt.Errorf("%w: %w", errReadMetadataSig, err)
	}

	verifyResult, err := verifier.VerifyDetached(metadataBytes, metadataSigBytes, crypto.Armor)
	if err != nil {
		return fmt.Errorf("%w: %w", errVerifyMetadataSig, err)
	}
	if sigErr := verifyResult.SignatureError(); sigErr != nil {
		return fmt.Errorf("%w: %w", errVerifyMetadataSig, sigErr)
	}

	// verify plugin binary is untampred
	pluginBytes, err := os.ReadFile(path.Join(pluginDir, pluginName))
	if err != nil {
		return fmt.Errorf("%w: %w", errReadPlugin, err)
	}

	metadata, err := readPluginMetadata(metadataPath)
	if err != nil {
		return fmt.Errorf("%w: %w", errReadPluginMetadata, err)
	}

	verifyResult, err = verifier.VerifyDetached(pluginBytes, []byte(metadata.Plugin.PGPSig), crypto.Armor)
	if err != nil {
		return fmt.Errorf("%w: %w", errVerifyPluginSig, err)
	}
	if sigErr := verifyResult.SignatureError(); sigErr != nil {
		return fmt.Errorf("%w: %w", errVerifyPluginSig, sigErr)
	}

	return nil
}

func pluginSHA256Sum(path string) ([]byte, error) {
	file, err := os.Open(path)
	if err != nil {
		return nil, err
	}
	defer file.Close()

	hash := sha256.New()
	if _, err = io.Copy(hash, file); err != nil {
		return nil, err
	}

	return hash.Sum(nil), nil
}
