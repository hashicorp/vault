...
// generateCurrentMonthClientData generates current month client data using slices of entity records for repeated clients
// and new current month data.
func generateCurrentMonthClientData(t *testing.T, core *Core, repeatedClients []*activity.EntityRecord, currentMonthData []*activity.EntityRecord) {
	t.Helper()

	a := core.activityLog

	// Create the records and write them to storage
	clientRecords := make([]*activity.EntityRecord, 0, len(repeatedClients)+len(currentMonthData))

	// add repeated clients
	clientRecords = append(clientRecords, repeatedClients...)

	// add new clients
	clientRecords = append(clientRecords, currentMonthData...)
	...
	startOfMonth := timeutil.StartOfMonth(time.Now().UTC())
	path := fmt.Sprintf("%ventity/%v/%v", ActivityLogPrefix, startOfMonth.Unix(), 0)
	WriteToStorage(t, core, path, data)
	...
	assert.NoError(t, err, "error loading clients")
	wg.Wait()
}

// generateClientData creates a slice of entity records given entity attributes
func generateClientData(numEntities, numTokens int, clientData ...string) []*activity.EntityRecord {
	totalClients := numEntities + numTokens
	clientRecords := make([]*activity.EntityRecord, 0, totalClients)
	mountAccessor := "randomMount"
	namespaceId := namespace.RootNamespaceID
	if len(clientData) > 0 {
		mountAccessor = clientData[0]
	}
	if len(clientData) > 1 {
		namespaceId = clientData[1]
	}
	clientPrefix := "client"
	if len(clientData) > 2 {
		clientPrefix += clientData[2]
	}

	for i := 0; i < totalClients; i++ {
		clientRecords = append(clientRecords, &activity.EntityRecord{
			ClientID:      fmt.Sprintf("%s%s", clientPrefix, RandStringBytes(7)),
			NamespaceID:   namespaceId,
			Timestamp:     time.Now().Unix(),
			NonEntity:     i >= numEntities,
...
// generateCurrentMonthClients writes current month entity and non-entity
// activity records to storage and calls refreshLogsFromStorage to update the
// partialMonthClientTracker.
func generateCurrentMonthClients(t *testing.T, core *Core, numEntities, numTokens int, repeatedClients []*activity.EntityRecord) {
	t.Helper()

	a := core.activityLog

	// Create the records and write them to storage
	totalClients := numEntities + numTokens + len(repeatedClients)
	clientRecords := make([]*activity.EntityRecord, 0, totalClients)
	...
	// add new clients
	clientRecords = append(clientRecords, generateClientData(numEntities, numTokens)...)
