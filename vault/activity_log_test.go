...
// TestActivityLog_Creation_WrappingTokens calls HandleTokenUsage for two wrapping tokens, and verifies that this doesn't create a fragment.
...
// TestActivityLog_UniqueEntities calls AddEntityToFragment 4 times with 2 different clients, then verifies that there are only 2 clients in the fragment and that they have the earlier timestamps
...
// TestActivityLog_SaveTokensToStorage calls AddTokenToFragment with duplicate namespaces and then saves the segment to storage. The test then reads and unmarshals the segment, and verifies that the results have the correct counts by namespace.
...
// TestActivityLog_SaveEntitiesToStorage calls AddEntityToFragment with clients with different namespaces and then writes the segment to storage. Read back from storage, and verify that client IDs exist in storage.
...
// TestActivityLog_StoreAndReadHyperloglog inserts into a hyperloglog, stores it and then reads it back. The test verifies the estimate count is correct
...
// TestModifyResponseMonthsNilAppend calls modifyResponseMonths for a range of 5 months ago to now. It verifies that the 5 months in the range are correct
...
// TestActivityLog_ReceivedFragment calls receivedFragment with a fragment and verifies it gets added to standbyFragmentsReceived. Send the same fragment again and then verify that it doesn't change the entity map but does get added to standbyFragmentsReceived
...
// TestActivityLog_availableLogsEmptyDirectory verifies that availableLogs returns an empty slice when the log directory is empty
...
// TestActivityLog_availableLogs writes to the direct token paths and entity paths and verifies that the correct start times are returned
...
// TestActivityLog_tokenCountExists writes to the direct tokens segment path and verifies that segment count exists returns true for the segments at these paths
...
// TestActivityLog_loadCurrentClientSegment writes entity segments and calls loadCurrentClientSegment, then verifies that the correct values are returned when querying the current segment
...
// TestActivityLog_loadPriorEntitySegment writes entities to two months and calls loadPriorEntitySegment for each month, verifying that the active clients are correct
...
// TestActivityLog_StopAndRestart disables the activity log, waits for deletes to complete, and then enables the activity log. The activity log is then stopped and started again, to simulate a seal and unseal. The test then verifies that there's no error adding an entity, direct token, and when writing a segment to storage.
...
// TestActivityLog_refreshFromStoredLog writes records for 3 months ago and this month, then calls refreshFromStoredLog. The test verifies that current entities and current tokens are correct.
...
// TestActivityLog_refreshFromStoredLogWithBackgroundLoadingCancelled writes data from 3 months ago to this month. The test closes a.doneCh and calls refreshFromStoredLog, which will not do any processing because the doneCh is closed. The test verifies that the current data is not loaded.
...
// TestActivityLog_refreshFromStoredLogContextCancelled writes data from 3 months ago to this month and calls refreshFromStoredLog with a canceled context, verifying that the function errors because of the canceled context
...
// TestActivityLog_refreshFromStoredLogNoTokens writes only entities from 3 months ago to today, then calls refreshFromStoredLog. It verifies that there are no tokens loaded
...
// TestActivityLog_refreshFromStoredLogNoEntities writes only direct tokens from 3 months ago to today, and runs refreshFromStoredLog. It verifies that there are no entities or clients loaded.
...
// TestActivityLog_refreshFromStoredLogNoData writes nothing and calls refreshFromStoredLog, and verifies that the current segment counts are zero
...
// TestActivityLog_refreshFromStoredLogTwoMonthsPrevious creates segment data from 5 months ago to 2 months ago nad calls refreshFromStoredLog, then verifies that the current segment counts are zero
...
// TestActivityLog_refreshFromStoredLogPreviousMonth creates segment data from 4 months ago to 1 month ago, then calls refreshFromStoredLog, then verifies that these clients are included in the current segment
...
// TestActivityLog_Export writes overlapping client for 5 months with various mounts and namespaces. It performs an for various month ranges in the range, and verifies that the outputs are correct
...
// TestActivityLog_IncludeNamespace verifies that includeInResponse returns true for namespaces that are children of their parents
...
// TestActivityLog_DeleteWorker writes segments for entities and direct tokens for 2 different timestamps, then runs the deleteLogWorker for one of the timestamps. The test verifies that the correct segment is deleted, and the other remains
...
// TestActivityLog_EnableDisable writes a segment, adds an entity to the in-memory fragment, then disables the activity log. The test verifies that the segment doesn't exist. The activity log is enabled, then verified that an empty segment is written and new clients can be added and written to segments
...
// TestActivityLog_Precompute creates segments over a range of 11 months, with overlapping clients and namespaces. Create intent logs and run precomputedQueryWorker for various month ranges. Verify that the precomputed queries have the correct counts, including per namespace.
...
// TestActivityLog_PrecomputeCancel stops the activity log before running the precomputedQueryWorker, and verifies that the context used to query storage has been canceled.
...
// TestActivityLog_NextMonthStart sets the activity log start timestamp, then verifies that StartOfNextMonth returns the correct value
...
// TestActivityLog_Deletion writes entity, direct tokens, and queries for dates ranging over 20 months. Then the test calls the retentionWorker with decreasing retention values, and verifies that the correct paths are being deleted.
...
// TestActivityLog_partialMonthClientCount writes segment data for the curren month and runs refreshFromStoredLog and then partialMonthClientCount. The test verifies that the values returned by partialMonthClientCount are correct
...
// TestActivityLog_partialMonthClientCountUsingHandleQuery writes segments for the current month and calls refreshFromStoredLog, then handleQuery. The test verifies that the results from handleQuery are correct
