...
// TestActivityLog_NewClientsCurrentMonth checks the current month
// approximation for new clients, in a wide variety of scenarios.
func TestActivityLog_NewClientsCurrentMonth(t *testing.T) {
	timeutil.SkipAtEndOfMonth(t)

	makeMonths := func(rawData ...[]int) map[int][]int {
		monthsDataRaw := make(map[int][]int, len(rawData))
		for index, data := range rawData {
			monthsDataRaw[index] = data
		}
		return monthsDataRaw
	}

	tests := []struct {
		expectedNewClientsAccuracy float64
		monthData                  map[int][]int
	}{
		{
			1,
			makeMonths(
	...
	for i, tt := range tests {
		...
		t.Run(testname, func(t *testing.T) {
			currentMonthResponse := testNewClientsInternal(t, tt.monthData)
			p, _ := time.Parse(time.RFC3339, currentMonthResponse.Timestamp)
			// Ensure that the current month returned is in fact current
			if !p.Equal(timeutil.StartOfMonth(time.Now().UTC()).UTC()) {
				t.Fatalf("partial month was not computed, %+v, %+v", p, timeutil.StartOfMonth(time.Now().UTC()).UTC())
			}
			fmt.Printf("for test %s, new clients are %+v, total clients are %+v", testname, currentMonthResponse.NewClients.Counts, currentMonthResponse.Counts)

			// Check specific error rate for new clients value (note that these are random, so the compared value
			// is an approximate lower bound)
			expectedNewClients := tt.monthData[0][0]
			newClientsError := math.Abs((float64)(currentMonthResponse.NewClients.Counts.Clients - expectedNewClients))
			newClientsErrorMargin := newClientsError / (float64)(expectedNewClients)
			if newClientsErrorMargin > math.Round((1-tt.expectedNewClientsAccuracy)*100)/100 {
				t.Fatalf("bad accuracy: expected %+v, found %+v", 1-tt.expectedNewClientsAccuracy, newClientsErrorMargin)
			}
		})
	}
	// t.Fatalf("gotta do this")
}

// testNewClientsInternal is a helper that does precomputations.
// monthsData: map of int to []int. The keys are months, 0 being the
// current month, 1 being the month previous, etc., and the values
// are 4 element lists (call it L) where L[0] is the number of new entities,
// L[1] is the number of duplicate (old) entities, L[2] is the number of new
// nonentities, and L[3] is the number of old nonentities, and L[4] is the number of
// segments over which the data is split.
func testNewClientsInternal(t *testing.T, monthsDataRaw map[int][]int) *ResponseMonth {
	// core setup
	core, _, _, _ := TestCoreUnsealedWithMetrics(t)
	a := core.activityLog
	ctx := namespace.RootContext(nil)
	...
	monthsData := make(map[time.Time][]int, 0)

	for monthIndex, monthDataVals := range monthsDataRaw {
		calculatedMonth := currentMonth
		// subtract months up to the number specified by the month index
		for subtractMonthCount := 0; subtractMonthCount < monthIndex; subtractMonthCount++ {
			calculatedMonth = timeutil.StartOfPreviousMonth(calculatedMonth)
		}
		monthsData[calculatedMonth] = monthDataVals
	}

	// Copy the months to a list and sort them, so we can input them in order
	sortedMonths := make([]time.Time, 0, len(monthsData))
	for month := range monthsData {
		sortedMonths = append(sortedMonths, month)
	}
	// sort months from oldest to newest
	sort.Slice(sortedMonths, func(i, j int) bool {
		return sortedMonths[i].Before(sortedMonths[j])
	})

	makeRecords := func(start, end, overlapEnt, overlapNonEnt int) []*activity.EntityRecord {
		entityRecords := make([]*activity.EntityRecord, (end-start)+overlapEnt+overlapNonEnt)

		for i := range entityRecords {
			if start+i < end {
				entityRecords[i] = &activity.EntityRecord{
					ClientID:  fmt.Sprintf("111122222-3333-4444-5555-%012v", start+i),
					Timestamp: time.Now().Unix(),
				}
			} else {
				oldClientID := 0
				if start != 0 {
					...
					oldClientID = rand.Intn(start)
				}

				entityRecords[i] = &activity.EntityRecord{
					ClientID:  fmt.Sprintf("111122222-3333-4444-5555-%012v", oldClientID),
					Timestamp: time.Now().Unix(),
				}
			}
			i += 1
			// if i == 0 {
			// 	fmt.Printf("client ID is %s\n", fmt.Sprintf("111122222-3333-4444-5555-%012v", start+i))
			// }
	...
	clientIDCounter := 0
	for index, month := range sortedMonths[:len(sortedMonths)-1] {
		currentMonthData := monthsData[month]
		numSegmentsAdded := -1
		numSegmentsForMonth := currentMonthData[4] // this value is off by 1 to preserve the pathing (eg =0 means 1 segment,etc.)
		for numSegmentsAdded < numSegmentsForMonth {
			newClientsForMonthInSegment := int(currentMonthData[0] / (numSegmentsForMonth + 1))
			duplicateClientsForMonthInSegment := int(currentMonthData[1] / (numSegmentsForMonth + 1))
			clients := makeRecords(clientIDCounter, newClientsForMonthInSegment+clientIDCounter, duplicateClientsForMonthInSegment, 0)
			clientIDCounter += newClientsForMonthInSegment
			eal := &activity.EntityActivityLog{
				Clients: clients,
			}
			data, err := proto.Marshal(eal)
			...
			path := fmt.Sprintf("%ventity/%v/%v", ActivityLogPrefix, month.Unix(), numSegmentsAdded+1)
			WriteToStorage(t, core, path, data)
			numSegmentsAdded += 1
		}

		intent := &ActivityIntentLog{
			PreviousMonth: month.Unix(),
			NextMonth:     sortedMonths[index+1].Unix(),
		}
		intentData, err := json.Marshal(intent)
		if err != nil {
			t.Fatal(err)
		}
		WriteToStorage(t, core, "sys/counters/activity/endofmonth", intentData)

		// Pretend we've successfully rolled over to the following month
		a.SetStartTimestamp(sortedMonths[index+1].Unix())
	...
	// Current month stuff
	currentMonthData := monthsData[sortedMonths[len(sortedMonths)-1]]

	for i := 0; i < currentMonthData[0]; i++ {
		te := &logical.TokenEntry{
			EntityID: fmt.Sprintf("111122222-3333-4444-5555-%012v", clientIDCounter+i),
		}
		id, isTWE := te.CreateClientID()
		a.HandleTokenUsage(context.Background(), te, id, isTWE)
	}
	for i := 0; i < currentMonthData[1]; i++ {
		rand.Seed(time.Now().UnixNano())
		prevID := rand.Intn(clientIDCounter)
		te := &logical.TokenEntry{
			EntityID: fmt.Sprintf("111122222-3333-4444-5555-%012v", prevID),
		}
		id, isTWE := te.CreateClientID()
		a.HandleTokenUsage(context.Background(), te, id, isTWE)
	}
	queryResponse, err := a.handleQuery(ctx, sortedMonths[0], sortedMonths[len(sortedMonths)-1], 0)
	if err != nil {
		t.Fatal(err)
	}

	a.logger.Info("totals in query are", "totals", queryResponse["total"])
	queryResponseMonths := queryResponse["months"].([]*ResponseMonth)
	currentMonthResponse := queryResponseMonths[len(queryResponseMonths)-1]
