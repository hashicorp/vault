...
func (c *Core) consumptionBillingMetricsWorker(ctx context.Context) {
	go func() {
		...
		clock := c.consumptionBilling.BillingConfig.Clock
		...
		for {
			...
			case <-endOfMonth.C:
				...
				c.HandleEndOfMonth(ctx, currentMonth)
...
// HandleEndOfMonth cleans up monthly billing data from
// n-2 months ago, and also resets all in memory billing metrics when the end of the month is reached.
func (c *Core) HandleEndOfMonth(ctx context.Context, currentMonth time.Time) {
	c.logger.Debug("handling end of month operations", "currentMonth", currentMonth)
...
func (c *Core) deletePreviousMonthBillingMetrics(ctx context.Context, currentMonth time.Time) error {
	...
	for _, localPathPrefix := range []string{billing.ReplicatedPrefix, billing.LocalPrefix} {
		billingPath := billing.GetMonthlyBillingPath(localPathPrefix, twoMonthsAgo)
