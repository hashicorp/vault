...
func (rlq *RateLimitQuota) allow(ctx context.Context, req *Request) (Response, error) {
	...
	if v, ok := rlq.blockedClients.Load(key); ok {
		...
		if time.Since(blockedAt) >= rlq.BlockInterval {
			...
			resp.Allowed = false
			retryAfter = rlq.retryAfterSeconds(blockedAt.Add(rlq.BlockInterval))
			return resp, nil
		}
	}
	...
	resp.Headers[httplimit.HeaderRateLimitReset] = rlq.retryAfterSeconds(time.Unix(0, int64(reset)))
...
func (rlq *RateLimitQuota) retryAfterSeconds(waitUntil time.Time) string {
	return strconv.Itoa(int(math.Ceil(time.Until(waitUntil).Seconds())))
}
