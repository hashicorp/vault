...
func TestCore_ListExternalSecretPlugins(t *testing.T) {
	...
	core.mounts.Entries = append(core.mounts.Entries,
		...
		// Official tier plugin: should be excluded.
		&MountEntry{Table: mountTableType, Path: "official/", Type: secretPluginV1.Name, RunningVersion: secretPluginV1.Version},
	...
	// Expect only the non-official external secrets plugin (v2) once and v3 once.
	require.Len(t, got, 3)
