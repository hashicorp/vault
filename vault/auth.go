...
func (c *Core) setupCredentials(ctx context.Context) error {
	...
	for _, entry := range c.auth.sortEntriesByPathDepth().Entries {
		...
		if err != nil {
			...
			if plug, plugerr := c.pluginCatalog.Get(ctx, entry.Type, consts.PluginTypeCredential, entry.Version); plugerr == nil && !plug.Builtin {
...
func (c *Core) newCredentialBackend(ctx context.Context, entry *MountEntry, sysView logical.SystemView, view logical.Storage) (logical.Backend, error) {
	...
	if !ok {
		plug, err := c.pluginCatalog.Get(ctx, entry.Type, consts.PluginTypeCredential, entry.Version)
		if err != nil {
			return nil, err
		}
		if plug == nil {
