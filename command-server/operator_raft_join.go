...
func (c *OperatorRaftJoinCommand) Flags() *command.FlagSets {
...
func (c *OperatorRaftJoinCommand) Run(args []string) int {
	...
	leaderCACert, err := command.ParseFlagFile(c.flagLeaderCACert)
	if err != nil {
		c.UI.Error(fmt.Sprintf("Failed to parse leader CA certificate: %s", err))
		return 1
	}

	leaderClientCert, err := command.ParseFlagFile(c.flagLeaderClientCert)
	if err != nil {
		c.UI.Error(fmt.Sprintf("Failed to parse leader client certificate: %s", err))
		return 1
	}

	leaderClientKey, err := command.ParseFlagFile(c.flagLeaderClientKey)
	if err != nil {
