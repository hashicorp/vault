...
import (
	...
	agentapprole "github.com/hashicorp/vault/command-server/agentproxyshared/auth/approle"
...
func TestCache_UsingAutoAuthToken(t *testing.T) {
	...
	mux.Handle("/", cache2.ProxyHandler(ctx, cacheLogger, leaseCache, inmemSink, true))
	server := &http.Server{
		Handler:           mux,
