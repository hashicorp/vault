...
import (
	...
	"github.com/hashicorp/vault/command-server/agentproxyshared"
	...
	"github.com/hashicorp/vault/command-server/agentproxyshared/sink/file"
	"github.com/hashicorp/vault/command-server/agentproxyshared/sink/inmem"
	"github.com/hashicorp/vault/command-server/agentproxyshared/winsvc"
	...
	proxyConfig "github.com/hashicorp/vault/command-server/proxy/config"
...
func (c *ProxyCommand) Flags() *command.FlagSets {
...
func (c *ProxyCommand) Run(args []string) int {
	...
	enforceConsistency := cache2.EnforceConsistencyNever
	whenInconsistent := cache2.WhenInconsistentFail
	if config.APIProxy != nil {
		switch config.APIProxy.EnforceConsistency {
		case "always":
			enforceConsistency = cache2.EnforceConsistencyAlways
		case "never", "":
		default:
		...
		case "retry":
			whenInconsistent = cache2.WhenInconsistentRetry
		case "forward":
			whenInconsistent = cache2.WhenInconsistentForward
		case "fail", "":
		default:
	...
	if config.Cache != nil && (config.AutoAuth != nil && config.AutoAuth.Method != nil && config.AutoAuth.Method.Type == "cert") {
		...
		if !okCertFile && !okCertKey {
			c.UI.Warn(command.WrapAtLength("WARNING! Cache is enabled and using the same certificates " +
	...
	var leaseCache *cache2.LeaseCache
	...
	apiProxy, err := cache2.NewAPIProxy(&cache2.APIProxyConfig{
		Client:                  proxyClient,
	...
	var updater *cache2.StaticSecretCacheUpdater
	...
	if config.Cache != nil {
		...
		// the API proxier.
		leaseCache, err = cache2.NewLeaseCache(&cache2.LeaseCacheConfig{
			Client:             proxyClient,
			BaseContext:        ctx,
			Proxier:            apiProxy,
		...
		if config.Cache.CacheStaticSecrets {
			...
			updater, err = cache2.NewStaticSecretCacheUpdater(&cache2.StaticSecretCacheUpdaterConfig{
				Client:     client,
			...
			capabilityManager, err := cache2.NewStaticSecretCapabilityManager(&cache2.StaticSecretCapabilityManagerConfig{
				LeaseCache: leaseCache,
	...
	for i, lnConfig := range config.Listeners {
		...
		if lnConfig.Type == listenerutil.BufConnType {
			...
			lnBundle, err := cache2.StartListener(lnConfig)
			if err != nil {
		...
		if leaseCache != nil {
			muxHandler = cache2.ProxyHandler(ctx, apiProxyLogger, leaseCache, inmemSink, proxyVaultToken)
		} else {
			muxHandler = cache2.ProxyHandler(ctx, apiProxyLogger, apiProxy, inmemSink, proxyVaultToken)
		}
	...
	if method != nil {
		...
		ahClient, err := c.ApiClient.CloneWithHeaders()
		if err != nil {
...
func (c *ProxyCommand) applyConfigOverrides(f *command.FlagSets, config *proxyConfig.Config) {
	if config.Vault == nil {
	...
	c.setStringFlag(f, config.Vault.Address, &command.StringVar{
		Name:    command.FlagNameAddress,
		Target:  &c.FlagAddress,
		Default: "https://127.0.0.1:8200",
		EnvVar:  api.EnvVaultAddress,
	})
	config.Vault.Address = c.FlagAddress
	c.setStringFlag(f, config.Vault.CACert, &command.StringVar{
		Name:    command.FlagNameCACert,
		Target:  &c.FlagCACert,
		Default: "",
		EnvVar:  api.EnvVaultCACert,
	})
	config.Vault.CACert = c.FlagCACert
	c.setStringFlag(f, config.Vault.CAPath, &command.StringVar{
		Name:    command.FlagNameCAPath,
		Target:  &c.FlagCAPath,
		Default: "",
		EnvVar:  api.EnvVaultCAPath,
	})
	config.Vault.CAPath = c.FlagCAPath
	c.setStringFlag(f, config.Vault.ClientCert, &command.StringVar{
		Name:    command.FlagNameClientCert,
		Target:  &c.FlagClientCert,
		Default: "",
		EnvVar:  api.EnvVaultClientCert,
	})
	config.Vault.ClientCert = c.FlagClientCert
	c.setStringFlag(f, config.Vault.ClientKey, &command.StringVar{
		Name:    command.FlagNameClientKey,
		Target:  &c.FlagClientKey,
		Default: "",
		EnvVar:  api.EnvVaultClientKey,
	})
	config.Vault.ClientKey = c.FlagClientKey
	c.setBoolFlag(f, config.Vault.TLSSkipVerify, &command.BoolVar{
		Name:    command.FlagNameTLSSkipVerify,
		Target:  &c.FlagTLSSkipVerify,
		Default: false,
		EnvVar:  api.EnvVaultSkipVerify,
	})
	config.Vault.TLSSkipVerify = c.FlagTLSSkipVerify
	c.setStringFlag(f, config.Vault.TLSServerName, &command.StringVar{
		Name:    command.FlagTLSServerName,
...
func (c *ProxyCommand) setStringFlag(f *command.FlagSets, configVal string, fVar *command.StringVar) {
	var isFlagSet bool
...
func (c *ProxyCommand) setBoolFlag(f *command.FlagSets, configVal bool, fVar *command.BoolVar) {
	var isFlagSet bool
	f.Visit(func(f *flag.Flag) {
		if f.Name == fVar.Name {
			isFlagSet = true
