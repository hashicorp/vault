...
func (c *ServerCommand) Flags() *command.FlagSets {
	set := c.FlagSet(command.FlagSetHTTP)
	...
	f.StringSliceVar(&command.StringSliceVar{
		Name:   "config",
	...
	f.StringSliceVar(&command.StringSliceVar{
		...
		Usage: "Name of an experiment to enable. Experiments should NOT be used in production, and " +
			...
			fmt.Sprintf("specified via the %s environment variable as a comma-separated list. ", command.EnvVaultExperiments) +
...
func (c *ServerCommand) parseConfig() (*server2.Config, []configutil.ConfigError, error) {
	var configErrors []configutil.ConfigError
	// Load the configuration
	var config *server2.Config
	for _, path := range c.flagConfigs {
		current, err := server2.LoadConfig(path)
		if err != nil {
...
func (c *ServerCommand) runRecoveryMode() int {
	...
	if config == nil {
		c.UI.Output(command.WrapAtLength(
	...
	c.FlagSets.ApplyLogConfigOverrides(config.SharedConfig)
	...
	for _, lnConfig := range config.Listeners {
		ln, _, _, err := server2.NewListener(lnConfig, c.logGate, c.UI)
		if err != nil {
	...
	if newCoreError != nil {
		c.UI.Warn(command.WrapAtLength(
...
func (c *ServerCommand) setupStorage(config *server2.Config) (physical.Backend, error) {
	...
	case storageTypeConsul:
		if config.ServiceRegistration == nil {
			...
			config.ServiceRegistration = &server2.ServiceRegistration{
				Type:   "consul",
...
func beginServiceRegistration(c *ServerCommand, config *server2.Config) (sr.ServiceRegistration, error) {
...
func (c *ServerCommand) InitListeners(config *server2.Config, disableClustering bool, infoKeys *[]string, info *map[string]string) (int, []listenerutil.Listener, []*net.TCPAddr, error) {
	...
	for i, lnConfig := range config.Listeners {
		ln, props, reloadFunc, err := server2.NewListener(lnConfig, c.logGate, c.UI)
		if err != nil {
...
func configureDevTLS(c *ServerCommand) (func(), *server2.Config, string, error) {
	...
	if c.flagDevTLS {
		...
		config, err = server2.DevTLSConfig(devStorageType, certDir, extraSANs)

		f = func() {
			if err := os.Remove(fmt.Sprintf("%s/%s", certDir, server2.VaultDevCAFilename)); err != nil {
				c.UI.Error(err.Error())
			}

			if err := os.Remove(fmt.Sprintf("%s/%s", certDir, server2.VaultDevCertFilename)); err != nil {
				c.UI.Error(err.Error())
			}

			if err := os.Remove(fmt.Sprintf("%s/%s", certDir, server2.VaultDevKeyFilename)); err != nil {
				c.UI.Error(err.Error())
			}
		...
		config, err = server2.DevConfig(devStorageType)
	}
...
func (c *ServerCommand) Run(args []string) int {
	...
	if envLicensePath := os.Getenv(command.EnvVaultLicensePath); envLicensePath != "" {
		config.LicensePath = envLicensePath
	}
	if envLicense := os.Getenv(command.EnvVaultLicense); envLicense != "" {
		config.License = envLicense
	}

	if err := server2.ExperimentsFromEnvAndCLI(config, command.EnvVaultExperiments, c.flagExperiments); err != nil {
		c.UI.Error(err.Error())
		return 1
	}
	...
	if allowPendingRemoval := os.Getenv(consts.EnvVaultAllowPendingRemovalMounts); allowPendingRemoval != "" {
		...
		if err != nil {
			c.UI.Warn(command.WrapAtLength("WARNING! failed to parse " +
	...
	if !entCheckStorageType(&coreConfig) {
		c.UI.Warn("")
		c.UI.Warn(command.WrapAtLength(fmt.Sprintf("WARNING: storage configured to use %q which is not supported for Vault Enterprise, must be \"raft\" or \"consul\"", coreConfig.StorageType)))
		c.UI.Warn("")
	}
	...
	if !c.flagDev {
		...
		if strutil.StrListContains(inMemStorageTypes, coreConfig.StorageType) {
			c.UI.Warn("")
			c.UI.Warn(command.WrapAtLength(fmt.Sprintf("WARNING: storage configured to use %q which should NOT be used in production", coreConfig.StorageType)))
			c.UI.Warn("")
		}
	}
	...
	if c.flagDev {
		...
		if c.flagDevTLS {
			clusterJson.CACertPath = fmt.Sprintf("%s/%s", certDir, server2.VaultDevCAFilename)
		}
...
func (c *ServerCommand) configureLogging(config *server2.Config) (hclog.InterceptLogger, error) {
...
func (c *ServerCommand) reloadHCPLink(hcpLinkVault *hcp_link.HCPLinkVault, conf *server2.Config, core *vault.Core, hcpLogger hclog.Logger) (*hcp_link.HCPLinkVault, error) {
	// trigger a shutdown
	if hcpLinkVault != nil {
	...
	for {
		...
		if err == nil {
			if migrationStatus != nil {
				...
				c.UI.Error(command.WrapAtLength(fmt.Sprintf("ERROR! Storage migration in progress (started: %s). "+
...
func setSeal(c *ServerCommand, config *server2.Config, infoKeys []string, info map[string]string, existingSealGenerationInfo *vaultseal.SealGenerationInfo, hasPartiallyWrappedPaths bool) (*SetSealResponse, error) {
	...
	for _, configSeal := range config.Seals {
		...
		if wrapperConfigError == nil {
			...
			configuredSeals++
		} else if server2.IsMultisealSupported() {
	...
	case server2.IsMultisealSupported():
...
func (c *ServerCommand) computeSealGenerationInfo(existingSealGenInfo *vaultseal.SealGenerationInfo, sealConfigs []*configutil.KMS, hasPartiallyWrappedPaths bool) (*vaultseal.SealGenerationInfo, error) {
	...
	if server2.IsMultisealSupported() {
...
func initHaBackend(c *ServerCommand, config *server2.Config, coreConfig *vault.CoreConfig, backend physical.Backend) (bool, error) {
	// Initialize the separate HA storage backend, if it exists
	var ok bool
	if config.HAStorage != nil {
...
func determineRedirectAddr(c *ServerCommand, coreConfig *vault.CoreConfig, config *server2.Config) error {
	var retErr error
...
func findClusterAddress(c *ServerCommand, coreConfig *vault.CoreConfig, config *server2.Config, disableClustering bool) error {
	if disableClustering {
...
func createCoreConfig(c *ServerCommand, config *server2.Config, backend physical.Backend, configSR sr.ServiceRegistration, barrierSeal, unwrapSeal vault.Seal,
...
func runListeners(c *ServerCommand, coreConfig *vault.CoreConfig, config *server2.Config, configSR sr.ServiceRegistration) error {
...
func initDevCore(c *ServerCommand, coreConfig *vault.CoreConfig, config *server2.Config, core *vault.Core, certDir string, clusterJSON *testcluster.ClusterJson) error {
...
// Initialize the HTTP servers
func startHttpServers(c *ServerCommand, core *vault.Core, config *server2.Config, lns []listenerutil.Listener) error {
	for _, ln := range lns {
		if ln.Config == nil {
