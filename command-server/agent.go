...
import (
	...
	agentConfig "github.com/hashicorp/vault/command-server/agent/config"
	"github.com/hashicorp/vault/command-server/agent/exec"
	"github.com/hashicorp/vault/command-server/agent/template"
...
func (c *AgentCommand) Flags() *command.FlagSets {
...
func (c *AgentCommand) Run(args []string) int {
	...
	if config.Cache != nil {
		...
		case "always":
			if enforceConsistency != cache2.EnforceConsistencyNever {
		...
		case "retry":
			if whenInconsistent != cache2.WhenInconsistentFail {
...
func (c *AgentCommand) applyConfigOverrides(f *command.FlagSets, config *agentConfig.Config) {
	if config.Vault == nil {
...
func (c *AgentCommand) setStringFlag(f *command.FlagSets, configVal string, fVar *command.StringVar) {
	var isFlagSet bool
...
func (c *AgentCommand) setBoolFlag(f *command.FlagSets, configVal bool, fVar *command.BoolVar) {
	var isFlagSet bool
	f.Visit(func(f *flag.Flag) {
		if f.Name == fVar.Name {
			isFlagSet = true
