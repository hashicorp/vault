...
import (
	...
	cserver "github.com/hashicorp/vault/command-server/server"
...
func (c *OperatorDiagnoseCommand) Flags() *command.FlagSets {
	set := command.NewFlagSets(c.UI)
...
SEALFAIL:
	...
	diagnose.Test(ctx, "Check Core Creation", func(ctx context.Context) error {
		...
		if newCoreError != nil {
			...
			diagnose.Warn(ctx, command.WrapAtLength(
