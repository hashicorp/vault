...
func TestMigration(t *testing.T) {
	t.Run("Default", func(t *testing.T) {
		...
		fromFactory := command.PhysicalBackends["file"]
		...
		toFactory := command.PhysicalBackends["inmem"]
	...
	t.Run("Start option", func(t *testing.T) {
		...
		fromFactory := command.PhysicalBackends["inmem"]
		confFrom := map[string]string{}
		from, err := fromFactory(confFrom, nil)
		if err != nil {
			t.Fatal(err)
		...
		toFactory := command.PhysicalBackends["file"]
		folder := t.TempDir()
		confTo := map[string]string{
			"path": folder,
		}
...
storage_destination "dest_type2" {
  path = "dest_path"
	...
	t.Run("DFS Scan", func(t *testing.T) {
		s, _ := command.PhysicalBackends["inmem"](map[string]string{}, nil)
