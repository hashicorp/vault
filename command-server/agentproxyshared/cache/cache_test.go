...
import (
	...
	"github.com/hashicorp/vault/command-server/agentproxyshared/sink/mock"
...
func testCachingCacheClearCommon(t *testing.T, clearType string) {
	...
	idx, err := leaseCache.db.Get(cachememdb2.IndexNameLease, gotLeaseID)
	if err != nil {
		t.Fatal(err)
	}
...
func TestCache_AuthTokenCreateOrphan(t *testing.T) {
	t.Run("create", func(t *testing.T) {
		t.Run("managed", func(t *testing.T) {
			...
			idx, err := leaseCache.db.Get(cachememdb2.IndexNameToken, token)
			if err != nil {
				t.Fatal(err)
			}
			if idx == nil {
