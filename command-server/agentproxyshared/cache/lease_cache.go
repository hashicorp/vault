...
func (c *LeaseCache) checkCacheForRequest(id string, req *SendRequest) (*SendResponse, error) {
	...
	if errors.Is(err, cachememdb2.ErrCacheItemNotFound) {
		return nil, nil
	}
	if err != nil {
		return nil, err
...
func (c *LeaseCache) cacheStaticSecret(ctx context.Context, req *SendRequest, resp *SendResponse, index *cachememdb2.Index) error {
	// If a cached version of this secret exists, we now have access, so
	// we don't need to re-cache, just update index.Tokens
	indexFromCache, err := c.db.Get(cachememdb2.IndexNameID, index.ID)
	if err != nil && err != cachememdb2.ErrCacheItemNotFound {
		return err
	}
...
func (c *LeaseCache) storeStaticSecretIndex(ctx context.Context, req *SendRequest, index *cachememdb2.Index) error {
	// Store the index in the cache
...
func (c *LeaseCache) retrieveOrCreateTokenCapabilitiesEntry(token string) (*cachememdb2.CapabilitiesIndex, bool, error) {
	// The index ID is a hash of the token.
	indexId := hashStaticSecretIndex(token)
	indexFromCache, err := c.db.GetCapabilitiesIndex(cachememdb2.IndexNameID, indexId)
	if err != nil && err != cachememdb2.ErrCacheItemNotFound {
...
func (c *LeaseCache) handleCacheClear(ctx context.Context, in *cacheClearInput) error {
	...
	case "request_path":
		...
		for _, index := range indexes {
			...
			if index.Type == cacheboltdb.StaticSecretType {
				err = c.db.Evict(cachememdb2.IndexNameID, index.ID)
				if err != nil {
					return err
				}
			} else {
...
// cache (if enabled)
func (c *LeaseCache) SetCapabilitiesIndex(ctx context.Context, index *cachememdb2.CapabilitiesIndex) error {
	...
	if c.cacheStaticSecrets {
		...
		if err != nil {
			...
			for _, staticSecret := range staticSecrets {
				newIndex, err := cachememdb2.Deserialize(staticSecret)
				if err != nil {
					errs = multierror.Append(errs, err)
					continue
				}
		...
		if err != nil {
			...
			for _, capabilityIndex := range capabilityIndexes {
				newIndex, err := cachememdb2.DeserializeCapabilitiesIndex(capabilityIndex)
				if err != nil {
					errs = multierror.Append(errs, err)
					continue
				}
