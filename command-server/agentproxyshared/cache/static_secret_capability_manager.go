...
func (sscm *StaticSecretCapabilityManager) StartRenewingCapabilities(indexToRenew *cachememdb2.CapabilitiesIndex) {
	var work func()
	work = func() {
		if sscm.workerPool.Stopped() {
		...
		capabilitiesIndex, err := sscm.leaseCache.db.GetCapabilitiesIndex(cachememdb2.IndexNameID, indexToRenew.ID)
		...
		for _, path := range indexReadablePaths {
			...
			if _, ok := newReadablePaths[path]; !ok {
				...
				index, err := sscm.leaseCache.db.Get(cachememdb2.IndexNameID, indexId)
				if errors.Is(err, cachememdb2.ErrCacheItemNotFound) {
					// Nothing to update!
					continue
				}
		...
		if len(newReadablePaths) == 0 {
			err := sscm.leaseCache.db.EvictCapabilitiesIndex(cachememdb2.IndexNameID, indexToRenew.ID)
			if err != nil {
