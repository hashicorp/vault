...
import (
	...
	"github.com/hashicorp/vault/command-server/agentproxyshared/cache/cacheboltdb"
	cachememdb2 "github.com/hashicorp/vault/command-server/agentproxyshared/cache/cachememdb"
	"github.com/hashicorp/vault/command-server/agentproxyshared/cache/keymanager"
...
func TestLeaseCache_SendCacheable(t *testing.T) {
	...
	// Check TokenParent
	cachedItem, err := lc.db.Get(cachememdb2.IndexNameToken, "testtoken")
	if err != nil {
		t.Fatal(err)
	}
	if cachedItem == nil {
...
func TestLeaseCache_StoreCacheableStaticSecret(t *testing.T) {
	...
	indexFromDB, err := lc.db.Get(cachememdb2.IndexNameID, index.ID)
	if err != nil {
		return
	}
	...
	capabilitiesIndexFromDB, err := lc.db.GetCapabilitiesIndex(cachememdb2.IndexNameID, hex.EncodeToString(cryptoutil.Blake2b256Hash(index.Token)))
	if err != nil {
		return
	}
	...
	expectedClearedIndex, err := lc.db.Get(cachememdb2.IndexNameID, index.ID)
	require.Equal(t, cachememdb2.ErrCacheItemNotFound, err)
...
func TestLeaseCache_StaticSecret_CacheClear_All(t *testing.T) {
	...
	expectedClearedCapabilitiesIndex, err := lc.db.GetCapabilitiesIndex(cachememdb2.IndexNameID, capabilitiesIndexFromDB.ID)
	require.Equal(t, cachememdb2.ErrCacheItemNotFound, err)
...
func TestLeaseCache_SendNonCacheableNonTokenLease(t *testing.T) {
	...
	_, err = lc.db.Get(cachememdb2.IndexNameRequestPath, "root/", urlPath)
	if err != cachememdb2.ErrCacheItemNotFound {
...
func compareBeforeAndAfter(t *testing.T, before, after *LeaseCache, beforeLen, afterLen int) {
	beforeDB, err := before.db.GetByPrefix(cachememdb2.IndexNameID)
	require.NoError(t, err)
	assert.Len(t, beforeDB, beforeLen)
	afterDB, err := after.db.GetByPrefix(cachememdb2.IndexNameID)
	require.NoError(t, err)
	...
	for _, cachedItem := range beforeDB {
		...
		restoredItem, err := after.db.Get(cachememdb2.IndexNameID, cachedItem.ID)
		require.NoError(t, err)
...
func TestLeaseCache_PersistAndRestore_WithManyDependencies(t *testing.T) {
	...
	for _, lease := range leases {
		index, err := cachememdb2.Deserialize(lease)
		require.NoError(t, err)
...
func TestRegisterAutoAuth_sameToken(t *testing.T) {
	...
	oldTokenIndex, err := lc.db.Get(cachememdb2.IndexNameToken, "autoauthtoken")
	assert.NoError(t, err)
	...
	entries, err := lc.db.GetByPrefix(cachememdb2.IndexNameToken, "autoauthtoken")
	assert.NoError(t, err)
	assert.Len(t, entries, 1)

	newTokenIndex, err := lc.db.Get(cachememdb2.IndexNameToken, "autoauthtoken")
	assert.NoError(t, err)
...
func Test_hasExpired(t *testing.T) {
	...
	for _, ct := range cacheTests {
		...
		index, err := lc.db.Get(cachememdb2.IndexNameRequestPath, "root/", ct.urlPath)
		require.NoError(t, err)
...
func TestLeaseCacheRestore_expired(t *testing.T) {
	...
	beforeDB, err := lc.db.GetByPrefix(cachememdb2.IndexNameID)
	require.NoError(t, err)
	assert.Len(t, beforeDB, 3)

	// There should only be one item in the restored cache: the autoauth token
	afterDB, err := restoredCache.db.GetByPrefix(cachememdb2.IndexNameID)
	require.NoError(t, err)
	...
	beforeItem, err := lc.db.Get(cachememdb2.IndexNameID, afterDB[0].ID)
	require.NoError(t, err)
