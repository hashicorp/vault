...
func TestSubmitWorkNoOp(t *testing.T) {
	...
	index := &cachememdb2.CapabilitiesIndex{
		ID: "test",
	}
...
func TestSubmitWorkUpdatesIndex(t *testing.T) {
	...
	newIndex, err := sscm.leaseCache.db.GetCapabilitiesIndex(cachememdb2.IndexNameID, indexId)
	require.Nil(t, err)
...
func TestSubmitWorkUpdatesIndexWithBadToken(t *testing.T) {
	...
	newIndex, err := sscm.leaseCache.db.GetCapabilitiesIndex(cachememdb2.IndexNameID, indexId)
	require.Equal(t, err, cachememdb2.ErrCacheItemNotFound)
...
func TestSubmitWorkUpdatesAllIndexes(t *testing.T) {
	...
	newPathIndex1, err := sscm.leaseCache.db.Get(cachememdb2.IndexNameID, pathIndexId1)
	require.Nil(t, err)
	require.Equal(t, map[string]struct{}{}, newPathIndex1.Tokens)

	// For this, we expect no change
	newPathIndex2, err := sscm.leaseCache.db.Get(cachememdb2.IndexNameID, pathIndexId2)
	require.Nil(t, err)
