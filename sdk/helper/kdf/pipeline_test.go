...
func TestSubsetNISTCAVPPipelineKBKDF(t *testing.T) {
	for index, test := range pipelineModeTestCases {
		...
		kbkdf, err := NewPipeline(prf, params, []int{test.L})
		if err != nil {
			t.Fatalf("unexpected error creating Pipeline KDF: %v", err)
		}
		...
		if err != nil {
			t.Fatalf("unexpected error reading from Pipeline KDF: %v", err)
		}
