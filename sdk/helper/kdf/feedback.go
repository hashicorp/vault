...
// Creates a new SP800-108 Feedback Mode KDF instance. This function takes
// a PRF instance (preferably HMAC or CMAC) already initialized with a key,
// an ordered list of KDF parameters to use (see params.go), an initialization
// vector (for the first PRF invocation), and a list of bit lengths of keys
// to derive (in order). When successfully constructed, the return is an
// io.Reader instance, which when called with byte arrays sized according to
// the specified originally specified keyLengths, is guaranteed to successfully
// return a key.
//
// See also NIST SP800-108 and PKCS#11 v3.0 for security concerns and
// more information about KBKDFParameters.
func NewFeedback(prf hash.Hash, params []KBKDFParameter, IV []byte, keyLengths []int) (io.Reader, error) {
	...
	if !foundChaining {
		return nil, fmt.Errorf("missing required parameter of type ChainingVariable for Feedback Mode KDFs")
	}
	...
	return &feedbackKDF{prf, params, keyLengths, 1, IV, 0}, nil
}
