...
func getChangeSubjectName(csr x509.CertificateRequest) (found bool, csn *ChangeSubjectName, err error) {
	for _, ext := range csr.Extensions {
		if ext.Id.Equal(OidExtensionChangeSubjectName) {
			// Relevant Information: https://datatracker.ietf.org/doc/html/rfc6402#section-2.8
			...
			type changeSubjectName struct {
				Subject pkix.RDNSequence `asn1:"optional"`
			}
			sn := &changeSubjectName{}
			leftover, err := asn1.Unmarshal(ext.Value, sn)
			if err != nil {
				return false, nil, fmt.Errorf("failed unmarshalling extension value: %w", err)
			}

			var subject pkix.Name
			subject.FillFromRDNSequence(&sn.Subject)

			changeSubjectNameObj := ChangeSubjectName{
				Subject: subject,
			}

			err = forEachSAN(leftover, func(tag int, data []byte) (err error) {
				switch tag {
				// Relevant Information: https://datatracker.ietf.org/doc/html/rfc5280#appendix-A.2
				...
				//     iPAddress                 [7]  OCTET STRING,
				//     registeredID              [8]  OBJECT IDENTIFIER }
				case 1: // rfc822Name is better known as an email
					changeSubjectNameObj.EmailAddresses = append(changeSubjectNameObj.EmailAddresses, string(data))
				case 2:
					changeSubjectNameObj.DNSNames = append(changeSubjectNameObj.DNSNames, string(data))
				case 6:
					url, err := url.Parse(string(data))
					if err != nil {
						return fmt.Errorf("error parsing url subject alt name %v: %w", data, err)
					}
					changeSubjectNameObj.URIs = append(changeSubjectNameObj.URIs, url)
				case 7:
					switch len(data) {
					case net.IPv4len, net.IPv6len:

						changeSubjectNameObj.IPAddresses = append(changeSubjectNameObj.IPAddresses, data)
					default:
						return errors.New("cannot parse IP address of length " + strconv.Itoa(len(data)))
					}
				case 0:
					var other OtherNameRaw
					_, err := asn1.UnmarshalWithParams(data, &other, "tag:0")
					...
					if changeSubjectNameObj.OtherSANs[val.Oid] == nil {
						changeSubjectNameObj.OtherSANs[val.Oid] = []string{val.Value}
					} else {
						changeSubjectNameObj.OtherSANs[val.Oid] = append(changeSubjectNameObj.OtherSANs[val.Oid], val.Value)
					}
				}
				return nil
			})
			return true, &changeSubjectNameObj, nil
		}
	}
