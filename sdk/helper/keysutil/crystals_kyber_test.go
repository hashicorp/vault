...
func TestDeriveAES256Key_BindsInputsAndIsDeterministic(t *testing.T) {
	secret := make([]byte, 32)
	...
	k1, err := deriveAES256Key(secret, cap1, ad, "kyber768-aes256-gcm-v1")
	if err != nil {
		t.Fatalf("deriveAES256Key: %v", err)
	}
	if len(k1) != 32 {
		t.Fatalf("want 32-byte key, got %d", len(k1))
	}

	k1b, err := deriveAES256Key(secret, cap1, ad, "kyber768-aes256-gcm-v1")
	if err != nil {
		t.Fatalf("deriveAES256Key: %v", err)
	}
	if !bytes.Equal(k1, k1b) {
		t.Fatalf("same inputs should derive same key")
	}

	kAD, _ := deriveAES256Key(secret, cap1, []byte("mismatch"), "kyber768-aes256-gcm-v1")
	if bytes.Equal(k1, kAD) {
		t.Fatalf("changing AD should change key")
	}

	kCap, _ := deriveAES256Key(secret, []byte("cap2"), ad, "kyber768-aes256-gcm-v1")
	if bytes.Equal(k1, kCap) {
		t.Fatalf("changing capsule should change key")
	}

	kLbl, _ := deriveAES256Key(secret, cap1, ad, "kyber1024-aes256-gcm-v1")
	if bytes.Equal(k1, kLbl) {
		t.Fatalf("changing label should change key")
	}

	kNilAD, _ := deriveAES256Key(secret, cap1, nil, "kyber768-aes256-gcm-v1")
	kEmptyAD, _ := deriveAES256Key(secret, cap1, []byte{}, "kyber768-aes256-gcm-v1")
	if !bytes.Equal(kNilAD, kEmptyAD) {
		t.Fatalf("nil AD and empty AD should derive same key")
	}
}

func TestNewGCMWithNonce_Success(t *testing.T) {
	key := bytes.Repeat([]byte{0}, 32)
	a, n, err := newGCMWithNonce(key)
	if err != nil {
		t.Fatalf("newGCMWithNonce: %v", err)
	}
	if len(n) != a.NonceSize() {
		t.Fatalf("nonce size mismatch: got %d want %d", len(n), a.NonceSize())
	}
}

func TestNewGCMWithNonce_InvalidKey_Err(t *testing.T) {
	badKey := bytes.Repeat([]byte{1}, 31)
	if a, n, err := newGCMWithNonce(badKey); err == nil || a != nil || n != nil {
