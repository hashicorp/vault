...
// jwt_audience, jwt_authorization_detail) are correctly
...
func TestEntryFormatter_Process_JSON_EnterpriseToken(t *testing.T) {
	...
	for name, tc := range cases {
		...
		t.Run(name, func(t *testing.T) {
			...
			require.Equal(t, tc.WantAuthorizationDetails, result.Auth.Metadata["jwt_authorization_detail"])
		})
	}
}
...
func TestEntryFormatter_EnterpriseTokenFieldsNotOnRequestOrAuthTopLevel(t *testing.T) {
	...
	tokenAuthzDetails, ok := metadataMap["jwt_authorization_detail"]
	require.True(t, ok)
