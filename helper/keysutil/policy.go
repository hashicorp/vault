...
type Policy struct {
	...
	// TrimmedMinVersion is the minimum version of the key present. All key
	// versions before this would have been deleted.
	TrimmedMinVersion int `json:"trimmed_min_version"`
...
func (p *Policy) handleArchiving(ctx context.Context, storage logical.Storage) error {
	...
	if !keysContainsMinimum {
		...
		for i := p.MinDecryptionVersion; i <= p.LatestVersion; i++ {
			p.Keys[strconv.Itoa(i)] = archive.Keys[i-p.TrimmedMinVersion]
		}
	...
	// key version
	if len(archive.Keys)+p.TrimmedMinVersion < p.LatestVersion+1 {
		// Increase the size of the archive slice
		newKeys := make([]KeyEntry, p.LatestVersion-p.TrimmedMinVersion+1)
	...
	for i := p.ArchiveVersion + 1; i <= p.LatestVersion; i++ {
		archive.Keys[i-p.TrimmedMinVersion] = p.Keys[strconv.Itoa(i)]
		p.ArchiveVersion = i
	}

	// Trim the keys if required
	if p.ArchiveMinVersion < p.TrimmedMinVersion {
		archive.Keys = archive.Keys[p.TrimmedMinVersion-p.ArchiveMinVersion:]
		p.ArchiveMinVersion = p.TrimmedMinVersion
	}
