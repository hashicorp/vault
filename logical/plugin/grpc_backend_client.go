...
func (b *backendGRPCPluginClient) HandleRequest(req *logical.Request) (*logical.Response, error) {
	if b.metadataMode {
	...
	reply, err := b.client.HandleRequest(req.Context, &pb.HandleRequestArgs{
		Request: protoReq,
	})
	if err != nil {
		return nil, err
...
func (b *backendGRPCPluginClient) HandleExistenceCheck(req *logical.Request) (bool, bool, error) {
	if b.metadataMode {
	...
	reply, err := b.client.HandleExistenceCheck(req.Context, &pb.HandleExistenceCheckArgs{
		Request: protoReq,
	})
	if err != nil {
		return false, false, err
